...
A Suspension Foot Bridge is a type of bridge that uses the features of a suspension bridge
 but is specifically designed for pedestrian use. These bridges are typically used to cross
 natural obstacles like valleys, rivers, or difficult terrain, whereas regular footbridges
...
In a suspension footbridge, steel or wire cables are suspended to support the bridge deck.
 These cables are anchored to tall towers on each side of the bridge, transferring the load from the deck to the towers.
Unlike regular footbridges, which are usually made of simple beams (either concrete or steel)
, a suspension footbridge’s primary structure consists of cables and towers.
Design Purpose:

These bridges are typically designed for pedestrian use, but compared to regular footbridges,
 they can span greater distances and are ideal for crossing deep valleys, rivers, or other natural barriers.
Suspension footbridges are often built in natural parks, hiking trails, or areas that require
 crossing significant natural obstacles.
Load Characteristics:

Suspension footbridges generally carry less load than full-scale suspension bridges (designed for vehicles or trains),
 as they are only built to support pedestrians. However, they still rely on suspended cables to carry the load.
The load on these bridges is primarily pedestrian traffic, but in some cases, they might also support bicycles
 or animals.
Advantages:

Longer Spans: Suspension footbridges can cover longer distances than regular pedestrian bridges without requiring
 additional supports in the middle of the span.
Crossing Natural Obstacles: These bridges are ideal for crossing rivers, valleys, and rugged terrain where building
 traditional bridges is difficult.
Aesthetic and Integration with Nature: Suspension footbridges often have a beautiful design that blends well with
 the natural surroundings, especially in tourist or recreational areas.
Construction and Installation:

Building a suspension footbridge requires specialized equipment for installing cables and towers. These bridges
 are commonly used in areas where the terrain is difficult, or access to traditional infrastructure is limited.
Installing cables and towers in such challenging locations requires precise engineering and safety considerations.
Suspension Foot Bridges in Mountainous Areas: Suspension footbridges are commonly found in mountainous regions or
 nature reserves where they allow pedestrians to cross valleys and rivers. These bridges provide access for both
 locals and tourists to difficult-to-reach areas.

Suspension Footbridges in Parks: In some natural parks or recreational areas, suspension footbridges are used
 to cross rivers or valleys. These bridges serve both a practical purpose and act as tourist attractions.

Differences from Regular Suspension Bridges:

Lower Load Capacity: While regular suspension bridges are designed to carry vehicles, trains, or heavy loads,
 and can bear significant stress, suspension footbridges are designed only for pedestrian use and therefore
 carry much lighter loads.
Smaller Dimensions: Suspension footbridges are generally smaller in scale and have shorter spans than full-size
 suspension bridges. They are primarily intended for pedestrians and are rarely used for vehicle traffic.
A Suspension Foot Bridge is a type of suspension bridge specifically designed for pedestrian use. Due to
 the use of suspended cables, these bridges can span longer distances and are very useful in areas where
 crossing valleys, rivers, or difficult terrain is necessary.
 
Due to the precast nature of the bridge concrete segments and the significant camber of the concrete deck,
 the flexural and axial effects of the concrete have been neglected in this type of modeling. Instead, only
 the effect of ten tendons along the bridge length has been considered for force transfer.

------------------------------------------------------------------------------------------------------------
Special thanks to Mr. Tanweer Malik for his constructive comments and the provision of high-quality images.
------------------------------------------------------------------------------------------------------------
"""
#%%------------------------------------------------------------------
...
image_path = 'OPENSEES_CABLE_SUSPENSION_BRIDGE_03.png'    
...
#   PUSHOVER ANALYSIS
# --------------------
def PUSHOVER_ANALYSIS(LINEAR, L, H1, arc_depth, E_cable, Cable_Dia_01, num_nodes, MAX_DISP, disp_incr, TOTAL_MASS, MAX_ITERATIONS, MAX_TOLERANCE):
    import openseespy.opensees as ops
    import numpy as np
    import matplotlib.pyplot as plt
    
    A_cable_01 = (np.pi * Cable_Dia_01 **2) / 4  # [mm^2] Horizontal Longitudinal Cable Area Bottom
    N_cable = 10                                 # Number of Cables
    ...
    for i in range(num_nodes - 1):
        ops.element('corotTruss', i + 1, i + 1, i + 2, N_cable*A_cable_01, 1)  # Deck element
  
    mid_node = num_nodes // 2 + 1
    #mid_node = int(0.5 * num_nodes)
    # Define load pattern with displacement at the middle span
    ops.timeSeries('Linear', 1)
    ops.pattern('Plain', 1, 1)
    ops.load(mid_node, 0.0, -1)  # Apply a vertical load at node
    #for i in range(num_nodes):
    #    ops.load(i+1, 0.0, -WEIGHT)  # Apply a vertical load at node
    ...
    for i in range(n_steps):
        ...
        DISPLACEMENTS.append([ops.nodeDisp(j + 1) for j in range(num_nodes)])
        ...
        BASE_REACTION_X.append(x1+x2) # CABLE REACION-X
        BASE_REACTION_Y.append(y1+y2) # CABLE REACION-Y
        print(f'STEP: {i+1}')
    
    # Get initial and final node coordinates for plotting
    initial_coords = np.array([ops.nodeCoord(i + 1) for i in range(num_nodes)])
...
L = 18000.0        # [mm] Bridge span lentgh
H1 = 500.0         # [mm] Height of Top Cable
arc_depth = 500.0  # [mm] Arc Depth
E_cable = 210e5    # [N/mm^2] Modulus of elasticity Cable
Cable_Dia_01 = 32  # [mm] Horizontal Longitudinal Cable Diameter Bottom
num_nodes = 51     # Cable Arc Number of nodes
MAX_DISP = -50.0   # [mm] Maximum Displacment
disp_incr = -0.01  # [mm] Incremental Displacement
TOTAL_MASS = 50000.0   # [kg] Total Mass of Structure
...
# Run the analysis
results = PUSHOVER_ANALYSIS(LINEAR, L, H1, arc_depth, E_cable, Cable_Dia_01, num_nodes, MAX_DISP, disp_incr, TOTAL_MASS, MAX_ITERATIONS, MAX_TOLERANCE)
...
S04.PLOT_2D_FRAME_TRUSS(deformed_scale=10)  # Adjust scale factor as needed
#%%------------------------------------------------------------------
#          ############################################################################################
#          #                                          IN THE NAME OF ALLAH                            #
...
# --------------------------
def FREE_VIBRATION_ANALYSIS(damping, damping_ratio, LINEAR, duration, dt, TOTAL_MASS, u0, L, H1, arc_depth, E_cable, Cable_Dia_01, num_nodes, MAX_ITERATIONS, MAX_TOLERANCE):
    import openseespy.opensees as ops
    import numpy as np
    import matplotlib.pyplot as plt
    ...
    # Define truss elements
    for i in range(num_nodes - 1):
        ops.element('corotTruss', i + 1, i + 1, i + 2, N_cable*A_cable_01, 1)  # Bottom Cable Element

    
    mid_node = num_nodes // 2 + 1
    #mid_node = int(num_nodes)
    # Define load pattern with displacement at the middle span
    ops.timeSeries('Linear', 1)
    ops.pattern('Plain', 1, 1)
    ops.load(mid_node, 0.0, -1)  # Apply a vertical load at node
    ...
    if damping == True:
        ...
        ops.eigen('-genBandArpack', 1) # eigenvalue mode 1
        Lambda01 = ops.eigen('-fullGenLapack', 10)  # eigenvalue mode 2
        #Lambda01 = ops.eigen('-genBandArpack', 10) # eigenvalue mode 2
...
E_cable = 210e5    # [N/mm^2] Modulus of elasticity Cable
Cable_Dia_01 = 32  # [mm] Horizontal Longitudinal Cable Diameter Bottom
num_nodes = 51     # Cable Arc Number of node

TOTAL_MASS = 50000.0   # [kg] Total Mass of Structure
u0 = 0.1               # [mm] Initial displacement
...
damping = False
LINEAR = True # False: Cable Nonlinear Materials Properties
results_undamped = FREE_VIBRATION_ANALYSIS(damping, damping_ratio, LINEAR, duration, dt, TOTAL_MASS, u0, L, H1, arc_depth, E_cable, Cable_Dia_01, num_nodes, MAX_ITERATIONS, MAX_TOLERANCE)
...
damping = True
LINEAR = True # False: Cable Nonlinear Materials Properties
results_damped = FREE_VIBRATION_ANALYSIS(damping, damping_ratio, LINEAR, duration, dt, TOTAL_MASS, u0, L, H1, arc_depth, E_cable, Cable_Dia_01, num_nodes, MAX_ITERATIONS, MAX_TOLERANCE)
...
#   DYNAMIC ANALYSIS
# -------------------
def DYNAMIC_ANALYSIS(damping, damping_ratio, LINEAR, periodTF, duration, dt, TOTAL_MASS, L, H1, arc_depth, E_cable, Cable_Dia_01, num_nodes, MAX_ITERATIONS, MAX_TOLERANCE):
    import openseespy.opensees as ops
    import numpy as np
    import matplotlib.pyplot as plt
...
LINEAR = True # False: Cable Nonlinear Materials Properties
periodTF = False
results_undamped = DYNAMIC_ANALYSIS(damping, damping_ratio, LINEAR, periodTF, duration, dt, TOTAL_MASS, L, H1, arc_depth, E_cable, Cable_Dia_01, num_nodes, MAX_ITERATIONS, MAX_TOLERANCE)
initial_coords, deformed_coords, DISPLACEMENTS_undamped, VELOCITY_undamped, ACCELERATION_undamped, DISP_X_undamped, DISP_Y_undamped, BASE_REACTION_X_undamped, BASE_REACTION_Y_undamped = results_undamped
...
LINEAR = True # False: Cable Nonlinear Materials Properties
periodTF = False
results_damped = DYNAMIC_ANALYSIS(damping, damping_ratio, LINEAR, periodTF, duration, dt, TOTAL_MASS, L, H1, arc_depth, E_cable, Cable_Dia_01, num_nodes, MAX_ITERATIONS, MAX_TOLERANCE)
initial_coords, deformed_coords, DISPLACEMENTS_damped, VELOCITY_damped, ACCELERATION_damped, DISP_X_damped, DISP_Y_damped, BASE_REACTION_X_damped, BASE_REACTION_Y_damped = results_damped
