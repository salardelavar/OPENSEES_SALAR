...
#%%-------------------------------------------------------------
"""
def WEB_FLANGE_PLATE_T_STEEL_SECTION_FUN_3D(secTag, matTag, bf, tf, h, tw,
                                   tsF, bsF,
                                   tsW, hsW, bsW,
                                   nFib, STEEL_DENSITY,
                                   plot=True):
    ...
    # -------------------------
    # Area of main T-beam components
    area_top_flange = bf * tf
    area_bottom_flange = 0.0
    area_web = tw * h # Height of web is total height minus thicknesses of top and bottom flanges
    
    # Area of stiffener plates
    area_left_web_stiffener = tsW * hsW
    area_right_web_stiffener = tsW * hsW
    area_top_flange_stiffener = bsF * tsF
    ...
    if plot:
        ...
        ax.set_title('T-Section with Stiffener Plates')
        ...
        geometry = [
            ...
            {"x": -tw/2, "y": -h/2, "w": tw, "h": h - tf, "color": main_color, "edgecolor": outline_color, "fill": True}, # Web
        ...
        handles = [
            patches.Patch(color=main_color, label='Main T-Section'),
    ...
    ELE_MASS  = STEEL_DENSITY * AREA   # Mass Per Length
