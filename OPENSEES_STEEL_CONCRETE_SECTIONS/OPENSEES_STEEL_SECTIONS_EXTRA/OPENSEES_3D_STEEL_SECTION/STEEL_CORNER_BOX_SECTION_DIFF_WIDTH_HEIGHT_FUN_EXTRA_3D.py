def STEEL_CORNER_BOX_SECTION_DIFF_WIDTH_HEIGHT_FUN_EXTRA_3D(secTag, STEEL_TYPE, STEEL_DENSITY, plot=True):
    """
    ...
    CD = 300.0   # [mm] Section Depth
    ACD = 100.0  # [mm] Corner Depth
    PT = 10.0    # [mm] Plate Thickness
    NFY, NFX = 10, 10 # Number of fibers in each Division in Y and X Dir.
    mat_layers = [
        (PT, ACD+PT, CD+0.5*PT, CD-0.5*ACD+0.5*PT, NFY, NFX, steelITag, 'lightgray'),               # 01 -> PLATE
        (PT, ACD+PT, 0.0-0.5*PT, 0.5*ACD-0.5*PT, NFY, NFX, steelITag, 'lightgray'),                 # 02 -> PLATE
        (ACD, PT, CD-0.5*ACD, CD+0.5*PT, NFY, NFX, steelITag, 'lightgray'),                         # 03 -> PLATE   
        (ACD, PT, 0.5*ACD, -0.5*PT, NFY, NFX, steelITag, 'lightgray'),                              # 04 -> PLATE
        
        (PT, ACD+PT, CD+0.5*PT, 50.0-0.5*PT, NFY, NFX, steelITag, 'lightgray'),                     # 05 -> PLATE
        (PT, ACD+PT, 0.0-0.5*PT, CD-0.5*ACD+0.5*PT, NFY, NFX, steelITag, 'lightgray'),              # 06 -> PLATE
        (ACD, PT, CD-0.5*ACD, 0.0-0.5*PT, NFY, NFX, steelITag, 'lightgray'),                        # 07 -> PLATE   
        (ACD, PT, 0.5*ACD, CD+0.5*PT, NFY, NFX, steelITag, 'lightgray'),                            # 08 -> PLATE
        
        (PT, CD-ACD, CD+1.5*PT, 0.5*CD, NFY, NFX, steelITag, 'lightgray'),                          # 09 -> PLATE
        (PT, CD-ACD, 0.0-1.5*PT, 0.5*CD, NFY, NFX, steelITag, 'lightgray'),                         # 10 -> PLATE
        (CD-ACD, PT, 0.5*CD, CD+1.5*PT, NFY, NFX, steelITag, 'lightgray'),                          # 11 -> PLATE   
        (CD-ACD, PT, 0.5*CD, -1.5*PT, NFY, NFX, steelITag, 'lightgray'),                            # 12 -> PLATE
    ...
    if plot:
        ...
        ax.set_title(f'Steel Corner Box section with plates (Numbers shown) - Section Tag: {secTag}')
