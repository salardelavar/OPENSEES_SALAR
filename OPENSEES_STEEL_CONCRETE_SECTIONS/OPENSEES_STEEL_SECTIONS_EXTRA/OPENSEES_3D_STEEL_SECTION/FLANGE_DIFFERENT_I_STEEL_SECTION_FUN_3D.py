...
# I-beam dimensions
bfT = 500     # TOP - Flange width
tfT = 20      # TOP - Flange thickness
bfB = 200     # BOTTOM - Flange width
tfB = 20      # BOTTOM - Flange thickness
h  = 400      # Total web height
tw = 12       # Web thickness

# Stiffener plates dimensions on flanges
tsF = 20      # Stiffener thickness
bsF = 180     # Stiffener length on flange
...
"""
def FLANGE_DIFFERENT_I_STEEL_SECTION_FUN_3D(secTag, matTag,
                                         bfT, tfT,
                                         bfB, tfB,
                                         h, tw,
                                   tsF, hsF, bsF,
    ...
    area_web = tw * (h - tfB - tfB) # Height of web is total height minus thicknesses of top and bottom flanges
    ...
    #%% I SECTION
    # Top flange
    add_rectangle_patch(h/2 - tfT, h/2, -bfT/2, bfT/2)
    
    # Bottom flange
    add_rectangle_patch(-h/2, -h/2 + tfB, -bfB/2, bfB/2)
    
    # Web
    add_rectangle_patch(-h/2 + tfB, h/2 - tfT, -tw/2, tw/2)
    ...
    if plot:
        ...
        geometry = [
            # Main I-beam parts
            {"x": -bfT/2, "y": h/2 - tfT, "w": bfT, "h": tfT, "color": main_color, "edgecolor": outline_color, "fill": True}, # Top flange
            {"x": -bfB/2, "y": -h/2, "w": bfB, "h": tfB, "color": main_color, "edgecolor": outline_color, "fill": True},      # Bottom flange
            {"x": -tw/2, "y": -h/2 + tfB, "w": tw, "h": h - tfB-tfT, "color": main_color, "edgecolor": outline_color, "fill": True}, # Web
        ...
        max_dim = max(max(bfB, bfT), h, hsF, bsF) + tsF + 5 # Add some padding
