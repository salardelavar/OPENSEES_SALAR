def COMPOSITE_FRP_CIRCULAR_CONFINED_CONCRETE_SECTION_FUN_EXTRA_3D(secTag, RI, RO, COVER,
                                                 fc, Kfc, THICKNESS,
                                                 STEEL_TYPE, STEEL_DENSITY, CONCRETE_DENSITY,
                                                 plot=True):
    """
    Create a Circular FRP Composite with confined concrete section with an external steel pipe.
    ...
    # Material tags
    coreTag   = secTag + 1000
    coverTag  = secTag + 2000
    steelTag  = secTag + 3000
    steelITag = secTag + 4000
    steelFTag = secTag + 5000
    ...
    # FRP pipe (outer tube)
    ops.patch('circ', steelFTag, nfCoverT, nfCoverR, 0.0, 0.0, RO, RO+THICKNESS, 0.0, 360.0)
