def RETROFITTED_COMPOSITE_SECTION_DIFF_WIDTH_HEIGHT_FUN_EXTRA_3D(secTag, STEEL_TYPE, fc, Kfc, CONCRETE_DENSITY, plot=True):
    """
    Create a retrofitted composite with different widths and heights, fiber section (OpenSees) and,
    ...
    step = CD / N   # [mm] Dimension of each fiber
    NFY, NFX = 10, 10 # Number of fibers in each Division in Y and X Dir.
    mat_layers = [
        (step, step, (i + 0.5) * step, (j + 0.5) * step, NFY, NFX, coreTag, 'lightgray')
        for i in range(N)   # N row from bottom (i=0) to top (i=N-1)
        for j in range(N)   # N columns from left (j=0) to right (j=N-1)
        ] + [
        (PT, 100.0+PT, 500.0+0.5*PT, 450.0+0.5*PT, NFY, NFX, steelITag, 'cyan'),               # 50 -> PLATE
        (PT, 100.0+PT, 0.0-0.5*PT, 0.5*100.0-0.5*PT, NFY, NFX, steelITag, 'cyan'),             # 51 -> PLATE
        (100.0, PT, 450.0, 500.0+0.5*PT, NFY, NFX, steelITag, 'cyan'),                         # 52 -> PLATE   
        (100.0, PT, 0.5*100.0, -0.5*PT, NFY, NFX, steelITag, 'cyan'),                          # 53 -> PLATE
        
        (PT, 100.0+PT, 500.0+0.5*PT, 50.0-0.5*PT, NFY, NFX, steelITag, 'cyan'),                # 54 -> PLATE
        (PT, 100.0+PT, 0.0-0.5*PT, 450.0+0.5*PT, NFY, NFX, steelITag, 'cyan'),                 # 55 -> PLATE
        (100.0, PT, 450.0, 0.0-0.5*PT, NFY, NFX, steelITag, 'cyan'),                           # 56 -> PLATE   
        (100.0, PT, 50.0, 500.0+0.5*PT, NFY, NFX, steelITag, 'cyan'),                          # 57 -> PLATE
        
        (PT, 400.0, 500.0+1.5*PT, 0.5*500.0, NFY, NFX, steelITag, 'cyan'),                     # 58 -> PLATE
        (PT, 400.0, 0.0-1.5*PT, 0.5*500.0, NFY, NFX, steelITag, 'cyan'),                       # 59 -> PLATE
        (400.0, PT, 0.5*500.0, 500.0+1.5*PT, NFY, NFX, steelITag, 'cyan'),                     # 60 -> PLATE   
        (400.0, PT, 0.5*500.0, -1.5*PT, NFY, NFX, steelITag, 'cyan'),                          # 61 -> PLATE

        (10.0, 150.0, 250.0-125.0, 250.0, NFY, NFX, steelITag, 'lightgreen'),                  # 62 -> CROSSED SECTION
        (240.0, 10.0, 250.0, 250.0, NFY, NFX, steelITag, 'lightgreen'),                        # 63 -> CROSSED SECTION
        (10.0, 150.0, 250+125.0, 250.0, NFY, NFX, steelITag, 'lightgreen'),                    # 64 -> CROSSED SECTION   
         
        (10.0, 105.0, 250.0, 250.0-57.5, NFY, NFX, steelITag, 'lightgreen'),                   # 65 -> CROSSED SECTION
        (10.0, 105.0, 250.0, 250.0+57.5, NFY, NFX, steelITag, 'lightgreen'),                   # 66 -> CROSSED SECTION
        
        (150.0, 10.0, 250.0, 250.0-115.0, NFY, NFX, steelITag, 'lightgreen'),                  # 67 -> CROSSED SECTION
        (150.0, 10.0, 250.0, 250.0+115.0, NFY, NFX, steelITag, 'lightgreen'),                  # 68 -> CROSSED SECTION
    ]
    ...
    mat_layers = [
        ...
        (PT, 100.0+PT, 500.0+0.5*PT, 450.0+0.5*PT, NFY, NFX, steelITag, 'cyan'),               # 26 -> PLATE
        (PT, 100.0+PT, 0.0-0.5*PT, 0.5*100.0-0.5*PT, NFY, NFX, steelITag, 'cyan'),             # 27 -> PLATE
        (100.0, PT, 450.0, 500.0+0.5*PT, NFY, NFX, steelITag, 'cyan'),                         # 28 -> PLATE   
        (100.0, PT, 0.5*100.0, -0.5*PT, NFY, NFX, steelITag, 'cyan'),                          # 29 -> PLATE
        
        (PT, 100.0+PT, 500.0+0.5*PT, 50.0-0.5*PT, NFY, NFX, steelITag, 'cyan'),                # 30 -> PLATE
        (PT, 100.0+PT, 0.0-0.5*PT, 450.0+0.5*PT, NFY, NFX, steelITag, 'cyan'),                 # 31 -> PLATE
        (100.0, PT, 450.0, 0.0-0.5*PT, NFY, NFX, steelITag, 'cyan'),                           # 32 -> PLATE   
        (100.0, PT, 50.0, 500.0+0.5*PT, NFY, NFX, steelITag, 'cyan'),                          # 33 -> PLATE
        
        (PT, 400.0, 500.0+1.5*PT, 0.5*500.0, NFY, NFX, steelITag, 'cyan'),                     # 34 -> PLATE
        (PT, 400.0, 0.0-1.5*PT, 0.5*500.0, NFY, NFX, steelITag, 'cyan'),                       # 35 -> PLATE
        (400.0, PT, 0.5*500.0, 500.0+1.5*PT, NFY, NFX, steelITag, 'cyan'),                     # 36 -> PLATE   
        (400.0, PT, 0.5*500.0, -1.5*PT, NFY, NFX, steelITag, 'cyan'),                          # 37 -> PLATE

        (10.0, 150.0, 250.0-125.0, 250.0, NFY, NFX, steelITag, 'lightgreen'),                  # 38 -> CROSSED SECTION
        (240.0, 10.0, 250.0, 250.0, NFY, NFX, steelITag, 'lightgreen'),                        # 39 -> CROSSED SECTION
        (10.0, 150.0, 250+125.0, 250.0, NFY, NFX, steelITag, 'lightgreen'),                    # 40 -> CROSSED SECTION   
         
        (10.0, 105.0, 250.0, 250.0-57.5, NFY, NFX, steelITag, 'lightgreen'),                   # 41 -> CROSSED SECTION
        (10.0, 105.0, 250.0, 250.0+57.5, NFY, NFX, steelITag, 'lightgreen'),                   # 42 -> CROSSED SECTION
        
        (150.0, 10.0, 250.0, 250.0-115.0, NFY, NFX, steelITag, 'lightgreen'),                  # 43 -> CROSSED SECTION
        (150.0, 10.0, 250.0, 250.0+115.0, NFY, NFX, steelITag, 'lightgreen'),                  # 44 -> CROSSED SECTION    
         ]
    ...
    if plot:
        ...
        ax.set_title(f'Retrofitted Composite section with rebars and plates (Numbers shown) - Section Tag: {secTag}')
