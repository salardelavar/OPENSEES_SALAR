...
section_tag = 1
"""
def CONCRETE_T_SECTION_FUN(secTag, matTag, STEEL_TYPE, fc, Kfc,
                           bf, tf, h, tw,
                           nFib, CONCRETE_DENSITY,
                           plot=True):
    """
    Create a T confined‑concrete fiber section (OpenSees) and,
    ...
    add_rect(coverTag, -h/2,      h/2 - tf, -tw/2, tw/2)   # web
    ...
    rebars = [
        ...
        (16.0,  -h/16,      -tw/4),   # 7
        (16.0,  -h/16,       tw/4),   # 8
        (20.0, -h/2 + tf/2, -bf/20),  # 9
        (20.0, -h/2 + tf/2,  bf/20)   # 10
    ]
    ...
    if plot:
        ...
        ax.set_title('T Section with Rebars (numbers shown)')
        ax.grid(True, ls='--', alpha=0.5)

        # Concrete geometry (light gray)
        geometry = [
            {"x": -bf/2, "y": h/2 - tf, "w": bf, "h": tf},         # top flange
            {"x": -tw/2, "y": -h/2,    "w": tw, "h": h - tf}       # web
        ]
        for g in geometry:
