...
1. Problem Formulation – We invert the conventional design process:
   instead of checking a given yield strength DUCT, we find the 'minimum'
   DUCT that makes the structure's hysteretic energy dissipation under
...
3. Nonlinear Forward Model – Each evaluation of DUCT requires solving
   the equation of motion: M·ü + C·u̇ + R(u, DUCT) = f_ext(t) with a
   ...
   DUCT through the yield surface.

4. Root‑Finding via Newton–Raphson – The residual g(DUCT) = EDCI(DUCT)-0.20
   is driven to zero using a secant‑like Newton iteration, where the
   Jacobian g'(DUCT) is approximated by central finite differences
   [g(DUCT+ε) - g(DUCT-ε)]/(2ε), with ε tuned to balance truncation and
   conditioning errors.
...
8. Physical Insight – The converged DUCT corresponds to the strength
...
9. Sensitivity and Stability – The finite‑difference step ε is set to
   1e-3·DUCT to avoid numerical ill‑conditioning, and the solver terminates
   when the relative change in DUCT falls below 1e-6, ensuring that the
...
#%%----------------------------------------------------
def MDOF_TWO(DUCT, MAT_TYPE, TOTAL_MASS, ANAL_TYPE):
    # Initialize model
    ops.wipe()
    ops.model('basic', '-ndm', 1, '-ndf', 1)
    
    MAX_ITERATIONS = 5000   # Maximum number of iterations
    MAX_TOLERANCE = 1.0e-2  # Specified tolerance for convergence
    ...
    for II in range(0, 4):
        ...
        MAT_TAG01 = 1000 + II # SPRING TAG FOR FRAME
        MAT_TAG02 = 2000 + II # SPRING TAG FOR BRACE OR SHEAR-WALL
        ...
        if MAT_TYPE == 'INELASTIC':
            # FOR FRAME
            S08.OPENSEEES_HYSTERETICSM_FORCE_DISP_FUN(MAT_TAG01, DP, FP, DN, FN, PLOT = False, X_LABEL='Displacement (m)', Y_LABEL='Force [N]', TITLE='FORCE-DISPLACEMENT CURVE FOR FRAME')
            # FOR BRACE OR SHEAR-WALL
            COE_F = 4.0   # THIS COEFFICIENT IS USED FOR SHEAR-WALL RESISTANT FORCE
            COE_D = 5.0   # THIS COEFFICIENT IS USED FOR SHEAR-WALL RESISTANT DISPLACEMENT
            DP[0], FP[0] = DY*COE_D, FY*COE_F
            DP[1], FP[1] = DSU*COE_D, FU*COE_F 
            DP[2], FP[2] = 1.1*DSU*COE_D, 0.20*FU*COE_F
            DP[3], FP[3] = 1.25*DSU*COE_D, 0.10*FU*COE_F
            DN[0], FN[0] = -DY*COE_D, -FY*COE_F 
            DN[1], FN[1] = -DSU*COE_D, -FU*COE_F
            DN[2], FN[2] = -1.1*DSU*COE_D, -0.20*FU*COE_F   
            DN[3], FN[3] = -1.25*DSU*COE_D, -0.10*FU*COE_F
            S08.OPENSEEES_HYSTERETICSM_FORCE_DISP_FUN(MAT_TAG02, DP, FP, DN, FN, PLOT = False, X_LABEL='Displacement (m)', Y_LABEL='Force [N]', TITLE='FORCE-DISPLACEMENT CURVE FOR SHEAR-WALL')
        if MAT_TYPE == 'ELASTIC':
            #ops.uniaxialMaterial('Elastic', MAT_TAG, Ke)             # TESNSION AND COMPRESSION IS SAME VALUES
            COE_F = 4.0   # THIS COEFFICIENT IS USED FOR SHEAR-WALL RESISTANT FORCE
            ops.uniaxialMaterial('Elastic', MAT_TAG01, Ke*COE_F ,0.0, 0.5*Ke*COE_F) # TESNSION AND COMPRESSION IS NOT SAME VALUES
            ops.uniaxialMaterial('Elastic', MAT_TAG02, Ke*COE_F ,0.0, 0.5*Ke*COE_F) # TESNSION AND COMPRESSION IS NOT SAME VALUES
            # INFO LINK: https://openseespydoc.readthedocs.io/en/latest/src/ElasticUni.html
        
        MatTag_C01 = 3000 + II # SPRING DAMPER FOR FRAME
        alpha = 1.0    # velocity exponent (usually 0.3–1.0)
        omega = np.sqrt(Ke / 0.5*MASS[II]) # HALF MASS FOR FRAME -> 0.5*MASS[II]
        Cd = 2 * DRi[II] * omega * 0.5*MASS[II]  # [N·s/m] Damping coefficient 
        ops.uniaxialMaterial('Viscous', MatTag_C01, Cd, alpha)  # Material for C (alpha=1.0 for linear)
        
        MatTag_C02 = 4000 + II # SPRING DAMPER FOR BRACE OR SHEAR-WALL
        alpha = 1.0    # velocity exponent (usually 0.3–1.0)
        omega = np.sqrt(Ke / 0.5*MASS[II]) # HALF MASS FOR SHEAR-WALL -> 0.5*MASS[II]
        # FOR SHEAR-WALL DAMPING RATIO IS 3 TIMES BIGGER -> 3*DRi[II]
        Cd = 2 * 3*DRi[II] * omega * 0.5*MASS[II]  # [N·s/m] Damping coefficient 
        ops.uniaxialMaterial('Viscous', MatTag_C02, Cd, alpha)  # Material for C (alpha=1.0 for linear)        
        
        # Define element
        ops.element('zeroLength', II+1, II+1, II+2,
                    '-mat', MAT_TAG01, MAT_TAG02, MatTag_C01, MatTag_C02,
                    '-dir', 1, 1, 1, 1)  # DOF LATERAL SPRING
    ...
    if ANAL_TYPE == 'DYNAMIC_EXTERNAL_TIME-DEPENDENT_LOADING': # DYNAMIC TIME-HISTORY ANALYSIS
        ...
        #DT_load02 = EXTERNAL_TIME_DEPENDENT(force_amplitude, omega_DT, DT, DT_time)
    ...
    if ANAL_TYPE == 'FREE-VIBRATION': # DYNAMIC TIME-HISTORY ANALYSIS
        ...
        if IU == True:
            ...
            ops.setNodeDisp(3, 1, u0, '-commit')
            ops.setNodeDisp(4, 1, u0, '-commit')
        ...
        if IA == True:
            ...
            ops.setNodeAccel(3, 1, a0, '-commit')
            ops.setNodeAccel(4, 1, a0, '-commit')
        ...
        #print(damping_ratio2, damping_ratio_03, damping_ratio_04, damping_ratio_05)
    ...
    if ANAL_TYPE == 'SEISMIC': # DYNAMIC TIME-HISTORY ANALYSIS
        ...
        SSF_X = 5.10                       # Seismic Acceleration Scale Factor in X Direction
        SSF_Y = 5.10                       # Seismic Acceleration Scale Factor in Y Direction
...
# ----------------------------------------------------------------------------------
# FIND THE OPTIMUM VALUE (NEWTON-RAPHSON SOLVER FOR OPTIMAL DUCTILITY RATIO)
# ----------------------------------------------------------------------------------
import time as TI
import numpy as np
 
X = 4.0           # Intial Guess for X -> Ductility Ratio of Structure
...
DEMAND = 20.0     # [%] Target Value -> ENERGY DISSIPATION CAPACITY INDEX 
...
while (RESIDUAL > TOLERANCE):
    ...
    DATA = MDOF_TWO(X, MAT_TYPE, TOTAL_MASS, ANAL_TYPE)
    
    (reaction_CP, disp_CP, DI_CP, _, _,
     PERIOD_MIN_CP, PERIOD_MAX_CP) = DATA
    
    ANAL_TYPE = 'SEISMIC'
    DATA = MDOF_TWO(X, MAT_TYPE, TOTAL_MASS, ANAL_TYPE)
    (time_SEI, reaction_SEI, disp_SEI, velo_SEI, acc_SEI, DI_SEI, _, _,
    ...
    SUPPLY = S09.ENERGY_DISSIPATION_CAPACITY_INDEX(disp_SEI, reaction_SEI, disp_CP, reaction_CP)
    F = SUPPLY - DEMAND
    print('F:    ', F)
    # XMIN -------------------
    ...
    print('Xmin:    ', Xmin)
    ANAL_TYPE = 'CYCLIC_DISPLACEMENT'
    DATA = MDOF_TWO(Xmin, MAT_TYPE, TOTAL_MASS, ANAL_TYPE)
    (reaction_CP, disp_CP, DI_CP, _, _,
     PERIOD_MIN_CP, PERIOD_MAX_CP) = DATA
    ...
    SUPPLYmin = S09.ENERGY_DISSIPATION_CAPACITY_INDEX(disp_SEI, reaction_SEI, disp_CP, reaction_CP)
    ...
    print('Xmax:    ', Xmax)
    ANAL_TYPE = 'CYCLIC_DISPLACEMENT'
    DATA = MDOF_TWO(Xmax, MAT_TYPE, TOTAL_MASS, ANAL_TYPE)
    (reaction_CP, disp_CP, DI_CP, _, _,
     PERIOD_MIN_CP, PERIOD_MAX_CP) = DATA
    ...
    SUPPLYmax = S09.ENERGY_DISSIPATION_CAPACITY_INDEX(disp_SEI, reaction_SEI, disp_CP, reaction_CP)
    ...
    if RESIDUAL < TOLERANCE:
        print(f'\t\t Optimum X (DUCTILITY RATIO):        {X:.4f}')
...
DATA = MDOF_TWO(DUCT, MAT_TYPE, TOTAL_MASS, ANAL_TYPE)
...
# PLOT NODAL DISPALEMENTS
PLOT_DISPLAEMENTS(time_ETDLD, node_displacements_ETDLD, TITLE = "Node Displacements vs Time for During Free-vibration Analysis")
...
# Save to Excel
with pd.ExcelWriter("MDOF_TWO_DUCT_OPTIMIZATION_8_ANA_OUTPUT.xlsx", engine='openpyxl') as writer:
