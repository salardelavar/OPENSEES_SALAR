...
# DETERMINATION OF OPTIMUM STRUCTURAL ULTIMATE DISPLACEMENT FROM ENERGY DISSIPATION CAPACITY INDEX USING  #
...
1. Problem Formulation – We invert the conventional design process:
   instead of checking a given yield strength DSU, we find the 'minimum'
...
#%%----------------------------------------------------
def MDOF_TWO(DSU, MAT_TYPE, TOTAL_MASS, ANAL_TYPE):
    # Initialize model
    ops.wipe()
...
# ----------------------------------------------------------------------------------
# FIND THE OPTIMUM VALUE (NEWTON-RAPHSON SOLVER FOR OPTIMAL ULTIMATE DISPLACEMENT)
# ----------------------------------------------------------------------------------
import time as TI
import numpy as np
 
X = 0.1           # Intial Guess for X -> Ultimate Displacement of Structure
...
while (RESIDUAL > TOLERANCE):
    ...
    if RESIDUAL < TOLERANCE:
        print(f'\t\t Optimum X (ULTIMATE DISPLACEMENT):  {X:.4f}')
...
DATA = MDOF_TWO(DSU, MAT_TYPE, TOTAL_MASS, ANAL_TYPE)
...
# Save to Excel
with pd.ExcelWriter("MDOF_TWO_DSU_OPTIMIZATION_8_ANA_OUTPUT.xlsx", engine='openpyxl') as writer:
