...
#             OF ENERGY DISSIPATION CAPACITY INDEX WITH INCREMENTAL DYNAMIC ANALYSIS (IDA)                #  
...
#%%------------------------------------------------------------------------------------------------
### OPENSEES FUNCTION
def SDOF(MAT_TYPE, TOTAL_MASS, ANAL_TYPE, i, g):
    # Initialize model
    ops.wipe()
    ...
    if ANAL_TYPE == 'SEISMIC': # DYNAMIC TIME-HISTORY ANALYSIS
        ...
        duration = 20.0                    # [s] Analysis duration
        dt = 0.01                          # [s] Time step
        GMfact = g                         # [m/s²]standard acceleration of gravity or standard acceleration
...
data = SDOF(MAT_TYPE, TOTAL_MASS, ANAL_TYPE, 0, 0)
...
for j in range(NUM_SEISMIC):
    ...
    for i in range(NUM_G):
        ...
        data = SDOF(MAT_TYPE, TOTAL_MASS, ANAL_TYPE, j, gi)
...
damage_states = {
'DS1_Slight': (0.15, 0.20),    # Median PGA=0.15g, β=0.20
'DS2_Moderate': (0.30, 0.20),
'DS3_Extensive': (0.60, 0.20),
'DS4_Complete': (1.00, 0.20)
}
"""
...
S077.FRAGILITY_CURVE(im_values, damage_states, X_LABEL, SEMILOGY=False, PLOT_KIND=False)
#===========================================================
