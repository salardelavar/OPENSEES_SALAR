...
MAT_TYPE = 'ELASTIC'
TOTAL_MASS = 5000.0
X1 = 30000            # initial damper stiffness multiplier
X2 = 0.001            # initial damping ratio multiplier
nK = 5
nD = 200
...
WW_matrix = np.zeros((nK, nD))   # damping ratio
ZZ_matrix = np.zeros((nK, nD))   # max period
ITER = 0
...
for i, II in enumerate(range(nK)):
    ...
    for j, JJ in enumerate(range(nD)):
        DRx = X2 * (JJ + 1)
        # -- Call your SDOF function --
        (_, _, _, _, _, _, _, _, damping_ratio_FV,
         _, PERIOD_MAX_FV) = SDOF(Kd, DRx, MAT_TYPE, TOTAL_MASS, 'FREE-VIBRATION')
        WW_matrix[i, j] = damping_ratio_FV
        ZZ_matrix[i, j] = max(PERIOD_MAX_FV)
        ITER += 1
        print('STEP: ', ITER)

totaltime = TI.process_time() - starttime
print(f'Finished in {totaltime:.4f} s\n')

#%% 1. Flatten data into a DataFrame for regression
Kd_vals = np.array([X1 * ii for ii in range(nK)])
DRx_vals = np.array([X2 * jj for jj in range(nD)])
Kd_grid, DRx_grid = np.meshgrid(Kd_vals, DRx_vals, indexing='ij')  # shape (nK, nD)

df = pd.DataFrame({
    'Kd':       Kd_grid.ravel(),
    'DRx':      DRx_grid.ravel(),
    'Damping':  WW_matrix.ravel(),
    'Period':   ZZ_matrix.ravel()
})

#%% 2. 2nd‑order polynomial regression for DAMPING RATIO
# Create polynomial terms
df['Kd_sq']   = df['Kd'] ** 2
df['DRx_sq']  = df['DRx'] ** 2
df['Kd_x_DRx'] = df['Kd'] * df['DRx']

# Predictors and response
X_damp = df[['Kd', 'DRx', 'Kd_sq', 'DRx_sq', 'Kd_x_DRx']]
X_damp = sm.add_constant(X_damp)
y_damp = df['Damping']

# Fit OLS model
model_damp = sm.OLS(y_damp, X_damp).fit()
print("=== DAMPING RATIO MODEL ===")
print(model_damp.summary())

# Prediction function for damping ratio
coeff_damp = model_damp.params
def predict_damping(Kd, DRx):
    return (coeff_damp['const'] +
            coeff_damp['Kd'] * Kd +
            coeff_damp['DRx'] * DRx +
            coeff_damp['Kd_sq'] * (Kd ** 2) +
            coeff_damp['DRx_sq'] * (DRx ** 2) +
            coeff_damp['Kd_x_DRx'] * (Kd * DRx))

#%% 3. 2nd‑order polynomial regression for PERIOD
y_period = df['Period']

model_period = sm.OLS(y_period, X_damp).fit()   # same predictors
print("\n=== PERIOD MODEL ===")
print(model_period.summary())

coeff_period = model_period.params
def predict_period(Kd, DRx):
    return (coeff_period['const'] +
            coeff_period['Kd'] * Kd +
            coeff_period['DRx'] * DRx +
            coeff_period['Kd_sq'] * (Kd ** 2) +
            coeff_period['DRx_sq'] * (DRx ** 2) +
            coeff_period['Kd_x_DRx'] * (Kd * DRx))

#%% 4. 3D plots: actual points + fitted polynomial surface
# Create a finer grid for smooth surface visualisation
Kd_plot = np.linspace(Kd_vals.min(), Kd_vals.max(), 50)
DRx_plot = np.linspace(DRx_vals.min(), DRx_vals.max(), 50)
Kd_mesh, DRx_mesh = np.meshgrid(Kd_plot, DRx_plot, indexing='ij')

# Predict on the grid for both responses
damp_surface = predict_damping(Kd_mesh, DRx_mesh)
period_surface = predict_period(Kd_mesh, DRx_mesh)

# --- 3D Plot: Damping Ratio ---
fig1 = plt.figure(figsize=(10, 6))
ax1 = fig1.add_subplot(111, projection='3d')
# Actual data scatter
ax1.scatter(df['Kd'], df['DRx'], df['Damping'],
            c='red', marker='o', s=15, label='Actual data')
# Fitted surface
ax1.plot_surface(Kd_mesh, DRx_mesh, damp_surface,
                 alpha=0.6, cmap='viridis', edgecolor='none')
ax1.set_xlabel('Damper Stiffness Kd [N/m]')
ax1.set_ylabel('Damper Damping Ratio DRx')
ax1.set_zlabel('Structural Damping Ratio')
ax1.set_title('Damping Ratio – Polynomial Fit')
ax1.legend()
plt.tight_layout()
plt.show()
...
ax2.scatter(df['Kd'], df['DRx'], df['Period'],
            c='blue', marker='o', s=15, label='Actual data')
ax2.plot_surface(Kd_mesh, DRx_mesh, period_surface,
                 alpha=0.6, cmap='plasma', edgecolor='none')
ax2.set_xlabel('Damper Stiffness Kd [N/m]')
ax2.set_ylabel('Damper Damping Ratio DRx')
ax2.set_zlabel('Structural Period [s]')
ax2.set_title('Period – Polynomial Fit')
ax2.legend()
plt.tight_layout()
plt.show()

#%% 5. Quick example predictions
example_Kd = 60000   # 2 * X1
example_DRx = 0.05
print(f'\nPredicted damping for Kd={example_Kd}, DRx={example_DRx}: '
      f'{predict_damping(example_Kd, example_DRx):.6f}')
print(f'Predicted period  for Kd={example_Kd}, DRx={example_DRx}: '
      f'{predict_period(example_Kd, example_DRx):.6f}')
