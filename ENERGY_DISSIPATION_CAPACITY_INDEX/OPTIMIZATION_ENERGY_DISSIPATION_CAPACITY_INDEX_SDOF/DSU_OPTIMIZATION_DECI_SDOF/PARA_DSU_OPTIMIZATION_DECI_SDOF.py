...
#%%----------------------------------------------------
def SDOF(DSU, MAT_TYPE, TOTAL_MASS, ANAL_TYPE):
    # Initialize model
    ops.wipe()
...
#%%----------------------------------------------------
# ---------------------------------------------------------------------------------------------------------
# FIND THE OPTIMUM VALUE (NEWTON-RAPHSON SOLVER FOR OPTIMAL ULTIMATE DISPLACEMENT) WITH PARALLEL COMPUTING
# ---------------------------------------------------------------------------------------------------------
import time as TI
...
def RUN_ANALYSIS_FUN(X_val):
    ...
    # Run both cyclic and seismic analyses for a given yield strength X_val
    # and return the dissipated energy capacity index (supply).
    
    # Cyclic displacement analysis
    ANAL_TYPE = 'CYCLIC_DISPLACEMENT'
    DATA_CP = SDOF(X_val, MAT_TYPE, TOTAL_MASS, ANAL_TYPE)
    reaction_CP, disp_CP, DI_CP, PERIOD_MIN_CP, PERIOD_MAX_CP = DATA_CP

    # Seismic analysis
    ANAL_TYPE = 'SEISMIC'
    DATA_SEI = SDOF(X_val, MAT_TYPE, TOTAL_MASS, ANAL_TYPE)
    (time_SEI, reaction_SEI, disp_SEI, velo_SEI, acc_SEI, DI_SEI,
     stiffness_SEI, PERIOD_SEI, damping_ratio_SEI,
     PERIOD_MIN_SEI, PERIOD_MAX_SEI) = DATA_SEI

    # Compute supply (energy dissipation capacity index)
    supply = S09.ENERGY_DISSIPATION_CAPACITY_INDEX(disp_SEI, reaction_SEI,
                                                    disp_CP, reaction_CP)
    return supply
    
def OPTIMIZE_DSU():
    X = 0.2           # Intial Guess for X -> Structural Ultimate Displacement
    ...
    with concurrent.futures.ProcessPoolExecutor() as executor:
        while RESIDUAL > TOLERANCE and IT < ITMAX:
            ...
            print(f'IT: {IT} - RESIDUAL: {RESIDUAL:.6e} - DUCTLITY RATIO: {X:.6e}')
