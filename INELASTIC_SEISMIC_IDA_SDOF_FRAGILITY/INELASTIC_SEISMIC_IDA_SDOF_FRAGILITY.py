...
J_MAX = 200    # Incremental Dynamic Analysis number for simulation
AREA = 0.003   # [m^2] Spring Section Area
...
damage_states = {
    'Slight': (0.2, 0.4),# Median DI=0.2, β=0.4
    'Moderate': (0.4, 0.4),
    'Extensive': (0.6, 0.5),
    'Complete': (1.0, 0.5)
}
...
#------------------------------------------------------------------------------------------------
# MACHINE LEARNING-BASED FRAGILITY CURVES

from sklearn.linear_model import LogisticRegression

X = max_DI # Structural Ductility Damage Index
Y = max_acceleration
...
plt.plot(im_values, probabilities, lw=2, label='Logistic Regression')
plt.xlabel('Structural Ductility Damage Index [IM]', fontsize=12)
plt.ylabel('Probability of Exceedance', fontsize=12)
plt.title('Machine Learning-Based Fragility Curve', fontsize=14)
plt.legend()
plt.ylim(0, 1.0)
plt.grid(True)
plt.show()
...
# Empirical data (intensity measures and failure probabilities) 
im_values = max_DI # Structural Ductility Damage Index
failure_prob = max_acceleration

# Lognormal CDF function for curve fitting
def lognormal_cdf(x, median, beta):
    return norm.cdf((np.log(x) - np.log(median)) / beta)

# Fit the curve to the data
params, _ = curve_fit(lognormal_cdf, im_values, failure_prob, p0=[0.5, 0.3])
median_fit, beta_fit = params

# Plot the empirical data and the fitted curve
plt.figure(figsize=(8, 6))
plt.scatter(im_values, failure_prob, color='red', label='Empirical Data')
im_fit = np.linspace(0.01, 1.5, 200)
plt.plot(im_fit, lognormal_cdf(im_fit, median_fit, beta_fit), lw=2, 
         label=f'Fitted Curve: η={median_fit:.2f}, β={beta_fit:.2f}')
plt.xlabel('Structural Ductility Damage Index [IM]', fontsize=12)
plt.ylabel('Probability of Exceedance', fontsize=12)
plt.title('Empirical Fragility Curve', fontsize=14)
plt.legend()
plt.grid(True)
plt.show()
