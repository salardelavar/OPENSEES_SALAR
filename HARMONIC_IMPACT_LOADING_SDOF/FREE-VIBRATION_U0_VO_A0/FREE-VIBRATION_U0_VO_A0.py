...
FY2n, DY2n = -3104.6, -0.02    # Peak compressive force
FY3n, DY3n = -1663.2, -0.04    # Post-peak softening
...
def FREE_VIBRATION_SDOF(IA, IU, IV, SPRING_KIND, M, C, K, KP, KN, u0, v0, a0, duration, dt):
    ...
    peaksT = np.array([TIME[i] for i in range(1, len(TIME)-1) if displacement[i] > displacement[i-1] and displacement[i] > displacement[i+1]])
    # Natural logarithm
    delta = np.log(peaks[:-1] / peaks[1:])   # Damping Ratio
    deltaT = np.log(peaksT[:-1] / peaksT[1:])  # Period
    ...
    x0 = 1  # Intial Guess for Period
    solutionT = fsolve(EQUATION, x0, args=(deltaT))
    print(f"Exact Period:        {solutionT[0]:.8f}     [s]")    
    return time, reaction, disp, vel, accel, stiffness, PERIOD, solutionD[0], solutionT[0]
