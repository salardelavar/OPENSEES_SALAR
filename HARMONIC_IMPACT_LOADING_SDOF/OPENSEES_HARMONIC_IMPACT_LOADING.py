...
# Calculate deformation response factor and phase angle
response_factors_phase_angle = []
...
for zeta in damping_ratios:
    ...
    for r in frequency_ratios:
        ...
        # Response factor formulas
        denominator = np.sqrt((1 - r**2)**2 + (2 * zeta * r)**2)
        phase_angle = np.arctan2(2 * zeta * r, 1 - r**2)

        # Append phase angle (in degrees) for plotting
        phase_angles.append(np.degrees(phase_angle))

    response_factors_phase_angle.append(phase_angles)

# Plot deformation response factor and phase angle
plt.figure(figsize=(15, 10))
...
plt.subplot(2, 1, 2)
for i, zeta in enumerate(damping_ratios):
    plt.plot(frequency_ratios, response_factors_phase_angle[i], label=f'Damping Ratio {zeta:.2f}')
plt.title('Phase Angle of Response')
plt.xlabel('Frequency Ratio (ω/ωn)')
