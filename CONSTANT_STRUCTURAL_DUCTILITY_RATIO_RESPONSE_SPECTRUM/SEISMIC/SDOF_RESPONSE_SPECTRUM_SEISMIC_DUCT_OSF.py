...
#     NONLINEAR DYNAMIC ANALYSIS UNDER MULTI GROUND MOTION RECORDS COMPUTATION AND VISUALIZATION          #
...
NUM_SIM = 50                                  # Total number for analysis
...
#%%------------------------------------------------------------------------------------------------
### OPENSEES FUNCTION
def ANALYSIS_SDOF(j, m, duct, osf):
    ...
    gm_accels = np.loadtxt(f'Ground_Acceleration_{j+1}.txt')  # Assumes acceleration in m/s²
...
DATA = {
    ...
    8: [],  # EQUIVALENT VISCOUS DAMPING RATIO
    }
...
for j in range(NUM_SIM):
    ...
    for i in range(NUM_PERIOD):
        ...
        data = ANALYSIS_SDOF(j, m, duct[j], osf[j])
...
labels = {
    ...
    6: "DAMPING_RATIO",
    7: "STIFFNESS",
    8: "EQUIVALENT_VISCOUS_DAMPING_RATIO",
}
...
Y = DATA
XLABEL = "Period T [s]"
YLABEL = "Equivalent Viscous Damping Ratio Spectrum [%]"
TITLE =  "Equivalent Viscous Damping Ratio Spectrum"
SEMILOGY = False

mean_evdr, median_evdr = PLOT_2D(8, XLABEL, YLABEL, TITLE, SEMILOGY)
...
XLABEL = 'Structural Ductility Ratio [m/m]'   
YLABEL = 'Structural Over-Strength Factor [N/N]' 
ZLABEL = "Equivalent Viscous Damping Ratio Spectrum [%]"
...
data = {
    ...
    'EQUIVALENT_VISCOUS_DAMPING_RATIO': median_evdr,
}
