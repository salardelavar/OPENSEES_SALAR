...
#%%------------------------------------------------------------------------------------------------
### OPENSEES FUNCTION
def ANALYSIS_MDOF(MAT_TYPE, TOTAL_MASS, duct, osf, u0):
    # Initialize model
    ops.wipe()
    ...
    while stable == 0 and current_time < duration:
        ...
        di = S066.DAMAGE_INDEX_FUN(disp_05[-1], DY, DSU)
...
DATA = {
    ...
    14: [],  # EQUIVALENT VISCOUS DAMPING RATIO
    }
...
U0 = np.linspace(0.01, 0.10, NUM_SIM).tolist()  # DEFINE MIN. AND MAX. VALUE FOR INTIAL DISPLACEMENT
...
for j in range(NUM_SIM):
    ...
    max_duct = []
    max_osf = []
    max_zeta02, max_zeta03, max_zeta04, max_zeta05 = [], [], [], []
    ...
    for i in range(NUM_PERIOD):
        ...
        data = ANALYSIS_MDOF(MAT_TYPE, m, duct[j], osf[j], U0[j])
        (time, 
        disp_02,
        disp_03,
        disp_04,
        ...
        zeta02 = S055.EQUIVALENT_VISCOUS_DAMPING_RATIO_FUN(disp_02, base_reaction, method, XLABEL, YLABEL, TITLE, PLOT)
        print(f"Equivalent viscous damping ratio = {zeta02:.4f} [%]")
        max_zeta02.append(zeta02)
        zeta03 = S055.EQUIVALENT_VISCOUS_DAMPING_RATIO_FUN(disp_03, base_reaction, method, XLABEL, YLABEL, TITLE, PLOT)
        print(f"Equivalent viscous damping ratio = {zeta03:.4f} [%]")
        max_zeta03.append(zeta03)
        zeta04 = S055.EQUIVALENT_VISCOUS_DAMPING_RATIO_FUN(disp_04, base_reaction, method, XLABEL, YLABEL, TITLE, PLOT)
        print(f"Equivalent viscous damping ratio = {zeta04:.4f} [%]")
        max_zeta04.append(zeta04)
        zeta05 = S055.EQUIVALENT_VISCOUS_DAMPING_RATIO_FUN(disp_05, base_reaction, method, XLABEL, YLABEL, TITLE, PLOT)
        print(f"Equivalent viscous damping ratio = {zeta05:.4f} [%]")
        max_zeta05.append(zeta05)
...
labels = {
    ...
    14: "EQUIVALENT_VISCOUS_DAMPING_RATIO",
}
...
Y = DATA
XLABEL = "Period T [s]"
YLABEL = "Structural Stiffness [N/m]"
...
Y = DATA
XLABEL = "Period T [s]"
YLABEL = "Effective Damping Ratio [%]"
...
# -------------------------------------------------------
# Plot 12: Equivalent Viscous Damping Ratio Spectrum [%]
# -------------------------------------------------------
X = T
Y = DATA
XLABEL = "Period T [s]"
...
mean_eqDR, median_eqDR = PLOT_2D(14, XLABEL, YLABEL, TITLE, SEMILOGY)
...
plt.show()
# ------------------------------------------------------------------------
#   Plot Structural Ductility Ratio and Equivalent Viscous Damping Ratio
# ------------------------------------------------------------------------
plt.figure(3, figsize=(12, 8))
plt.plot(duct, median_eqDR, color='black', linewidth=2)
plt.xlabel('Structural Ductility Ratio [m/m]')
plt.ylabel('Damping Ratio [%]')
plt.title('Equivalent Viscous Damping Ratio vs Structural Ductility Ratio')
plt.legend(['Median Equivalent Viscous Damping Ratio [%]'])
plt.grid()
plt.show()
#%%------------------------------------------------------------------------------
...
XLABEL = 'Structural Ductility Ratio [m/m]'   
YLABEL = 'Structural Over-Strength Factor [N/N]' 
ZLABEL = "Equivalent Viscous Damping Ratio [%]"
PLOT_3D_CONTOUR_XYZ(9, X, Y, Z, XLABEL, YLABEL, ZLABEL)
#%%------------------------------------------------------------------------------------------------
####  FRAGILITY ANALYSIS
...
DATA_TOTAL = {
    ...
    'EQ_Damping_Ratio_02': max_zeta02,
    'EQ_Damping_Ratio_03': max_zeta03,
    'EQ_Damping_Ratio_04': max_zeta04,
    'EQ_Damping_Ratio_05': max_zeta05,
...
data = {
    ...
    'Max_Base Reaction': median_reaction,
    'Equivalent Viscous Damping Ratio': median_eqDR,
}
