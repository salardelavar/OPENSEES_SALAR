...
#                      FREE-VIBRATION ANALYSIS OF MULTI-DEGREE-FREEDOM STRUCTURE USING OPENSEES                      #
#--------------------------------------------------------------------------------------------------------------------#
...
def FREE_VIBRATION_SDOF(IA, IU, IV, SPRING_KIND, M, C, K_elastic, KP, KN, u0, v0, a0, duration, dt):
    ...
    if SPRING_KIND == 'ELASTIC': 
        ...
        ops.element('zeroLength', 1, 1, 2,'-mat', 11,'-dir', 1) # INFO LINK: https://openseespydoc.readthedocs.io/en/latest/src/ZeroLength.html
        ops.element('zeroLength', 2, 2, 3,'-mat', 22,'-dir', 1)
        ops.element('zeroLength', 3, 3, 4,'-mat', 33,'-dir', 1)
        ops.element('zeroLength', 4, 4, 5,'-mat', 44,'-dir', 1)
    if SPRING_KIND == 'INELASTIC': 
        ...
        ops.element('zeroLength', 1, 1, 2,'-mat', 11, 311,'-dir', 1, 1)
        ops.element('zeroLength', 2, 2, 3,'-mat', 22, 322,'-dir', 1, 1)
        ops.element('zeroLength', 3, 3, 4,'-mat', 33, 333,'-dir', 1, 1)
        ops.element('zeroLength', 4, 4, 5,'-mat', 44, 344,'-dir', 1, 1)
    ...
    while stable == 0 and current_time < duration:
        ...
        disp.append(ops.nodeDisp(5, 1))   # DISPLACEMENT  
        vel.append(ops.nodeVel(5, 1))     # VELOCITY
        accel.append(ops.nodeAccel(5, 1)) # ACCELERATION
    ...
    return time, reaction, disp, vel, accel, stiffness, PERIOD, damping_ratio, deltaT_mean, np.array(PERIOD_MIN), np.array(PERIOD_MAX)
...
SPRING_KIND = 'ELASTIC'
time, reactionE, dispE, velE, accelE, stiffnessE, periodE, E_damping_ratioE, E_periodE, period_minE, period_maxE = FREE_VIBRATION_SDOF(IA, IU, IV, SPRING_KIND, M, C, K_elastic, KP, KN, u0, v0, a0, duration, dt)
S02.PERIOD_FUN(dispE, dt)

SPRING_KIND = 'INELASTIC'
time, reactionI, dispI, velI, accelI, stiffnessI, periodI, E_damping_ratioI, E_periodI, period_minP, period_maxP = FREE_VIBRATION_SDOF(IA, IU, IV, SPRING_KIND, M, C, K_elastic, KP, KN, u0, v0, a0, duration, dt)
