...
SPRING_KIND = 'ELASTIC'
DATA = FREE_VIBRATION_SDOF(IA, IU, IV, SPRING_KIND, u0, v0, a0, duration, dt)
(time, reactionE, dispE, velE, accelE,
 stiffnessE, periodE, E_damping_ratioE, E_periodE,
 FDe, FSe, FIe, diE) = DATA
...
(time, reactionI, dispI, velI, accelI,
 stiffnessI, periodI, E_damping_ratioI, E_periodI,
 FDi, FSi, FIi, diI) = DATA
...
plt.title(f'Displacement vs Base-reaction', fontsize=10)
...
plt.title(f'Damping Force (fD) vs Velocity Curve', fontsize=10)
plt.legend(['ELASTIC', 'INELASTIC'])
plt.grid()
plt.show()
...
plt.title(f'Inertia Force (fI) vs Acceleration Curve', fontsize=10)
plt.legend(['ELASTIC', 'INELASTIC'])
plt.grid()
plt.show()
...
plt.title(f'Spring Force (fS) vs Displacement', fontsize=10)
plt.legend(['ELASTIC', 'INELASTIC'])
plt.grid()
plt.show()
...
plt.xlabel('Displacement [m]', fontsize=10)
plt.ylabel('Damping Force (fD) [N]', fontsize=10)
plt.title(f'Damping Force (fD) vs Displacement Curve')
plt.legend(['ELASTIC', 'INELASTIC'])
plt.grid()
plt.show()
...
# Print the Model
#printModel()
ops.printModel("-JSON", "-file", "FREE-VIBRATION_U0_VO_A0_TWO_FS_FD_FI.json")
#%%-------------------------------------------------------------------------------
# EXCEL OUTPUT
import pandas as pd

# Create DataFrame function
def create_df(dispE, dispI, velE, velI, accelE, accelI, reactionE, reactionI, FDe, FDi, FIe, FIi, FSe, FSi, diE, diI):
    df = pd.DataFrame({
...
# Save to Excel
with pd.ExcelWriter("FREE-VIBRATION_U0_VO_A0_TWO_FS_FD_FI_OUTPUT.xlsx", engine='openpyxl') as writer:
    
    df1 = create_df(dispE, dispI, velE, velI, accelE, accelI, reactionE, reactionI, FDe, FDi, FIe, FIi, FSe, FSi, diE, diI)
