...
NUM_SIM = 50000                                # Total number for simulation
#------------------------------------------------------------------------------------------------
...
M = S01.BETA_PDF(60000.0, 65000.0, 2, 1, NUM_SIM)             # [kg] Mass of the Structure
...
T_ELASTIC = 2 * np.pi * np.sqrt(np.max(M)/np.max(EsW))       # Period of Elastic Structure
T_PLASTIC = 2 * np.pi * np.sqrt(np.max(M)/np.max(fuW/esuW))  # Period of Plastic Structure
...
print(f'Displacement Ductility Ratio (μ):    {mu[-1]:.2f}')
print(f'Ductility Coefficient (Rμ):          {R_mu[-1]:.2f}')
print(f'Structural Behavior Coefficient (R): {R[-1]:.2f}')
#------------------------------------------------------------------------------------------------
...
# STRUCTURAL BEHAVOIR COEFFICIENT
S01.HISROGRAM_BOXPLOT(Omega_0, HISTO_COLOR='lime', LABEL='Over Strength Coefficient (Ω0)')
S01.HISROGRAM_BOXPLOT(mu, HISTO_COLOR='grey', LABEL='Displacement Ductility Ratio (μ)')
S01.HISROGRAM_BOXPLOT(R_mu, HISTO_COLOR='yellow', LABEL='Ductility Coefficient (Rμ)')
S01.HISROGRAM_BOXPLOT(R, HISTO_COLOR='purple', LABEL='Structural Behavior Coefficient (R)')
#------------------------------------------------------------------------------------------------
### OPENSEES FUNCTION
def ANALYSIS_SDOF(i):
    ...
    ops.uniaxialMaterial('Hysteretic', MatTag02, fy[i], ey[i], fu[i], esu[i], 0.2*fu[i], 1.1*esu[i], -fy[i], -ey[i], -fu[i], -esu[i], -0.2*fu[i], -1.1*esu[i], pinchX, pinchY, damage1, damage2, beta)
