...
def ANALYSIS_SDOF(i):
    ...
    ops.uniaxialMaterial('HystereticSM', MatTag02, '-posEnv', *KP.flatten(), '-negEnv', *KN.flatten(), '-pinch', 1, 1, '-damage', 0.1, 0.01, '-beta', 0,'-defoLimitStates', 0.01, -0.01, 0.02, -0.02, '-forceLimitStates', fy[i], -fy[i], fu[i], -fu[i],'printInput')
    ...
    Omega02 = 2 * Omega01
    a0 = DR[i] * (2 * Omega01 * Omega02) / (Omega01 + Omega02) # c = a0 * m : Mass-proportional damping
    a1 = DR[i] * 2 / (Omega01 + Omega02) # c = a1 * k : Stiffness-proportional damping
