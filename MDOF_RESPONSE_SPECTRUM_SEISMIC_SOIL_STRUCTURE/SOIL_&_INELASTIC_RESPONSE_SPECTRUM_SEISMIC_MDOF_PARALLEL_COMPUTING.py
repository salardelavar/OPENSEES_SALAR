...
import PLOT_2D_SEIMIC_FUN as S08
...
# ===================================================
# PARALLEL FUNCTION — runs one seismic record
# ===================================================
def run_single_seismic(j):
    ...
    for i in range(NUM_PERIOD):
        m_eff = mi * ((i + 1) / NUM_PERIOD) * 0.2

        data = ANALYSIS_MDOF(j, m_eff)

        (time,
         displacement_soil, displacement_stru,
         velocity_soil, velocity_stru,
         acceleration_soil, acceleration_stru,
         base_reaction, SDF, DI,
         PERIOD, damping_ratio, stiffness) = data
        ...
        # Damage Index clipped
        DI_val = np.max(np.abs(DI))
        DI_val = min(max(DI_val, 0.0), 100.0)
    ...
    return {
        "soil_disp": max_displacement_soil,
        "soil_vel": max_velocity_soil,
        "soil_acc": max_acceleration_soil,
        "stru_disp": max_displacement_stru,
        "stru_vel": max_velocity_stru,
        "stru_acc": max_acceleration_stru,
        "SOIL-REACTION-FORCE": max_base_reaction,
        "DI": max_DI,
        "damping": DAMPING_RATIO,
        "K": max_K,
        "PERIOD": max_T,
        "SOIL-DAMPING-FORCE": max_SDF,
        }
...
if __name__ == "__main__":
    ...
    NUM_SEISMIC = 1      # Example
    NUM_PERIOD = 15       # Example
    ...
    DATA = {
        ...
        5: [],   # Str Vel
        6: [],   # Str Acc
        7: [],   # Soil Reaction Force
        8: [],   # DI
        9: [],   # Damping Ratio
        10: [],  # Soil Stiffness
        11: [],  # Period
        12: [],  # Soil Damping Force
    ...
    # ===================================================
    print("\nRunning parallel seismic analysis...\n")

    results = Parallel(n_jobs=-1, backend='loky')(
        delayed(run_single_seismic)(j)
    ...
    # ===================================================
    for res in results:
        DATA[1].append(res["soil_disp"])
        DATA[2].append(res["soil_vel"])
        DATA[3].append(res["soil_acc"])

        DATA[4].append(res["stru_disp"])
        DATA[5].append(res["stru_vel"])
        DATA[6].append(res["stru_acc"])

        DATA[7].append(res["SOIL-REACTION-FORCE"])
        DATA[8].append(res["DI"])
        DATA[9].append(res["damping"])
        DATA[10].append(res["K"])
        DATA[11].append(res["PERIOD"])
        DATA[12].append(res["SOIL-DAMPING-FORCE"])
        #print(f'\n SEISMIC {j + 1} DONE \n')
    ...
    S08.PLOT_2D_SEIMIC(1, XLABEL, YLABEL, TITLE, SEMILOGY)
    
    print("\n\n Analysis completed successfully (Parallel Model)\n") 
...
results_df.to_excel('SOIL_INELASTIC_RESPONSE_SPECTRUM_SEISMIC_SDOF_LAST_ANALYSIS_DATA_RESULTS.xlsx', index=False)
#%%------------------------------------------------------------------------------------------------  
XLABEL = 'Displacement'
