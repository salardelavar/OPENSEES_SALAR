...
for step in range(n_steps):
    ...
    if abs(dispY) == abs(Collapse_Disp) and not delete_element: 
        ...
        delete_element = True 
        forceH = ops.nodeReaction(1, 1) + ops.nodeReaction(7, 1) # SHEAR BASE REACTION
        forceV = ops.nodeReaction(1, 2) + ops.nodeReaction(7, 2) # AXIAL BASE REACTION
        forceM = ops.nodeReaction(1, 3) + ops.nodeReaction(7, 3) # MOMENT BASE REACTION
    if abs(dispY) > abs(Collapse_Disp):
        forceH = ops.nodeReaction(1, 1) + ops.nodeReaction(7, 1) # SHEAR BASE REACTION
        forceV = ops.nodeReaction(1, 2) + ops.nodeReaction(7, 2) # AXIAL BASE REACTION
        forceM = ops.nodeReaction(1, 3) + ops.nodeReaction(7, 3) # MOMENT BASE REACTION
