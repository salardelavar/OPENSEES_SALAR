...
#      PARALLEL COMPUTING IN OPENSES FOR SCALABLE NONLINEAR DYNAMIC ANALYSIS: RESPONSE SPECTRA AND        #
#                         DUCTILITY DAMAGE INDEX ASSESSMENT UNDER 20 GROUND MOTIONS                       #
#---------------------------------------------------------------------------------------------------------#
...
def RUN_ONE_SEISMIC(j, NUM_PERIOD, mi):

    max_disp, max_vel, max_acc = [], [], []
    max_reac, max_DI, max_T = [], [], []
    max_K, DAMPING_RATIO = [], []
    ...
    for i in range(NUM_PERIOD):
        ...
        (time, disp, vel, acc,
         base_reac, DI, PERIOD, damping_ratio, stiffness) = ANALYSIS_SDOF(j, m)

        max_disp.append(np.max(np.abs(disp)))
        max_vel.append(np.max(np.abs(vel)))
...
results = Parallel(n_jobs=-1, backend="loky")(delayed(RUN_ONE_SEISMIC)(j, NUM_PERIOD, mi)
...
T = np.array(results[0]['T'])

print("\nANALYSIS COMPLETED SUCCESSFULLY (PARALLEL)")

#%%------------------------------------------------------------------------------------------------
# EXPORT RESPONSE SPECTRUM DATA TO EXCEL
...
with pd.ExcelWriter(
    "INELASTIC_RESPONSE_SPECTRUM_SEISMIC_SDOF_RESULTS.xlsx",
    engine="openpyxl"
) as writer:
    for k, v in DATA.items():
        pd.DataFrame(v).to_excel(
            writer,
            sheet_name=labels[k][:31],
            index=False
        )
...
def PLOT_2D(COUNT, XLABEL, YLABEL, TITLE):
    ...
    for j in range(NUM_SEISMIC):
        plt.plot(T, DATA[COUNT][j], alpha=0.35)
    ...
    plt.plot(T, mean, 'k-', lw=3, label='Mean')
    plt.plot(T, median, 'r--', lw=2, label='Median')
    plt.plot(T, q1, 'g-.', lw=2, label='Q1')
    plt.plot(T, q3, 'orange', lw=2, label='Q3')
    plt.fill_between(T, mean-std, mean+std, alpha=0.25, label='Mean ± Std')
...
PLOT_2D(1, "Period T [s]", "Max Displacement [m]", "Displacement Response Spectrum")
PLOT_2D(2, "Period T [s]", "Max Velocity [m/s]", "Velocity Response Spectrum")
PLOT_2D(3, "Period T [s]", "Max Acceleration [m/s²]", "Acceleration Response Spectrum")
PLOT_2D(4, "Period T [s]", "Max Base Reaction [N]", "Base Reaction Spectrum")
PLOT_2D(5, "Period T [s]", "Damage Index [%]", "Damage Index Spectrum")
PLOT_2D(6, "Period T [s]", "Damping Ratio [%]", "Damping Ratio Spectrum")
PLOT_2D(7, "Period T [s]", "Stiffness [N/m]", "Structural Stiffness Spectrum")

#%%------------------------------------------------------------------------------------------------
# FRAGILITY ANALYSIS (PGA-BASED)
...
im_values_PGA = np.array(DATA[3]).flatten()
S07.FRAGILITY_CURVE(im_values_PGA, damage_states_PGA,
                    'Peak Ground Acceleration (g)', SEMILOGY=False, PLOT_KIND=False)

#%%------------------------------------------------------------------------------------------------
# FRAGILITY ANALYSIS (DAMAGE INDEX-BASED)
...
im_values_DI = np.array(DATA[5]).flatten()
S07.FRAGILITY_CURVE(im_values_DI, damage_states_DI,
                    'Damage Index (%)', SEMILOGY=False, PLOT_KIND=False)

#%%------------------------------------------------------------------------------------------------
# EXPORT LAST ANALYSIS DATA
...
pd.DataFrame([DATA_TOTAL]).to_excel(
    'SDOF_LAST_ANALYSIS_CONSTANTS.xlsx',
    index=False
)
