...
#    A NONLINEAR DYNAMIC ANALYSIS USING 20 GROUND MOTIONS: CALCULATION AND PLOTTING OF RESPONSE SPECTRA   #
#                                  AND A DUCTILITY-BASED DAMAGE INDEX USING OPENSEES                      #
#---------------------------------------------------------------------------------------------------------#
...
for j in range(NUM_SEISMIC):
    ...
    for i in range(NUM_PERIOD):
        m = mi
        g = 2*9.81 * (i+1 / NUM_PERIOD) # [m/s^2] standard acceleration of gravity or standard acceleration 
        data = ANALYSIS_SDOF(j, m, g)
...
def PLOT_2D(COUNT, XLABEL, YLABEL, TITLE):
    ...
    for j in range(NUM_SEISMIC):
        plt.plot(T, DATA[COUNT][j], alpha=0.4,
                 label=f'Sim {j+1} | Max: {np.max(np.abs(DATA[COUNT][j])):.4e}')
...
TITLE =  "Ductiity Damage Index Spectrum"
