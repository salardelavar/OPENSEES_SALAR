...
3. Dynamic Response:  
   - Period: Natural period (`T`) calculated from eigenanalysis.  
...
def SEISMIC_ANALYSIS_IDA(j, J_MAX, LENGTH_COL, LENGTH_BM, STEEL_KIND):
    ...
    return FORCE_S, FORCE_A, MOMENT, DISP_X, DISP_Y, ROT, KA, KS, KI, time, velocity_X, velocity_Y, acceleration_X, acceleration_Y, PERIOD_01, PERIOD_02, GMfact, DRIFT, solution[0]
...
for j in range(J_MAX):
    ...
    FORCE_S, FORCE_A, MOMENT, DISP_X, DISP_Y, ROT, KA, KS, KI, time, velocity_X, velocity_Y, acceleration_X, acceleration_Y, PERIOD_01, PERIOD_02, GM, DRIFT, damping = DATA
    ...
    max_GM.append(GM)
    max_DRIFT.append(np.max(np.abs(DRIFT)))
...
#plt.plot(max_GM, max_DRIFT, color='purple', linewidth=2)
plt.scatter(max_GM, max_DRIFT, color='purple', linewidth=2)
plt.title('Standard Acceleration of Gravity During the Analysis')
plt.ylabel('Lateral Structural Drift [%]')
plt.xlabel('Standard Acceleration of Gravity [mm/s^2]')
plt.grid()
plt.show()
...
plt.title(f'Displacement vs Shear Base-reaction - MAX. ABS: {BASE_Z[-1]}')
plt.grid()
plt.show()
...
plt.title(f'Displacement vs Axial Base-reaction - MAX. ABS: {BASE_ZA[-1]}')
plt.grid()
plt.show()
...
# Print the Model
#printModel()
ops.printModel("-JSON", "-file", "CONCRETE_FRAME_SEISMIC_IDA.json")
#%%-------------------------------------------------------------------------------
