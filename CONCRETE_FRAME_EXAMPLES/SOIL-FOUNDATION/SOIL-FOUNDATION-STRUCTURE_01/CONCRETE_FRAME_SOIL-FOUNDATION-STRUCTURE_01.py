...
dt = 0.01        # [s] Time step
MASS = 0.0       # [kg] Mass on the each column
...
def PD_ANALYSIS(ELE_TYPE, STEEL_TYPE, CONCRETE_TYPE, ANA_TYPE, SOIL_TYPE):
    ...
    ops.node(1000, 0.0, 0.0)               # Base Soil Connection
    ops.node(2000, LENGTH_BM, 0.0)         # Base Soil Connection

    ops.fix(1, 1, 1, 0) 
    ops.fix(2, 1, 1, 0) 
    ops.fix(1000, 1, 1, 1)  # Fix all foundation nodes (x, y, rotation)
    ops.fix(2000, 1, 1, 1)  # Fix all foundation nodes (x, y, rotation)
    ...
    if SOIL_TYPE == 'ELASTIC':
        ...
        ops.uniaxialMaterial('Elastic', 4000, 0.3 * k_horizontal, 0, k_horizontal)  # Horizontal spring
        ops.uniaxialMaterial('Elastic', 5000, 0.3 * k_vertical, 0, k_vertical)      # Vertical spring
        ops.uniaxialMaterial('Elastic', 6000, 0.3 * k_rotational, 0, k_rotational)  # Rotational spring
        
    if SOIL_TYPE == 'NO-TENSION':
        # Spring constants for soil-structure interaction
        k_horizontal = 210.0e10  # [N/mm] Horizontal spring constant
        k_vertical = 150.0e10    # [N/mm] Vertical spring constant
        k_rotational = 550.0e6   # [N.mm/Rad] Rotational spring constant
    ...
    if SOIL_TYPE == 'INELASTIC': 
        # Define the MultiLinear material properties for springs
        kh = np.array([[0.05, 2200.0], [0.15, 2700.0], [0.25, 2800.0], [0.3, 2500.5], [0.5, 2100.5]])                   # k horizontal
        kv = np.array([[0.15, 1800.5], [0.3, 2700.0], [0.4, 2900.0], [0.5, 3000.5], [1.1, 2400.4]])                     # k vertical
        kr = np.array([[0.01, 1050000.0], [0.04, 2900000.0], [0.06, 3000000.0], [0.09, 3050000.0], [0.15, 15000000.0]])  # k rotational
        # Define soil-structure interaction springs at base (Node 1)
        #                                      DISP    FORCE
        ops.uniaxialMaterial('MultiLinear', 4000, *kh.flatten()) # Horizontal spring
        ops.uniaxialMaterial('MultiLinear', 5000, *kv.flatten()) # Vertical spring
        ops.uniaxialMaterial('MultiLinear', 6000, *kr.flatten()) # Rotational spring  
        
    # SOIL-FOUNDATION INTERACTIONS SIMULATED WITH SIMPLE SPRINGS
    # BASE 01
    ops.element('zeroLength', 40000, 1000, 1, '-mat', 4000, '-dir', 1) # Horizontal spring for soil stiffness
    ops.element('zeroLength', 50000, 1000, 1, '-mat', 5000, '-dir', 2) # Vertical spring for soil stiffness
    ops.element('zeroLength', 60000, 1000, 1, '-mat', 6000, '-dir', 3) # Rotational spring for soil stiffness
    # BASE 02
    ops.element('zeroLength', 70000, 2000, 2, '-mat', 4000, '-dir', 1) # Horizontal spring for soil stiffness
    ops.element('zeroLength', 80000, 2000, 2, '-mat', 5000, '-dir', 2) # Vertical spring for soil stiffness
    ops.element('zeroLength', 90000, 2000, 2, '-mat', 6000, '-dir', 3) # Rotational spring for soil stiffness
    #%%--------------------------
    ...
    if ELE_TYPE == 'INELASTIC':
        ...
        # LEFT COLUMN
        ops.element('nonlinearBeamColumn', 1, 1, 5, numIntgrPts, secTagC01, transfTag, '-mass', Ele_MassC)             # COLUMN - ELEMENT 01
        ...
        # RIGHT COLUMN
        ops.element('nonlinearBeamColumn', 4, 2, 7, numIntgrPts, secTagC01, transfTag, '-mass', Ele_MassC)              # COLUMN - ELEMENT 04
    ...
    if ELE_TYPE == 'ELASTIC':
        ...
        # LEFT COLUMN
        ops.element('elasticBeamColumn', 1, 1, 5, secTagC01, transfTag, '-mass', Ele_MassC) # COLUMN - ELEMENT 01
        ...
        # RIGHT COLUMN
        ops.element('elasticBeamColumn', 4, 2, 7, secTagC01, transfTag, '-mass', Ele_MassC) # COLUMN - ELEMENT 04
    ...
    if ANA_TYPE == 'PUSHOVER':
        ...
        for step in range(steps):
            ...
            # Record results
            ops.reactions()
            reactionX.append(ops.nodeReaction(1, 1) + ops.nodeReaction(2, 1))       # SHEAR BASE REACTION
            reactionY.append(ops.nodeReaction(1, 2) + ops.nodeReaction(2, 2))       # AXIAL BASE REACTION
            reactionZ.append(ops.nodeReaction(1, 3) + ops.nodeReaction(2, 3))       # MOMENT BASE REACTION
...
DATA = PD_ANALYSIS(ELE_TYPE='INELASTIC', STEEL_TYPE=2, CONCRETE_TYPE=2, ANA_TYPE='PUSHOVER', SOIL_TYPE = 'NO-TENSION')
...
DATA = PD_ANALYSIS(ELE_TYPE='INELASTIC', STEEL_TYPE=2, CONCRETE_TYPE=2, ANA_TYPE='DYNAMIC', SOIL_TYPE = 'NO-TENSION')
...
plt.plot(DISP_Xp, PERIOD_MINp, linewidth=3)
plt.plot(DISP_Xp, PERIOD_MAXp, linewidth=3)
plt.plot(DISP_Xd, PERIOD_MINd, linewidth=3)
plt.plot(DISP_Xd, PERIOD_MAXd, linewidth=3)
plt.title('Period of Structure During Analysis')
plt.ylabel('Structural Period [s]')
plt.xlabel('Displacement [mm]')
#plt.semilogy()
plt.grid()
plt.legend([f'PUSHOVER PERIOD - MIN VALUES: Min: {np.min(PERIOD_MINp):.3f} (s) - Mean: {np.mean(PERIOD_MINp):.3f} (s) - Max: {np.max(PERIOD_MINp):.3f} (s)', 
            f'PUSHOVER PERIOD - MAX VALUES:  Min: {np.min(PERIOD_MAXp):.3f} (s) - Mean: {np.mean(PERIOD_MAXp):.3f} (s) - Max: {np.max(PERIOD_MAXp):.3f} (s)',
            f'DYNAMIC PERIOD - MIN VALUES: Min: {np.min(PERIOD_MINd):.3f} (s) - Mean: {np.mean(PERIOD_MINd):.3f} (s) - Max: {np.max(PERIOD_MINd):.3f} (s)', 
            f'DYNAMIC PERIOD - MAX VALUES:  Min: {np.min(PERIOD_MAXd):.3f} (s) - Mean: {np.mean(PERIOD_MAXd):.3f} (s) - Max: {np.max(PERIOD_MAXd):.3f} (s)',
            ])
plt.show()
