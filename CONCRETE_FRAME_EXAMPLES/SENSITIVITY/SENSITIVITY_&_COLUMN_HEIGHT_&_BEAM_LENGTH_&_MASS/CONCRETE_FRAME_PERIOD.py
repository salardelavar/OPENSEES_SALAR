...
def PD_ANALYSIS(STEEL_KIND, ANA_KIND):
    ...
    if ANA_KIND == 'DYNAMIC':
        ...
        print(f"Min. Period:        {np.min(PERIOD):.4f} [s]") 
        print(f"Mean Period:        {deltaT_mean:.4f} [s]")   
        print(f"Max. Period:        {np.max(PERIOD):.4f} [s]")  
        
        #ops.wipe()  
        return FORCE_S, FORCE_A, MOMENT, DISP_X, DISP_Y, ROT, KA, KS, KI, time, velocity_X, velocity_Y, acceleration_X, acceleration_Y, PERIOD_01, PERIOD_02, delta, PERIOD, deltaT_mean
...
FORCE_Sd, FORCE_Ad, MOMENTd, DISP_Xd, DISP_Yd, ROTd, KAd, KSd, KId, timed, velocity_Xd, velocity_Yd, acceleration_Xd, acceleration_Yd, PERIOD_01d, PERIOD_02d, deltad, periodI, E_periodI = DATA
...
plt.plot(timed, periodI, color='purple', linewidth=1.5, label=f'Structure Period: {E_periodI:.3e}')
...
S05.HISROGRAM_BOXPLOT(periodI, HISTO_COLOR='purple', LABEL='Structure Period')
#%%------------------------------------------------------------------------------  
# EXACT SOLUTION:
...
results_df.to_excel('CONCRETE_FRAME_DUCTILITY_DAMAGE_INDEX_RESULTS.xlsx', index=False) 
#%%------------------------------------------------------------------------------
...
# Print the Model
#printModel()
ops.printModel("-JSON", "-file", "CONCRETE_FRAME_DUCTILITY_DAMAGE_INDEX.json")
#%%-------------------------------------------------------------------------------
