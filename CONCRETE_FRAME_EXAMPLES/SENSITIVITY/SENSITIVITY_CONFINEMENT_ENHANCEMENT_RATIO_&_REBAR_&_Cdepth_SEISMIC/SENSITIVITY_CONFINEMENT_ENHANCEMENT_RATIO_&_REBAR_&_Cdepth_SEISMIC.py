...
#     AND CONFINEMENT ENHANCEMENT RATIO USING OPENSEES FOR STRUCTURAL BEHAVIOR COEFFICIENT CALCULATION         #
#--------------------------------------------------------------------------------------------------------------#
...
#%%------------------------------------------------------------------------------
def PD_ANALYSIS(Hc, DIAc, Kfc, STEEL_KIND, ANA_KIND):
    ...
    if ANA_KIND == 'DYNAMIC':
        ...
        #ops.wipe()  
        return RO_COL, RO_BE, FORCE_S, FORCE_A, MOMENT, DISP_X, DISP_Y, ROT, KA, KS, KI, time, velocity_X, velocity_Y, acceleration_X, acceleration_Y, PERIOD_01, PERIOD_02, damping_ratio
...
velocity_X_MAX_d, velocity_Y_MAX_d, acceleration_X_MAX_d, acceleration_Y_MAX_d = [], [], [], []
...
for hc in HCC:
    for dia in DIAc:
        for kf in Kfc:
            II = II + 1
            print(f'\n STEP: {II} - Hc: {hc} - REBAR DIAMETER: {dia} - K: {kf}\n')
            HCC_MAX.append(hc)
            DIAc_MAX.append(dia)
            Kfc_MAX.append(kf)
            DATA = PD_ANALYSIS(hc, dia, kf, STEEL_KIND=2, ANA_KIND='PUSHOVER')
            ...
            print('\n STEP: {II} PUSHOVER DONE. \n')
            
            DATA = PD_ANALYSIS(hc, dia, kf, STEEL_KIND=2, ANA_KIND='DYNAMIC')
            (_, _, FORCE_Sd, FORCE_Ad, MOMENTd, DISP_Xd, DISP_Yd, ROTd,
             KAd, KSd, KId, timed, velocity_Xd, velocity_Yd, acceleration_Xd,
             acceleration_Yd, PERIOD_01, PERIOD_02, damping_ratio) = DATA
            
            FORCE_S_MAX_d.append(np.max(np.abs(FORCE_Sd)))
            FORCE_A_MAX_d.append(np.max(np.abs(FORCE_Ad)))
            MOMENT_MAX_d.append(np.max(np.abs(MOMENTd)))
            DISP_X_MAX_d.append(np.max(np.abs(DISP_Xd)))
            DISP_Y_MAX_d.append(np.max(np.abs(DISP_Yd)))
            ROT_MAX_d.append(np.max(np.abs(ROTd)))
            velocity_X_MAX_d.append(np.max(np.abs(velocity_Xd))) 
            velocity_Y_MAX_d.append(np.max(np.abs(velocity_Yd))) 
            acceleration_X_MAX_d.append(np.max(np.abs(acceleration_Xd))) 
            acceleration_Y_MAX_d.append(np.max(np.abs(acceleration_Yd))) 
            KA_MAX_d.append(np.max(np.abs(KAd)))
            KS_MAX_d.append(np.max(np.abs(KSd)))
            KI_MAX_d.append(np.max(np.abs(KId)))
            PERIOD_01_MAX.append(PERIOD_01)  
            PERIOD_02_MAX.append(PERIOD_02)  
            DAMPING_RATIO_MAX.append(damping_ratio)
            
            print('\n STEP: {II} DYNAMIC DONE. \n')
...
XLABEL = 'Column Steel Reinforcement Ratio [%]'   
YLABEL = 'Beam Steel Reinforcement Ratio [%]' 
ZLABEL = 'Structural Period Min. [s]' 
...
XLABEL = 'Column Steel Reinforcement Ratio [%]'   
YLABEL = 'Beam Steel Reinforcement Ratio [%]' 
ZLABEL = 'Structural Period Max. [s]' 
...
results_df.to_excel('SENSITIVITY_CONFINEMENT_ENHANCEMENT_RATIO_&_REBAR_&_Cdepth_SEISMIC_RESULTS.xlsx', index=False) 
#%%------------------------------------------------------------------------------
def PLOT_HEATMAP(df):
    import matplotlib.pyplot as plt
    import numpy as np
...
# Print the Model
#printModel()
ops.printModel("-JSON", "-file", "SENSITIVITY_CONFINEMENT_ENHANCEMENT_RATIO_&_REBAR_&_Cdepth_SEISMIC.json")
#%%------------------------------------------------------------------------------
