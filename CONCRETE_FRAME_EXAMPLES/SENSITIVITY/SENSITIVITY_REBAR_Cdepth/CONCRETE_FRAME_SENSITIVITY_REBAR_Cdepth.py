...
################################################################################################################
"""
This Python script performs a sensitivity analysis on a 2D reinforced concrete (RC) frame by varying
 column rebar diameter and column section depth to evaluate their impact on structural behavior
 coefficients, which are critical in earthquake engineering. The analysis is conducted using OpenSeesPy,
 a powerful finite element framework for nonlinear structural analysis.  

# Key Features:  
1. Material Modeling:  
   - Concrete: Uses `Concrete01` (uniaxial material with linear tension/compression) or `Concrete02` (parabolic model with hardening).  
   - Steel Reinforcement: Models rebars with hysteretic behavior (pinching, hardening, and degradation).  

2. Section Modeling:  
   - Fiber-based sections for columns and beams with confined/unconfined concrete.  
   - Rebar ratio sensitivity by varying diameters (e.g., 20–32 mm).  

3. Pushover Analysis:  
   - Displacement-controlled nonlinear static analysis to simulate seismic demands.  
   - Tracks base shear, displacement, stiffness degradation, and ductility.  

4. Bilinear Curve Fitting:  
   - Extracts elastic stiffness, post-yield stiffness, and ultimate capacity.  
   - Computes ductility ratio (μ) and overstrength factor (Ω₀) per FEMA-356/ATC-40.  

5. Structural Behavior Coefficient (R):  
   - Calculates R = Ω₀ × Rμ, where Rμ accounts for ductility (μ).  
   - Critical for seismic design codes (e.g., ASCE 7, Eurocode 8).  

6. 3D Visualization:  
   - Plots R, ductility, and stiffness as functions of rebar size and column depth.  

7. Drift Analysis:  
   - Evaluates interstory drift ratios, a key metric for seismic performance.  

8. Automated Data Export:  
   - Results saved to Excel for further statistical analysis.  

9. Correlation Heatmaps:  
   - Identifies relationships between design variables (e.g., rebar size vs. ductility).  

10. Advanced Numerical Techniques:  
    - Newton-Raphson iterations with adaptive displacement control.  
    - Corotational geometric transformations for large displacements.  

# Applications:  
- Performance-Based Earthquake Engineering (PBEE): Assesses collapse prevention and life safety.  
- Parametric Optimization: Guides design choices for retrofitting or new constructions.  
- Code Compliance: Validates R-factor assumptions in seismic design.  

# Outputs:  
- Force-displacement hysteresis, moment-rotation curves, stiffness degradation.  
- 3D sensitivity plots for design trade-offs.  
"""
...
#%% DEFINE PARAMETRS FOR NONLINEAR STATIC ANALYSIS 
...
def PUSHOVER_ANALYSIS(DIAc, Hc, LENGTH_COL, LENGTH_BM, DMAX, DINCR, STEEL_KIND, CONCRETE_KIND):
    ...
    return RO_COL, RO_BE, FORCE_S, FORCE_A, MOMENT, DISP_X, DISP_Y, ROT, KA, KS, KI, Elastic_ST, Plastic_ST, Tangent_ST, Ductility_Rito, Over_Strength_Factor, R, STEP, DRIFT
...
for dia in DIAc:
    for hc in Hc:
        ...
        RO_COL, RO_BE, FORCE_S, FORCE_A, MOMENT, DISP_X, DISP_Y, ROT, KA, KS, KI, Elastic_ST, Plastic_ST, Tangent_ST, Ductility_Rito, Over_Strength_Factor, R, STEP, DRIFT = DATA
...
# Print the Model
#printModel()
ops.printModel("-JSON", "-file", "CONCRETE_FRAME_SENSITIVITY_REBAR_Cdepth.json")
#%%------------------------------------------------------------------------------  
