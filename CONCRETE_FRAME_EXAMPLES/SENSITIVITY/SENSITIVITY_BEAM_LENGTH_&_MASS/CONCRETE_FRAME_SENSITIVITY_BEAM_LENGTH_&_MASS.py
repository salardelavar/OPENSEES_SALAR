...
#      SENSITIVITY ANALYSIS OF CONCRETE FRAME BEHAVIOR: INVESTIGATING THE IMPACT OF BEAM LENGTH AND MASS ON    #
#                    STRUCTURAL PERIOD AND OTHER KEY PARAMETERS USING OPENSEES AND PYTHON                      #
#--------------------------------------------------------------------------------------------------------------#
#                             THIS PROGRAM WRITTEN BY SALAR DELAVAR GHASHGHAEI (QASHQAI)                       #
#                                        EMAIL: salar.d.ghashghaei@gmail.com                                   #
################################################################################################################
"""
# Nonlinear Dynamic and Sensitivity Analysis of a Concrete Frame Using OpenSees  
---------------------------------------------------------------
This study performs a comprehensive nonlinear dynamic analysis and sensitivity assessment
...
1. Beam length – Examining how different span lengths influence dynamic behavior  
2. Structural mass – Investigating the effect of mass variation on seismic performance  
---------------------------------------------------------------
## Methodology  
1. Model Development  
   - Create a nonlinear finite element model of a reinforced concrete moment-resisting frame  
   - Implement fiber sections with appropriate material models (Concrete02, Steel02)  
   - Include geometric nonlinearities (P-Delta effects)  
...
This analysis framework provides a robust approach for evaluating parameter sensitivity in
 structural systems using OpenSees' advanced nonlinear analysis capabilities.
"""
#%%------------------------------------------------------------------------------
...
MAX_TOLERANCE = 1.0e-5  # Convergence tolerance for test
#STEEL_KIND: 1 -> WITHOUT HARDENING AND ULTIMATE STRAIN
#STEEL_KIND: 2 -> WITH HARDENING AND ULTIMATE STRAIN
#%%------------------------------------------------------------------------------
def PD_ANALYSIS(LENGTH_BM, MASS, STEEL_KIND, ANA_KIND):
    # Reset model
    ops.wipe()
...
#%%------------------------------------------------------------------------------
# SENSTIVITY ANALYSIS BY CHANGING BEAM LENGTH AND MASS
# Analysis Durations:
...
for bl in BL:
    for m in mass:
        II = II + 1
        print(f'\n STEP: {II} - BEAM LENGTH: {bl} - MASS: {m} \n')
        BL_MAX.append(bl)
        MASS_MAX.append(m)
        DATA = PD_ANALYSIS(bl, m, STEEL_KIND=2, ANA_KIND='DYNAMIC')
...
results_df.to_excel('CONCRETE_FRAME_BEAM_LENGTH_&_MASS_RESULTS.xlsx', index=False) 
#%%------------------------------------------------------------------------------
def PLOT_HEATMAP(df):
    import matplotlib.pyplot as plt
    import numpy as np
...
# Print the Model
#printModel()
ops.printModel("-JSON", "-file", "CONCRETE_FRAME_BEAM_LENGTH_&_MASS.json")
#%%------------------------------------------------------------------------------  