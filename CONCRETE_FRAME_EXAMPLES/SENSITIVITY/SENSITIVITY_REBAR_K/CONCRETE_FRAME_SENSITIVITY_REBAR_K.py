...
#  SENSITIVITY ANALYSIS OF CONCRETE FRAME BY CHANGING COLUMN REBAR DIAMETER AND STRENGTH ENHANCEMENT FACTOR    # 
...
#%%------------------------------------------------------------------------------
def PUSHOVER_ANALYSIS(DIAc, Kfc, LENGTH_COL, LENGTH_BM, DMAX, DINCR, STEEL_KIND, CONCRETE_KIND):
    ...
    if CONCRETE_KIND == 1:        
        ...
        fcUC = fcC*0.7      # [N/mm²] Concrete Compressive Ultimate Strength
    ...
    if CONCRETE_KIND == 2:
        ...
        fc1C = Kfc*fcU;		# [N/mm²] CONFINED concrete (mander model), maximum stress - COLUMN AND BEAM
...
#%%------------------------------------------------------------------------------
# SENSTIVITY ANALYSIS BY CHANGING REBAR DIAMETER AND STRENGTH ENHANCEMENT FACTOR
# Analysis Durations:
...
DIAc = [20, 22, 28, 30, 32]
# STRENGTH ENHANCEMENT FACTOR (K)
Kfc = [1.05, 1.10, 1.15, 1.175, 1.20, 1.25, 1.30]
II = 0
for dia in DIAc:
    for kf in Kfc:
        II = II + 1
        ...
        DIAc_MAX.append(dia)
        Kfc_MAX.append(kf)
        DATA = PUSHOVER_ANALYSIS(dia, kf, LENGTH_COL, LENGTH_BM, DMAX, DINCR, STEEL_KIND=2, CONCRETE_KIND=1)
...
results_df.to_excel('CONCRETE_FRAME_SENSITIVITY_REBAR_K_RESULTS.xlsx', index=False) 
#%%------------------------------------------------------------------------------
def PLOT_HEATMAP(df):
    import matplotlib.pyplot as plt
    import numpy as np
...
# Print the Model
#printModel()
ops.printModel("-JSON", "-file", "CONCRETE_FRAME_SENSITIVITY_REBAR_K.json")
#%%------------------------------------------------------------------------------
