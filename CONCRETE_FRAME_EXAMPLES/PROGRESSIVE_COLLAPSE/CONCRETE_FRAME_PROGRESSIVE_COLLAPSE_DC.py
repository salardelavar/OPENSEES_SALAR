...
PX, PY, MZ = 1.0, -10000.0, 0.0

# Apply the triangular distributed load
D_MAX = 150.0            # [mm] Maximum Displacement
D_INCR = 0.05            # [mm] Incremental Displacement

# Apply the triangular distributed load
DL = -0.0175              # [N/mm] Maximum Distributed Load
dl = False                # True: Apply - False: Not Apply

Collapse_Disp = 10.0      # [mm] Absolute Value Collapse Vertical Displacement
...
#%%------------------------------------------------------------------------------
def PUSHOVER_ANALYSIS(LENGTH_COL, LENGTH_BM, D_MAX, D_INCR, STEEL_KIND):
    # Reset model
    ops.wipe()
    ...
    if dl == True:
        ...
        ops.eleLoad('-ele', 9, '-type', '-beamUniform', DL * 0.25, 0.0) 
        ops.eleLoad('-ele', 10, '-type', '-beamUniform', DL * 0.50, 0.0)
        ops.eleLoad('-ele', 11, '-type', '-beamUniform', DL * 0.75, 0.0)
        ops.eleLoad('-ele', 12, '-type', '-beamUniform', DL * 1.00, 0.0)
    ...
    ops.integrator('DisplacementControl', 4, 1, D_INCR) # INFO LINK: https://openseespydoc.readthedocs.io/en/latest/src/displacementControl.html
    ops.analysis('Static')
    
    # Total steps
    steps = int(np.abs(D_MAX / D_INCR))
    delete_element = False

    for step in range(steps):
        OK = ops.analyze(1)
        S02.ANALYSIS(OK, 1, MAX_TOLERANCE, MAX_ITERATIONS) # CHECK THE ANALYSIS
        disp_X = ops.nodeDisp(4, 1) # LATERAL DISPLACEMENT IN X FOR NODE 4
        disp_Y = ops.nodeDisp(4, 2) # LATERAL DISPLACEMENT IN Y FOR NODE 4
        rot = ops.nodeDisp(4, 3)    # ROTATION IN Z FOR NODE 4  
        """
        # NOTICE: YOU CAN CHECK ELEMENT FORCES FOR YOUR COLLAPSE EVALUTION. I JUST USED DISPLACEMENT FOR CHECKING
        shear = ops.eleResponse(8, 'force')[0]   # COLUMN 08 - SHEAR FORCE
        axial = ops.eleResponse(8, 'force')[1]   # COLUMN 08 - AXIAL FORCE
        moment = ops.eleResponse(8, 'force')[2]  # COLUMN 08 - MOMENT FORCE
        """
        ops.reactions()
        ...
        if abs(disp_Y) == abs(Collapse_Disp) and not delete_element: # DISPLACEMENT CONTROL FOR COLUMN 3 AND DELETE ELEMENT
            ...
            #ops.remove('element', 5, 6, 7, 8) # REMOVE ELEMENTS
            #ops.remove('sp', 2, 11, 12 , 13)  # REMOVE NODES
            ops.remove('element', 5) # REMOVE ELEMENT 5
            ops.remove('element', 6) # REMOVE ELEMENT 6
            ops.remove('element', 7) # REMOVE ELEMENT 7
            ops.remove('element', 8) # REMOVE ELEMENT 8
            ops.remove('sp', 2)      # REMOVE NODE 2
            ops.remove('sp', 11)     # REMOVE NODE 11
        ...
        STEP.append(step)        # DISTRIBUTED LOAD 
...
#%%------------------------------------------------------------------------------
# Analysis Durations for Static Dynamic Analysis:
starttime = TI.process_time()

FORCE_S, FORCE_A, MOMENT, DISP_X, DISP_Y, ROT, KA, KS, KI, STEP = PUSHOVER_ANALYSIS(LENGTH_COL, LENGTH_BM, D_MAX, D_INCR, STEEL_KIND=2)
