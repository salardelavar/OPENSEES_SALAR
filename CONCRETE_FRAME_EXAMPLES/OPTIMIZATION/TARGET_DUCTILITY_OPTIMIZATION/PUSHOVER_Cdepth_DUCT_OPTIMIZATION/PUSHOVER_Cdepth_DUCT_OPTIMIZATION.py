...
#         STRUCTURAL DUCTILITY RATIO OPTIMIZATION WITH PUSHOVER ANALYSIS OF CONCRETE FRAME SECTION        # 
#               EVALUATING STRAIN HARDENING AND ULTIMATE STRAIN CRITERIA USING OPENSEES                   #
#---------------------------------------------------------------------------------------------------------#
#             FIND BEST COLUMN CONCRETE SECTION DEPTH WITH DEFINED STRUCTURAL DUCTILITY RATIO             #
...
7. A Newton-Raphson root-finding algorithm adjusts the column section depth to
...
9. Each iteration updates the column section depth until convergence is achieved or the
 maximum number of iterations is reached.
10. Convergence is based on the residual of the diameter update (DX) relative
 to a tolerance threshold.
11. The optimal column section depth is printed upon successful convergence.
12. This method allows automated column section depth design optimization 
 based on seismic performance criteria.
"""
...
#%%------------------------------------------------------------------------------
def PUSHOVER_ANALYSIS(Hc, LENGTH_COL, LENGTH_BM, DMAX, DINCR, STEEL_KIND, CONCRETE_KIND):
    # Reset model
    ops.wipe()
    ...
    return FORCE_S, FORCE_A, MOMENT, DISP_X, DISP_Y, ROT, KA, KS, KI, mu, STEP

#%%------------------------------------------------------------------------------
# WITHOUT HARDENING AND ULTIMATE STRAIN
#FORCE_S, FORCE_A, MOMENT, DISP_X, DISP_Y, ROT, KA, KS, KI, R, STEP = PUSHOVER_ANALYSIS(Hc, LENGTH_COL, LENGTH_BM, DMAX, DINCR, STEEL_KIND=1, CONCRETE_KIND=1)
# WITH HARDENING AND ULTIMATE STRAIN
#FORCE_S02, FORCE_A02, MOMENT02, DISP_X02, DISP_Y02, ROT02, KA02, KS02, KI02, R02, STEP02  = PUSHOVER_ANALYSIS(Hc, LENGTH_COL, LENGTH_BM, DMAX, DINCR, STEEL_KIND=2, CONCRETE_KIND=1)
#%%------------------------------------------------------------------------------
# FIND BEST COLUMN CONCRETE SECTION DEPTH WITH STRUCTURAL DUCTILITY RATIO:
...
TARGET_DUCT = 7.5  # [mm/mm] Target Structural Ductility Ratio
...
while (RESIDUAL > TOLERANCE):
    ...
    FORCE_S02, FORCE_A02, MOMENT02, DISP_X02, DISP_Y02, ROT02, KA02, KS02, KI02, DUCT_ANA, STEP02 = DATA
    ...
    FORCE_S02, FORCE_A02, MOMENT02, DISP_X02, DISP_Y02, ROT02, KA02, KS02, KI02, DUCT_ANA_MIN, STEP02 = DATA
    Fmin = DUCT_ANA_MIN - TARGET_DUCT
    print('Fmin: ', Fmin)
    # Xmax -------------------------------------------------------
    XMAX = X + ESP 
    DATA  = PUSHOVER_ANALYSIS(XMAX, LENGTH_COL, LENGTH_BM, DMAX, DINCR, STEEL_KIND=2, CONCRETE_KIND=1)
    FORCE_S02, FORCE_A02, MOMENT02, DISP_X02, DISP_Y02, ROT02, KA02, KS02, KI02, DUCT_ANA_MAX, STEP02 = DATA
...
results_df.to_excel('PUSHOVER_Cdepth_DUCT_OPTIMIZATION_RESULTS.xlsx', index=False) 
#%%------------------------------------------------------------------------------
