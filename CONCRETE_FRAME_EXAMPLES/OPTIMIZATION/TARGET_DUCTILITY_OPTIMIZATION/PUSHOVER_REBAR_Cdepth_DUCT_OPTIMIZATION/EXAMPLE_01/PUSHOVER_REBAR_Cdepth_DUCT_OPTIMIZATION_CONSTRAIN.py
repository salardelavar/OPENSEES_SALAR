...
###########################################################################################################
"""
This study presents a performance-based framework for the constrained optimization
 of reinforced concrete (RC) column design using nonlinear pushover analysis implemented
 in OpenSeesPy. The proposed methodology integrates detailed fiber-based modeling of RC
 sections with nonlinear material behavior for confined and unconfined concrete and 
 reinforcing steel, enabling accurate simulation of strength degradation, stiffness
 nonlinearity, and ductile response. Key design variables, including column longitudinal
 reinforcement diameter and section depth, are treated as optimization parameters and
 are bounded by practical construction limits.
A constrained nonlinear optimization strategy is employed to identify optimal column
 configurations that satisfy target seismic performance criteria, defined in terms of
 displacement ductility ratio and overstrength factor. The objective function
 minimizes the deviation from prescribed performance targets while enforcing
 constraint compliance through penalty functions. The optimization is solved
 using a bounded quasi-Newton algorithm, ensuring numerical stability and
 robustness in the presence of highly nonlinear structural response. 
 The proposed framework demonstrates the feasibility of automated, 
 analysis-driven seismic design of RC columns and provides a practical
 tool for performance-based earthquake engineering applications.
...
ITMAX = 200        # Max. Iteration

TARGET_DUCT = 7.50 # [mm/mm] Target Structural Behavior Coefficient
TARGET_OSF = 1.10  # [N/N] Over Srength Factor
...
result = minimize(
    ...
    bounds=bounds,
    options={
        'maxiter': ITMAX,    # Maximum number of optimization iterations
        'ftol': TOLERANCE,   # Function tolerance (convergence criterion)
