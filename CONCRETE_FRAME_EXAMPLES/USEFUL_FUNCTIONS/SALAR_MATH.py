...
def MULTIPLE_REGRESSION(df):
    ...
    X = sm.add_constant(df[[
        'velocity_X_DYNAMIC',
        'acceleration_X_DYNAMIC',
        'FORCE_S_DYNAMIC',
        'Damping_ratio_DYNAMIC',
        'Structural_Behavior_Coefficient'
    ]])

    # Fit the multiple regression model
    model = sm.OLS(df['DISP_X_DYNAMIC'], X).fit()
...
def RANDOM_FOREST(df):
    ...
    target_threshold = np.median(df['DISP_X_DYNAMIC'])  # Example threshold
    df['Safety_label'] = (df['DISP_X_DYNAMIC'] <= target_threshold).astype(int)
