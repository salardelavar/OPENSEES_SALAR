...
#----------------------------------------------------------------------
def CONCRETE_CONFINED_REC_PLATE_SECTION_QUAD(id, fc, Kc, plateThickness, STEEL_TYPE, CONCRETE_TYPE, PLOT=True, CONCRETE_DENSITY=2500/1e9, STEEL_DENSITY=7850/1e9):
    """
    ...
    ftU = fcU * 0.1                           # [N/mm²] Tensile strength
    ...
    ftC = fcC * 0.1                           # [N/mm²] Tensile strength
    ...
    esuP = 0.25        # [mm/mm] Steel Rebar Ultimate Strain
    EshP = (fuP - fyP)/(esuP - eyP)
    BsP = EshP / EsP
    
    plateThickness = 10.0 # [mm] Plate Thickness

    BSec = 400              # [mm] Depth of the Section 
    HSec = 500              # [mm] Height of the Section  
    coverB = 50             # [mm] Concrete Section Cover
    coverH = 50             # [mm] Concrete Section Cover
    ...
    if STEEL_TYPE == 1:# WITHOUT HARDENING AND ULTIMATE STRAIN
        ops.uniaxialMaterial('Steel01', steelID, fy, Es, 0.0) 
        ops.uniaxialMaterial('Steel01', steelPlateID, fyP, EsP, 0.0) 
    if STEEL_TYPE == 2:# WITH HARDENING AND ULTIMATE STRAIN    
        pinchX = 0.8   # Pinching factor in X direction
        pinchY = 0.5   # Pinching factor in Y direction
        damage1 = 0.0  # Damage due to ductility
        damage2 = 0.0  # Damage due to energy
        beta = 0.1 # Stiffness degradation parameter
        ops.uniaxialMaterial('Hysteretic', steelID, fy, ey, fu, esu, 0.2*fu, 1.1*esu, -fy, -ey, -fu, -esu, -0.2*fu, -1.1*esu, pinchX, pinchY, damage1, damage2, beta)
        
        ops.uniaxialMaterial('Hysteretic', steelPlateID , fyP, eyP, fuP, esuP, 0.2*fuP, 1.1*esuP, -fyP, -eyP, -fuP, -esuP, -0.2*fuP, -1.1*esuP, pinchX, pinchY, damage1, damage2, beta)
        # INFO LINK: https://opensees.berkeley.edu/wiki/index.php/Hysteretic_Material

    if CONCRETE_TYPE == 1:
        ops.uniaxialMaterial('Concrete01', coreID, fcC, ec0C, fcUC, ecuC)  # Core concrete (confined)
        ops.uniaxialMaterial('Concrete01', coverID, fcU, ec0U, fcUU, ecuU) # Cover concrete (unconfined)
    if CONCRETE_TYPE == 2:
        ops.uniaxialMaterial('Concrete02', coreID, fcC, ec0C, fcUC, ecuC, lamdaC, ftC, EtsC)  # Core concrete (confined)
        ops.uniaxialMaterial('Concrete02', coverID, fcU, ec0U, fcUU, ecuU, lamdaU, ftU, EtsU) # Cover concrete (unconfined)
    ...
    if PLOT:
        ...
        ax.scatter(z_bot, [-coreY] * numBarsBot, s=50, c='purple', marker='o', label='Bottom Steel')

        # Intermediate bars (sides)
        y_int = np.linspace(-coreY + DD, coreY - DD, numBarsIntTot // 2)
        ax.scatter([coreZ] * len(y_int), y_int, s=30, c='blue', marker='o', label='Skin Steel')
        ax.scatter([-coreZ] * len(y_int), y_int, s=30, c='blue', marker='o')
