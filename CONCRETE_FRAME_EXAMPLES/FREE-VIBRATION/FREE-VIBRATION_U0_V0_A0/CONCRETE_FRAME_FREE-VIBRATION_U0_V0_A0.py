...
FORCE_S, FORCE_A, MOMENT, DISP_X, DISP_Y, ROT, KA, KS, KI, time, velocity_X, velocity_Y, acceleration_X, acceleration_Y, PERIOD_01, PERIOD_02, DRIFT_X, DRIFT_Y, damping_ratio = DATA

# WITH HARDENING AND ULTIMATE STRAIN
DATA02 = FV_ANALYSIS(IA, IU, IV, u0, v0, a0, STEEL_KIND=2)
FORCE_S02, FORCE_A02, MOMENT02, DISP_X02, DISP_Y02, ROT02, KA02, KS02, KI02, time02, velocity_X02, velocity_Y02, acceleration_X02, acceleration_Y02, PERIOD_012, PERIOD_022, DRIFT_X02, DRIFT_Y02, damping_ratio02 = DATA02
...
plt.plot(time, DRIFT_X, color='red', linewidth=2)
plt.plot(time, DRIFT_Y, color='blue', linewidth=2)
plt.title('Structure Lateral Drift During the Dynamic Analysis')
plt.ylabel('Lateral Drift [%]')
plt.xlabel('Times')
plt.legend([f'IN X DIR.: {np.max(np.abs(DRIFT_X)):.3e}', f'IN Y DIR.: {np.max(np.abs(DRIFT_Y)):.3e}'])
plt.grid()
plt.show()
#%%------------------------------------------------------------------------------  
...
plt.plot(time, BASE_ZY, color='red', linewidth=2)
plt.xlabel('Time [s]')
plt.ylabel('Axial Base-reaction [N]')
