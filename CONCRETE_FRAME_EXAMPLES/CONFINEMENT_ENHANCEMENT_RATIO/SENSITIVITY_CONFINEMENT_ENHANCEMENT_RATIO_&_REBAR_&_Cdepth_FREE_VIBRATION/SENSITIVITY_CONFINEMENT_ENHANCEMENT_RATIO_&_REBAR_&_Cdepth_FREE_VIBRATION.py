...
#   SENSITIVITY ANALYSIS OF A CONCRETE FRAME BY VARYING COLUMN REBAR DIAMETER, SECTION DEPTH, AND CONFINEMENT  #
#                                             ENHANCEMENT RATIO USING OPENSEES                                 #
#--------------------------------------------------------------------------------------------------------------#
...
for hc in HCC:
    for dia in DIAc:
        for kf in Kfc:
            ...
            DIAc_MAX.append(dia)
            Kfc_MAX.append(kf)
            DATA = PD_ANALYSIS(hc, dia, kf, u0, v0, a0, IA, IU, IV, STEEL_KIND=2, ANA_KIND='PUSHOVER')
            (RO_COL, RO_BE, FORCE_S, FORCE_A, MOMENT, DISP_X, DISP_Y,
             ROT, KA, KS, KI, Elastic_ST, Plastic_ST,
             Tangent_ST, Ductility_Rito, Over_Strength_Factor, R, DRIFT, STEP) = DATA
            ...
            DATA = PD_ANALYSIS(hc, dia, kf, u0, v0, a0, IA, IU, IV, STEEL_KIND=2, ANA_KIND='DYNAMIC')
            (_, _, FORCE_Sd, FORCE_Ad, MOMENTd, DISP_Xd, DISP_Yd, ROTd,
             KAd, KSd, KId, timed, velocity_Xd, velocity_Yd, acceleration_Xd,
             acceleration_Yd, PERIOD_01, PERIOD_02, DRIFT, damping_ratio) = DATA
            ...
            DRIFT_MAX_d.append(np.max(np.abs(DRIFT)))
