...
#              CONFINEMENT ENHANCEMENT RATIO OF CONCRETE FRAME ELEMENTS LIKE BEAM AND COLUMNS USING OPENSEES         #
#--------------------------------------------------------------------------------------------------------------------#
...
FORCE_Sd, FORCE_Ad, MOMENTd, DISP_Xd, DISP_Yd, ROTd, KAd, KSd, KId, timed, velocity_Xd, velocity_Yd, acceleration_Xd, acceleration_Yd, PERIOD_01d, PERIOD_02d, DRIFT_Xd, DRIFT_Yd, damping_ratio, PERIOD_MIN, PERIOD_MAX = DATA
...
plt.plot(timed, PERIOD_MIN)
plt.plot(timed, PERIOD_MAX)
plt.title('Period of Structure')
plt.ylabel('Structural Period [s]')
plt.xlabel('Time [s]')
#plt.semilogy()
plt.grid()
plt.legend([f'MIN PERIOD VALUES: Min: {np.min(PERIOD_MIN):.3f} (s) - Mean: {np.mean(PERIOD_MIN):.3f} (s) - Max: {np.max(PERIOD_MIN):.3f} (s)', 
            f'MAX PERIOD VALUES: Min: {np.min(PERIOD_MAX):.3f} (s) - Mean: {np.mean(PERIOD_MAX):.3f} (s) - Max: {np.max(PERIOD_MAX):.3f} (s)',
            ])
plt.show()
...
results_df.to_excel('CONCRETE_FRAME_DUCTILITY_DAMAGE_INDEX_CONFINEMENT_ENHANCEMENT_RATIO_RESULTS.xlsx', index=False) 
#%%------------------------------------------------------------------------------
...
# Print the Model
#printModel()
ops.printModel("-JSON", "-file", "CONCRETE_FRAME_DUCTILITY_DAMAGE_INDEX_CONFINEMENT_ENHANCEMENT_RATIO.json")
#%%-------------------------------------------------------------------------------
