...
def PD_ANALYSIS(STEEL_KIND, ANA_KIND):
    ...
    """
    # LEFT COLUMN NODES
    ops.node(5, 0.0, 0.25*LENGTH_COL)
    ops.node(6, 0.0, 0.50*LENGTH_COL)
    ops.node(7, 0.0, 0.75*LENGTH_COL)
    
    # RIGHT COLUMN NODES
    ops.node(8, LENGTH_BM, 0.25*LENGTH_COL)
    ops.node(9, LENGTH_BM, 0.50*LENGTH_COL)
    ops.node(10, LENGTH_BM, 0.75*LENGTH_COL)
    """
    # BEAM
...
#%%----------------------------------------------------------------------------------------------
    ...
    if ANA_KIND == 'DYNAMIC':
        ...
        ops.printModel() # Print the Analysis Data
#%%------------------------------------------------------------------------------
# LOW-PERFORMANCE MODE (WITH VISUAL OUTPUT)
"""
...
def RUN_ANALYSIS():
    with concurrent.futures.ProcessPoolExecutor() as executor:
        _ = executor.submit(PD_ANALYSIS(STEEL_KIND=2, ANA_KIND='PUSHOVER'))
        _ = executor.submit(PD_ANALYSIS(STEEL_KIND=2, ANA_KIND='DYNAMIC'))
...
"""
#%%------------------------------------------------------------------------------
# HIGH-PERFORMANCE MODE (NO VISUAL OUTPUT)
import concurrent.futures
from multiprocessing import freeze_support
import time as TI  # Import time module with alias

def RUN_ANALYSIS():
    # Executes static and dynamic analyses in parallel
    with concurrent.futures.ProcessPoolExecutor() as executor:
        # Correct submission: pass function reference + arguments separately
        future_static = executor.submit(PD_ANALYSIS, STEEL_KIND=2, ANA_KIND='PUSHOVER')
        future_dynamic = executor.submit(PD_ANALYSIS, STEEL_KIND=2, ANA_KIND='DYNAMIC')
        
        # Retrieve results (blocks until completion)
        static_result = future_static.result()
        dynamic_result = future_dynamic.result()
    
    return static_result, dynamic_result

if __name__ == '__main__':
    # Record start time
    start_time = TI.strftime("%H:%M:%S", TI.localtime())
    ...
    end_time = TI.strftime("%H:%M:%S", TI.localtime())
    print(f"\nAnalysis completion time: {end_time}")
#%%------------------------------------------------------------------------------
