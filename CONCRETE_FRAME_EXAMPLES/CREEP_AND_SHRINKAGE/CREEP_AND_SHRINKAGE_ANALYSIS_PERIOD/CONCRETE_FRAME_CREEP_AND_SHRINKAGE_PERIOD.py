...
#     CREEP AND SHRINKAGE ANALYSIS OF CONCRETE FRAME AND EVALUATION STRUCURAL PERIOD USING OPENSEES            #
#--------------------------------------------------------------------------------------------------------------#
...
import CONCRETE_SECTION_CREEP_AND_SHRINKAGE_FUN as S03
...
MASS = 10000.0           # [kg] Mass on the Top of Column
 #%%------------------------------------------------------------------------------
...
def CREEP_ANALYSIS(LENGTH_COL, LENGTH_BM, DMAX, DINCR, STEEL_KIND):
    ...
    # COLUMN SECTION
    Kfc = 1.2
    Tcr = S03.CONFINED_CONCRETE_CREEP_AND_SHRINKAGE_SECTION(secTagC, Hc, Bc, coverC, AsC, fc, Kfc, STEEL_KIND, COL=True)
    # BEAM SECTION
    Kfc = 1.1
    Tcr = S03.CONFINED_CONCRETE_CREEP_AND_SHRINKAGE_SECTION(secTagB, Hb, Bb, coverB, AsB, fc, Kfc, STEEL_KIND, COL=False)
    """
    DD = 0.5 * Hb
    # Output Data
    ...
    return FORCE_S, FORCE_A, MOMENT, DISP_X, DISP_Y, ROT, KA, KS, KI, STEP, TIME, np.array(PERIOD_MIN), np.array(PERIOD_MAX)
...
FORCE_S, FORCE_A, MOMENT, DISP_X, DISP_Y, ROT, KA, KS, KI, STEP, TIME, _, _ = CREEP_ANALYSIS(LENGTH_COL, LENGTH_BM, TMAX, TINCR, STEEL_KIND=1)
# WITH HARDENING AND ULTIMATE STRAIN
FORCE_S02, FORCE_A02, MOMENT02, DISP_X02, DISP_Y02, ROT02, KA02, KS02, KI02, STEP02, TIME02, period_min, period_max  = CREEP_ANALYSIS(LENGTH_COL, LENGTH_BM, TMAX, TINCR, STEEL_KIND=2)
...
plt.title('Period of Structure vs Displacement During Creep and Shrinkage Analysis')
plt.ylabel('Structural Period [s]')
plt.xlabel('Lateral Displacement in X Dir. [m]')
#plt.semilogy()
plt.grid()
...
results_df.to_excel('CONCRETE_FRAME_CREEP_AND_SHRINKAGE_PERIOD_RESULTS.xlsx', index=False) 
#%%------------------------------------------------------------------------------
...
# Print the Model
#printModel()
ops.printModel("-JSON", "-file", "CONCRETE_FRAME_CREEP_AND_SHRINKAGE_PERIOD.json")
#%%------------------------------------------------------------------------------  
