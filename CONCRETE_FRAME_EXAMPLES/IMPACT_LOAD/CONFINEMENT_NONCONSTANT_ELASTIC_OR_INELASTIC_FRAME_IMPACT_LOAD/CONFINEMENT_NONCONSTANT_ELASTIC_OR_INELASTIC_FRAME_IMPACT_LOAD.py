...
import HARMONIC_IMPACT_LOAD_FUN as S08
...
DT = dt                      # [s] Time step
impact_time = 50.0           # [s] Total Harmonic Load Analysis Durations
omega_n = 5.0715             # [rad/s] Natural angular frequency
force_amplitude = 50000.0*20 # [N] Harmonic Load

load_time, harmonic_load = S08.HARMONIC_IMPACT_LOAD(force_amplitude, impact_time, DT, omega_n, PLOT=True)    
#%%------------------------------------------------------------------------------
...
plt.title(f'Time vs Shear Base-reaction from Elastic Dynamic Analysis - MAX. ABS: {BASE_Ze[-1]}')
plt.grid()
plt.show()
...
plt.title(f'Time vs Axial Base-reaction from Elastic Dynamic Analysis - MAX. ABS: {BASE_ZYe[-1]}')
plt.grid()
plt.show()
...
plt.xlabel('Time [s]')
plt.ylabel('Axial Base-reaction [N]')
plt.title(f'Time vs Axial Base-reaction from Inelastic Dynamic Analysis - MAX. ABS: {BASE_ZYp[-1]}')
plt.grid()
plt.show()
...
results_df.to_excel('CONFINEMENT_NONCONSTANT_ELASTIC_OR_INELASTIC_FRAME_IMPACT_LOAD_RESULTS.xlsx', index=False) 
#%%------------------------------------------------------------------------------
...
# Print the Model
#printModel()
ops.printModel("-JSON", "-file", "CONFINEMENT_NONCONSTANT_ELASTIC_OR_INELASTIC_FRAME_IMPACT_LOAD_FRAME.json")
#%%-------------------------------------------------------------------------------
