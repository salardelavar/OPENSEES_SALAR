...
def SEISMIC_ANALYSIS_IDA(j, J_MAX, LENGTH_COL, LENGTH_BM, STEEL_KIND):
    ...
    while stable == 0 and current_time < duration:
        ...
        if step <= len(gm_accels)-1:               # LATERAL ACCELERATION IN X FOR NODE 3
            acceleration_X.append(ops.nodeAccel(2, 1) + gm_accels[step]) # Structure Acceleration and Seismic Acceleration
        else:
            acceleration_X.append(ops.nodeAccel(2, 1)) # Structure Acceleration
        step += 1 
