...
import EIGENVALUE_ANALYSIS_FUN as S08
...
def PD_ANALYSIS(area, FY, MASS, STEEL_KIND, ANA_KIND, DAMPER_KIND, DR, DRD):
    ...
    if ANA_KIND == 'DYNAMIC':
        ...
        while stable == 0 and current_time < duration:
            ...
            Period_MIN, Period_MAX = S08.EIGENVALUE_ANALYSIS(4, PLOT=True)
...
#plt.semilogy()
plt.grid()
plt.legend([f'MIN VALUES: Min: {np.min(PERIOD_MIN):.3f} (s) - Mean: {np.mean(PERIOD_MIN):.3f} (s) - Max: {np.max(PERIOD_MIN):.3f} (s)', 
            f'MAX VALUES:  Min: {np.min(PERIOD_MAX):.3f} (s) - Mean: {np.mean(PERIOD_MAX):.3f} (s) - Max: {np.max(PERIOD_MAX):.3f} (s)',
            ])
plt.show()
