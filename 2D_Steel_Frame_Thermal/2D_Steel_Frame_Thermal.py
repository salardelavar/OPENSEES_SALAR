...
"""
This code models and analyzes a 2D steel frame subjected to thermal and distributed loads using OpenSees. 
Key points:  
1. Model Definition: The 2D frame has specified node coordinates for stories and bays, with fixed supports at the base. Material properties for steel (with thermal effects) and section geometries (I-section, circular, and rectangular tubes) are defined using fiber elements.  
...
distributed_load = -0.1  # [kN/mm] Distributed load
...
def C_TUBE_SECTION(secTag):
    # Define geometric properties of the steel tube
    #secTag = 1
    D = 0.5  # Outer diameter of the tube (m)
    t = 0.02  # Wall thickness of the tube (m)
    r = D / 2  # Outer radius of the tube (m)
    r_inner = r - t  # Inner radius of the tube (m)
...
def R_TUBE_SECTION(secTag):
    ...
    B = 0.3  # Outer width of the tube (m)
    H = 0.5  # Outer height of the tube (m)
    t = 0.02  # Wall thickness of the tube (m)
    B_inner = B - 2 * t  # Inner width of the tube (m)
    H_inner = H - 2 * t  # Inner height of the tube (m)
...
secTag = 1
I_SECTION(secTag)
#C_TUBE_SECTION(secTag)
#R_TUBE_SECTION(secTag)
#--------------------------------------------------------------------
...
# Plot frame shapes
plot_frame(deformed_scale=1000)  # Adjust scale factor as needed
