...
#               COMPARATIVE ANALYSIS OF AXIAL FORCE-MOMENT (P-M) INTERACTION BEHAVIOR IN                  #
#     FOUR DIFFERENT STEEL SECTIONS : EVALUATING STRAIN HARDENING EFFECTS AND ULTIMATE STRAIN CRITERIA    #
#---------------------------------------------------------------------------------------------------------#
#                          THIS PROGRAM WRITTEN BY SALAR DELAVAR GHASHGHAEI (QASHQAI)                     #
#                                   EMAIL: salar.d.ghashghaei@gmail.com                                   #
###########################################################################################################
"""
Performs a comparative analysis of Steel Sections columns' axial force-moment (P-M)
...
3. Section Modeling: Creates Steel Sections with fiber discretization.
...
# ------------------------------------------
# STEEL SECTION
fy = 240          # [N/mm²] Steel Rebar Yield Strength   
Es = 2e5          # [N/mm²] Modulus of Elasticity
ey = fy/Es        # [mm/mm] Steel Rebar Yield Strain
fu = 1.1818*fy    # [N/mm²] Steel Rebar Ultimate Strength
esu = 0.25        # [mm/mm] Steel Rebar Ultimate Strain
Esh = (fu - fy)/(esu - ey)
Bs = Esh / Es

B = 100.0         # [mm] Section Width
...
#%%------------------------------------------------------------------------------
def P_M_INTERACTION(b, h, Es, fy, ECU, NUM, STEEL_KIND):
    ECU = -ECU
    ...
    # STEEL I SECTION
    #d, MASS = S03.STEEL_I_SECTION_QUAD(secTag, steelTag, PLOT=True, DENSITY=7850/1e9)
    ...
    d, MASS = S03.TWO_UNP_SECTION_WITH_TOP_BOTTOM_PLATES(secTag, steelTag, PLOT=True, DENSITY=7850/1e9, include_plates=True)
    
    # Define element
    ops.element('zeroLengthSection', 1, 1, 2, secTag)
    EY = fy / Es  # Steel yield strain

    ep_list = np.linspace(ECU, 350*EY, NUM)
...
AXIAL, MOMENT, STRAIN, CUR, X, EI = P_M_INTERACTION(B, H, Es, fy, esu, NUM=5000, STEEL_KIND=1)
# WITH HARDENING AND ULTIMATE STRAIN
AXIAL02, MOMENT02, STRAIN02, CUR02, X02, EI02  = P_M_INTERACTION(B, H, Es, fy, esu, NUM=5000, STEEL_KIND=2)
...
results_df.to_excel('P-M_INTERACTION_STEEL_RESULTS.xlsx', index=False) 