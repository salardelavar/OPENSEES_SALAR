...
fy = S01.BETA_PDF(0.39, 0.41, 1, 2, NUM_SIM)    # [N/m^2] Yield strength of steel rebar
fu = 1.5 * fy                                   # [N/m^2] Ultimate strength of steel rebar
Es = S01.BETA_PDF(2.0e2, 2.1e2, 1, 2, NUM_SIM)  # [N/m^2] Spring material Elastic modulus
ey = fy / Es                                    # [m/m] Yield steel strain
esu = S01.BETA_PDF(0.33, 0.36, 1, 2, NUM_SIM)   # [m/m] Ultimate steel strain
Esh = (fu - fy) / (esu - ey)                    # [N/m^2] Strain hardening modulus
...
S01.HISROGRAM_BOXPLOT(Es, HISTO_COLOR='pink', LABEL='Spring material Elastic modulus [N/m]')
...
S01.HISROGRAM_BOXPLOT(fy, HISTO_COLOR='gold', LABEL='Yield strength of steel rebar [N/m^2]')
S01.HISROGRAM_BOXPLOT(esu, HISTO_COLOR='cyan', LABEL='Ultimate steel strain [m/m]')
S01.HISROGRAM_BOXPLOT(b, HISTO_COLOR='blue', LABEL='Strain hardening ratio')
...
def ANALYSIS_SDOF(i):
    ...
    ops.uniaxialMaterial('Steel01', 1, fy[i], Es[i], b[i]) # Steel with bilinear kinematic hardening Material
...
mean_capacity = np.mean(fu * AREA)    # Mean Element Ultimate Capacity
std_dev_capacity = np.std(fu * AREA)  # Std Element Ultimate Capacity
num_sim = NUM_SIM
