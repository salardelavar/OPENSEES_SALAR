...
######################################################################################################################
"""
Performs free-vibration and pushover analysis of a Multi Degree of Freedom (MDOF)
...
DMAX = 0.10             # [mm] Maximum Displacement
DINCR = 0.0001          # [mm] Incremental Displacement
...
plt.show()
#%%-------------------------------------------------------------------------------
def ANALYSIS_MDOF(ANA_KIND, DMAX, DINCR, IA, IU, IV, SPRING_KIND, M, C, K_elastic, KP, KN, u0, v0, a0, duration, dt):
    # Create model
    ops.wipe()
    ...
    if ANA_KIND == 'PUSHOVER':
        ...
        return STEP, reaction, disp, stiffness, np.array(PERIOD_MIN), np.array(PERIOD_MAX), node_displacements     
...
DATA = ANALYSIS_MDOF(ANA_KIND, DMAX, DINCR, IA, IU, IV, SPRING_KIND, M, C, K_elastic, KP, KN, u0, v0, a0, duration, dt)
...
# Print the Model
#printModel()
ops.printModel("-JSON", "-file", "FREE-VIBRATION_&_PUSHOVER_MDF.json")
#%%-------------------------------------------------------------------------------
# PUSHOVER
SPRING_KIND = 'ELASTIC'
ANA_KIND = 'PUSHOVER'
DATA = ANALYSIS_MDOF(ANA_KIND, DMAX, DINCR, IA, IU, IV, SPRING_KIND, M, C, K_elastic, KP, KN, u0, v0, a0, duration, dt)
step, reactionE, dispE, stiffnessE, period_minE, period_maxE, node_displacementsE = DATA

SPRING_KIND = 'INELASTIC'
ANA_KIND = 'PUSHOVER'
DATA = ANALYSIS_MDOF(ANA_KIND, DMAX, DINCR, IA, IU, IV, SPRING_KIND, M, C, K_elastic, KP, KN, u0, v0, a0, duration, dt)
step, reactionI, dispI, stiffnessI, period_minI, period_maxI, node_displacementsI = DATA

totaltime = TI.process_time() - starttime
print(f'\nTotal Analysis Durations (s): {totaltime:.4f} \n\n')

plt.figure(0, figsize=(12, 8))
plt.plot(dispE, reactionE)
plt.plot(dispI, reactionI)
plt.title('Pushover of Structure')
