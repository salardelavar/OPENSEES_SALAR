...
#   COMPREHENSIVE NONLINEAR SEISMIC ASSESSMENT OF A MULTI-DEGREE-FREEDOM ELEVATOR & STRUCTURE SYSTEM :    #
...
#---------------------------------------------------------------------------------------------------------#
#                              MARKOV CHAIN FOR EVALUTIONS OF PROBABILITY OF FAILURE                      #
#---------------------------------------------------------------------------------------------------------#
#                    THIS PYTHON SCRIPT WRITTEN BY SALAR DELAVAR GHASHGHAEI (QASHQAI)                     #
#                                   EMAIL: salar.d.ghashghaei@gmail.com                                   #
###########################################################################################################
"""
Nonlinear Seismic Performance Assessment of a MDOF Elevator and Structure System:
An OpenSeesPy Framework for Material and Geometric Nonlinearity Under Static, Cyclic, and Earthquake Loading

This OpenSeesPy script performs rigorous nonlinear static and dynamic analysis of a MDOF Elevator and Structure System
...
# WEBSITE:
'https://academy2.youngarchitect.com/types-of-elevators/'
# WEBSITE: Types of Elevators: Hydraulic, Traction, and MRL Explained
'https://en.wikipedia.org/wiki/Elevator'   
# PAPER: Analysis and investigation of the failure elevator induced by dual effects of cyclic alternating load and improper heat treatment
'https://www.sciencedirect.com/science/article/abs/pii/S1350630722005994'
# YOUTUBE: Dynamic Lift Loads Calculations | Advance Structural Design 
'https://www.youtube.com/watch?v=IkZafMXv8-Y'
# YOUTUBE: Static Load on the Elevator Motor | Deep Engineering Analysis of Traction Elevators
'https://www.youtube.com/watch?v=EFcciH3zpLo'
#%%----------------------------------------------------
import numpy as np
...
"""
def ELEVATOR_STRUCTURE_MDOF(MAT_TYPE, TOTAL_MASS, ANAL_TYPE):
    # Initialize model
    ops.wipe()
    ...
    # [kg] Mass
    MASS = [0.95 * TOTAL_MASS, # STRUCTURE
            0.05 * TOTAL_MASS] # ELEVATOR 
    # Damping ratio
    DRi = [0.01, # STRUCTURE
           0.10] # ELEVATOR
    ...
    for II in range(0, 2):
        ...
        if II == 0:
            Ke = 9000000.0                               # [N/m] Spring Elastic Stiffness for Structure
        if II == 1:
            Ke = 1250000.0                               # [N/m] Spring Elastic Stiffness for Elevator       
        ...
        Kd = 1250000.0   # [N/m] ELASTIC STIFFNESS OF DAMPER
        if II == 0:      # OMEGA FOR STRUCTURE
        ...
        if II == 1:      # OMEGA FOR ELEVATOR
            omega = np.sqrt(Kd / MASS[II]) 
            kz = Kd
            Cd = 2 * DRi[II] * omega * MASS[II]  # [N·s/m] Damping coefficient 
            #ops.uniaxialMaterial('ViscousDamper', MatTag_C, Kz Cd, alpha)
            # INFO LINK: https://openseespydoc.readthedocs.io/en/stable/src/ViscousDamper.html
            ops.uniaxialMaterial('BilinearOilDamper', MatTag_C, Kz, Cd)# , Fr=1.0, p=1.0, LGap=0.0, NM=1, RelTol=1e-6, AbsTol=1e-10, MaxHalf=15
    ...
    if ANAL_TYPE == 'STATIC_EXTERNAL_TIME-DEPENDENT_LOADING': # STATIC TIME-HISTORY ANALYSIS
        ...
        W_kg = MASS[1]#2500         # [kg] Total weight (cabin + capacity + counterweight)
        v_mps = 2.0                 # [m/s] Impact velocity
        S_m = 0.45                  # [m] Buffer stroke
        ...
        # Maximum impact force using buffer formula
        R_max = W * (1 + (v_mps**2) / (2 * g * S_m))
        force_amplitude = R_max     # [N] Amplitude Force
        ...
        def EXTERNAL_TIME_DEPENDENT_01(force_amplitude, omega_DT, DT, DT_time):
            # P(t) = P0 sin(wt)
            import numpy as np
            import matplotlib.pyplot as plt
            ...
            plt.figure(figsize=(10, 6))
            plt.plot(load_time, DT_load, label=f'External Loading: P(t) = P0 sin(wt) - Max: {np.max(DT_load):.3f}', linewidth=5)
        ...
        #DT_load = EXTERNAL_TIME_DEPENDENT_01(force_amplitude, omega_DT, DT, DT_time)
        ...
        def EXTERNAL_TIME_DEPENDENT_02(force_amplitude, omega_DT, DT, DT_time):
            ...
            load_time = np.linspace(0, DT_time, num_steps) 
            target_frequency = 1.0 * omega_DT  # Target excitation frequency
            DT_load = force_amplitude * np.exp(-0.05*target_frequency * load_time)
            # Plot External Time-dependent Loading
            plt.figure(figsize=(10, 6))
            plt.plot(load_time, DT_load, label=f'External Loading: P(t) = P0 exp(-0.05wt) - Max: {np.max(DT_load):.3f}', linewidth=5)
            plt.title('External Time-dependent Loading Over Time')
            plt.xlabel('Time (s)')
            plt.ylabel('Force (N)')
            plt.grid(True)
        ...
        def EXTERNAL_TIME_DEPENDENT_03(m, omega_DT, DT, DT_time): # P(t): m . ug**(t)
            # P(t) = P0 exp(-0.05wt) sin(wt)     
            import numpy as np
            import matplotlib.pyplot as plt
            # External Load Durations
            ...
            # Plot External Time-dependent Loading
            plt.figure(figsize=(10, 6))
            plt.plot(load_time, DT_load, label=f'External Loading: P(t) = P0 exp(-0.05wt) sin(wt)  - Max: {np.max(DT_load):.3f}', linewidth=5)
            plt.title('External Time-dependent Loading Over Time')
            plt.xlabel('Time (s)')
            plt.ylabel('Force (N)')
            plt.grid(True)
        ...
        #DT_load03 = EXTERNAL_TIME_DEPENDENT_03(M, omega_DT, DT, DT_time)

        def EXTERNAL_TIME_DEPENDENT_04(m, omega_DT, DT, DT_time): # P(t): m . ug**(t)
            # P(t) = P0 cos(wt)    
            import numpy as np
            import matplotlib.pyplot as plt
            # External Load Durations
            ...
            load_time = np.linspace(0, DT_time, num_steps) 
            target_frequency = 1.0 * omega_DT  # Target excitation frequency
            DT_load = m * np.cos(target_frequency * load_time)
            # Plot External Time-dependent Loading
            plt.figure(figsize=(10, 6))
            plt.plot(load_time, DT_load, label=f'External Loading: P(t) = P0 cos(wt) - Max: {np.max(DT_load):.3f}', linewidth=5)
            plt.title('External Time-dependent Loading Over Time')
            plt.xlabel('Time (s)')
            plt.ylabel('Force (N)')
            plt.grid(True)
        ...
        #DT_load04 = EXTERNAL_TIME_DEPENDENT_04(M, omega_DT, DT, DT_time)

        def EXTERNAL_TIME_DEPENDENT_05(m, omega_DT, DT, DT_time): # P(t): m . ug**(t)
            ...
            load_time = np.linspace(0, DT_time, num_steps) 
            target_frequency = 1.0 * omega_DT  # Target excitation frequency
            DT_load = m * np.exp(-0.05*target_frequency * load_time) * np.cos(target_frequency * load_time)
            # Plot External Time-dependent Loading
            plt.figure(figsize=(10, 6))
            plt.plot(load_time, DT_load, label=f'External Loading: P(t) = P0 exp(-0.05wt) cos(wt)  - Max: {np.max(DT_load):.3f}', linewidth=5)
            plt.title('External Time-dependent Loading Over Time')
            plt.xlabel('Time (s)')
            plt.ylabel('Force (N)')
            plt.grid(True)
        ...
        #DT_load05 = EXTERNAL_TIME_DEPENDENT_04(M, omega_DT, DT, DT_time)

        def EXTERNAL_TIME_DEPENDENT_06(m, omega_DT, DT, DT_time): # P(t): m . ug**(t)
            # P(t) = P0 exp(-0.05wt) sin(wt) cos(wt)   
            import numpy as np
            import matplotlib.pyplot as plt
            # External Load Durations
            num_steps = int(DT_time / DT)
            load_time = np.linspace(0, DT_time, num_steps) 
            target_frequency = 1.0 * omega_DT  # Target excitation frequency
            DT_load = m * np.exp(-0.05*target_frequency * load_time) * np.sin(target_frequency * load_time) * np.cos(target_frequency * load_time)
            # Plot External Time-dependent Loading
            plt.figure(figsize=(10, 6))
            plt.plot(load_time, DT_load, label=f'External Loading: P(t) = P0 exp(-0.05wt) sin(wt) cos(wt) - Max: {np.max(DT_load):.3f}', linewidth=5)
            plt.title('External Time-dependent Loading Over Time')
            plt.xlabel('Time (s)')
            plt.ylabel('Force (N)')
            plt.grid(True)
        ...
        #DT_load06 = EXTERNAL_TIME_DEPENDENT_04(M, omega_DT, DT, DT_time)

        def EXTERNAL_TIME_DEPENDENT_07(m, omega_DT, DT, DT_time): # P(t): m . ug**(t)
            # P(t) = P0 exp(+0.05wt) cos(wt)    
            import numpy as np
            import matplotlib.pyplot as plt
            # External Load Durations
        ...
        #DT_load07 = EXTERNAL_TIME_DEPENDENT_07(M, omega_DT, DT, DT_time)

        def EXTERNAL_TIME_DEPENDENT_08(m, omega_DT, DT, DT_time): # P(t): m . ug**(t)
            # P(t) = P0 exp(+0.05wt) cos(wt)    
            import numpy as np
            import matplotlib.pyplot as plt
            # External Load Durations
            num_steps = int(DT_time / DT)
            load_time = np.linspace(0, DT_time, num_steps) 
            target_frequency_01 = 1.0 * omega_DT  # Target excitation frequency
            target_frequency_02 = 1.0 * omega_DT  # Target excitation frequency
            target_frequency_03 = 1.0 * omega_DT  # Target excitation frequency
            DT_load = m * np.exp(0.05*target_frequency_01 * load_time) * (np.sin(target_frequency_02 * load_time) + np.cos(target_frequency_03 * load_time))
            # Plot External Time-dependent Loading
            plt.figure(figsize=(10, 6))
            plt.plot(load_time, DT_load, label=f'External Loading: P(t) = P0 exp(-0.05wt) (sin(wt) + cos(wt))  - Max: {np.max(DT_load):.3f}', linewidth=5)
            plt.title('External Time-dependent Loading Over Time')
        ...
        #DT_load08 = EXTERNAL_TIME_DEPENDENT_08(M, omega_DT, DT, DT_time)

        #DT_load = EXTERNAL_TIME_DEPENDENT_01(force_amplitude, omega_DT, DT, DT_time)
        #DT_load = EXTERNAL_TIME_DEPENDENT_02(force_amplitude, omega_DT, DT, DT_time)
        DT_load = EXTERNAL_TIME_DEPENDENT_03(force_amplitude, omega_DT, DT, DT_time)
        #DT_load = EXTERNAL_TIME_DEPENDENT_04(force_amplitude, omega_DT, DT, DT_time)
        #DT_load = EXTERNAL_TIME_DEPENDENT_05(force_amplitude, omega_DT, DT, DT_time)
        #DT_load = EXTERNAL_TIME_DEPENDENT_06(force_amplitude, omega_DT, DT, DT_time)
        #DT_load = EXTERNAL_TIME_DEPENDENT_07(force_amplitude, omega_DT, DT, DT_time)
        #DT_load = EXTERNAL_TIME_DEPENDENT_08(force_amplitude, omega_DT, DT, DT_time)
        # Static Time-depenent External loading analysis
        TS_TAG = 3
        PATT_TAG = 3
        # Apply time-dependent explosion loading
        ops.timeSeries('Path', TS_TAG, '-dt', dt, '-values', *DT_load)
...
DATA = ELEVATOR_STRUCTURE_MDOF(MAT_TYPE, TOTAL_MASS, ANAL_TYPE)
...
IMs, XLABEL, SCATTER, SEMILOGY = DI_ETDLD, 'DYN. TIME-HISTORY ANA. DUCTILITY DAMAGE INDEX [%]', True, False
FF.FRAGILITY_CURVE_FUN(IMs, DAMAGE_STATES, XLABEL, SCATTER, SEMILOGY)

IMs, XLABEL, SCATTER, SEMILOGY = DI_FV, 'FREE-VIBRTION ANA. DUCTILITY DAMAGE INDEX [%]', True, False
FF.FRAGILITY_CURVE_FUN(IMs, DAMAGE_STATES, XLABEL, SCATTER, SEMILOGY)
    
IMs, XLABEL, SCATTER, SEMILOGY = DI_SEI, 'SEISMIC ANA. DUCTILITY DAMAGE INDEX [%]', True, False
FF.FRAGILITY_CURVE_FUN(IMs, DAMAGE_STATES, XLABEL, SCATTER, SEMILOGY)
#%%----------------------------------------------------
# EXCEL OUTPUT
import pandas as pd
...
# Save to Excel
with pd.ExcelWriter("MDOF_ELEVATOR_STRUCTURE_8_ANA_OUTPUT.xlsx", engine='openpyxl') as writer:
