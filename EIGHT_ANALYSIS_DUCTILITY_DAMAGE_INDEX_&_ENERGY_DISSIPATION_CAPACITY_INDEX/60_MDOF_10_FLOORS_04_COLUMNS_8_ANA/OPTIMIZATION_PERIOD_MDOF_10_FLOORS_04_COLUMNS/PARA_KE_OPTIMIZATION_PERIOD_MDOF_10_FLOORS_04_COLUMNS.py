...
# DETERMINATION OF EACH COLUMN’S OPTIMUM STRUCTURAL INITIAL ELASTIC STIFFNESS FROM STRUCTURAL PERIOD USING#
...
#%%----------------------------------------------------
def MDOF(Ke, MAT_TYPE, TOTAL_MASS, ANAL_TYPE):
    # Initialize model
    ops.wipe()
...
# ------------------------------------------------------------------------------------------------------
# FIND THE OPTIMUM VALUE (NEWTON-RAPHSON SOLVER FOR OPTIMAL ELASTIC STIFFNESS) WITH PARALLEL COMPUTING
# ------------------------------------------------------------------------------------------------------
...
# Problem constants (global so they are available in worker processes)
TOTAL_MASS = 500000.0 # [kg] Total Mass of Structure
MAT_TYPE = 'ELASTIC'   # 'ELASTIC' or 'INELASTIC'
ESP = 1e-3            # finite difference step
TOLERANCE = 1e-6      # convergence tolerance
ITMAX = 100000        # maximum iterations

# Function that must be at module level for pickling (multiprocessing)
def RUN_ANALYSIS_FUN(X_val):
    
    ANAL_TYPE = 'PERIOD'
    DATA = MDOF(X_val, MAT_TYPE, TOTAL_MASS, ANAL_TYPE)
...
def OPTIMIZE_KE():
    ...
    DEMAND = 0.10     # [s] Target Value -> STRUCTURAL PERIOD
    ...
    with concurrent.futures.ProcessPoolExecutor() as executor:
        while RESIDUAL > TOLERANCE and IT < ITMAX:
            ...
            futures = [executor.submit(RUN_ANALYSIS_FUN, param) for param in params]
            ...
            print(f'IT: {IT} - RESIDUAL: {RESIDUAL:.6e} - INTIAL ELASTIC STIFFNESS: {X:.6e}')
