...
def HISROGRAM_BOXPLOT(X, HISTO_COLOR, LABEL):
    ...
    plt.title(f"Histogram - Probability of Positive {LABEL} is {100*prob:.3e} %")
    plt.legend()
    #plt.grid()
    plt.show()
...
"""
def PREDICT_LSTM(x, y, Demand_X, look_back, ITERATION, XLABEL, YLABEL):
    import numpy as np
    ...
    next_x = Demand_X #np.array(x[-look_back:]).reshape(1, look_back, 1)
    ...
    plt.scatter(next_x, predicted_y, color='red', marker='o', label='Predicted next DDI')
...
def MULTIPLE_REGRESSION(df):
    ...
    X = sm.add_constant(df[['Max_velocity',
                            'Max_acceleration',
                            'Max_Base_Reaction',
                            'Energy_Dissipation_Capacity_Index',
                            'Equivalent_viscous_damping_ratio']])
...
def RANDOM_FOREST(df):
    ...
    target_threshold = np.median(df['Max_displacement'])  # Example threshold
    df['Safety_label'] = (df['Max_displacement'] <= target_threshold).astype(int)
...
def PLOT_SCATTER(X, Y , XLABEL, YLABEL, TITLE, COLOR, LOG, ORDER):
    ...
    if ORDER == 1:
        plt.plot(X, y, color='black', label=f'y = {a:.3e}x + {b:.3e} - R^2 = {R_squared:.3f}')
    if ORDER == 2:
        plt.plot(X, y, color='black', label=f'y = {a:.3e}x^2 + {b:.3e}x + {c:.3e} - R^2 = {R_squared:.3f}')
    if ORDER == 3:
        plt.plot(X, y, color='black', label=f'y = {a:.3e}x^3 + {b:.3e}x^2 + {c:.3e}x + {d:.3e} - R^2 = {R_squared:.3f}')  
    if ORDER == 4:
        plt.plot(X, y, color='black', label=f'y = {a:.3e}x^4 + {b:.3e}x^3 + {c:.3e}x^2 + {d:.3e}x + {e:.3e} - R^2 = {R_squared:.3f}') 
    if ORDER == 5:
        plt.plot(X, y, color='black', label=f'y = {a:.3e}x^5 + {b:.3e}x^4 + {c:.3e}x^3 + {d:.3e}x^2 + {e:.3e}x + {f:.3e} - R^2 = {R_squared:.3f}')  
    if ORDER == 6:
        plt.plot(X, y, color='black', label=f'y = {a:.3e}x^6 + {b:.3e}x^5 + {c:.3e}x^4 + {d:.3e}x^3 + {e:.3e}x^2 + {f:.3e}x + {I:.3e} - R^2 = {R_squared:.3f}')  
    if ORDER == 7:
        plt.plot(X, y, color='black', label=f'y = {a:.3e}x^7 + {b:.3e}x^6 + {c:.3e}x^5 + {d:.3e}x^4 + {e:.3e}x^3 + {f:.3e}x^2 + {I:.3e}x + {J:.3e} - R^2 = {R_squared:.3f}')               
...
def NEURAL_NETWORK_FAILURE_PROBABILIYY_ESTIMATION(CAPACITY, LOAD, num_samples):
    import numpy as np
    ...
    # Combine data
    data = np.column_stack((CAPACITY, LOAD))
    ...
    model = Sequential([
        Dense(16, activation='relu', input_shape=(2,)),  # Input layer with 2 neurons (CAPACITY, LOAD)
    ...
    CAPACITY_mc = np.random.normal(10, 2, num_mc_samples)
    LOAD_mc = np.random.normal(5, 1, num_mc_samples)
    data_mc = np.column_stack((CAPACITY_mc, LOAD_mc))
