...
#  DETERMINATION OF THE OPTIMUM ELASTIC STIFFNESS OF EACH COLUMN BASED ON THE CONSTANT STRUCTURAL PERIOD  #
#               THROUGH MASS VARIATION USING FINITE-DIFFERENCE NEWTON ITERATION AND OPENSEES              #
#---------------------------------------------------------------------------------------------------------#
#            IN THIS ANALYSIS MASS IS NOT CONSTANT BUT DEMAND VALUE (STRUCTURAL PERIOD) IS CONSTANT       #
#---------------------------------------------------------------------------------------------------------#
...
--------------------------------------------------------------------------------
    ...
    repeatedly calls the nonlinear MDOF solver with ANAL_TYPE = 'PERIOD'
...
--------------------------------------------------------------------------------
    ...
    Target demand:
        DEMAND = 0.50   [in the chosen period units]

    Residual function:
        F(X) = PERIOD_MAX - DEMAND

        where PERIOD_MAX is returned by the MDOF solver when called with
    ...
    Each evaluation of F requires a full nonlinear analysis via the MDOF
    ...
    equivalent to three full MDOF period analyses.

3. WORKFLOW SUMMARY
--------------------------------------------------------------------------------
    Step 1 : Initialize X (initial stiffness guess) and solver parameters.
    Step 2 : Evaluate F(X)         via MDOF(ANAL_TYPE='PERIOD').
    Step 3 : Evaluate F(X - ESP)   via MDOF(ANAL_TYPE='PERIOD').
    Step 4 : Evaluate F(X + ESP)   via MDOF(ANAL_TYPE='PERIOD').
...
--------------------------------------------------------------------------------
    ...
    ANAL_TYPE : Analysis type passed to MDOF               ('PERIOD')
...
--------------------------------------------------------------------------------
    ...
    - Because the MDOF solver may involve non-smooth hysteretic behaviour,
...
#%%----------------------------------------------------
def MDOF(Ke, MAT_TYPE, TOTAL_MASS, ANAL_TYPE, i):
    # Initialize model
    ops.wipe()
...
# -----------------------------------------------------------------------------
# FIND THE OPTIMUM VALUE (NEWTON-RAPHSON SOLVER FOR OPTIMAL ELASTIC STIFFNESS)
# -----------------------------------------------------------------------------
# Determines the optimum initial elastic stiffness of an MDOF system via a
# Newton-Raphson root-finding scheme, such that the maximum structural period
# matches a prescribed target demand.

# NOTICE: IN THIS ANALYSIS MASS IS NOT CONSTANT BUT DEMAND VALUE (STRUCTURAL PERIOD) IS CONSTANT

import time as TI
import numpy as np
 
X = 4500000.0     # Intial Guess for X -> [N/m] Initial Elastic Stiffness
...
DEMAND = 0.50     # [s] Target Value -> STRUCTURAL PERIOD
...
for II in range(0, 40):
    RESIDUAL = 100                      # REFRESH ANALYSIS fOR RUNNING AGAIN
    TOTAL_MASS = 50_000.0 * (II + 1)   # [kg] Total Mass of Structure
    
    while (RESIDUAL > TOLERANCE):
        # X -------------------    
        ANAL_TYPE = 'PERIOD'
        DATA = MDOF(X, MAT_TYPE, TOTAL_MASS, ANAL_TYPE, 0)
        (PERIOD_MIN, PERIOD_MAX) = DATA
        ...
        ANAL_TYPE = 'PERIOD'
        DATA = MDOF(Xmin, MAT_TYPE, TOTAL_MASS, ANAL_TYPE, 0)
        (PERIOD_MIN, PERIOD_MAX) = DATA
        
        SUPPLYmin = PERIOD_MAX
        ...
        ANAL_TYPE = 'PERIOD'
        DATA = MDOF(Xmax, MAT_TYPE, TOTAL_MASS, ANAL_TYPE, 0)
        (PERIOD_MIN, PERIOD_MAX) = DATA
        
        SUPPLYmax = PERIOD_MAX
        ...
        if RESIDUAL < TOLERANCE:
            print(f'\t\t Structural Mass:                       {TOTAL_MASS:.4f}')
            print(f'\t\t Optimum X (COL. ELASTIC STIFFNESS):    {X:.4f}')
            print(f'\t\t Iteration Counts:                      {IT}')
            print(f'\t\t Convergence Residual:                  {RESIDUAL:.10e}')
        #print(X)
        
    OPT_COL_KE_target.append(X)
    TOTAL_MASS_target.append(TOTAL_MASS)     

totaltime = TI.process_time() - starttime
print(f'\nTotal time (s): {totaltime:.4f} \n\n')


XDATA = OPT_COL_KE_target
YDATA = TOTAL_MASS_target
XLABEL = 'OPTIMUM ELASTIC COL. STIFFNESS [N/m]'
YLABEL = 'STRUCTURAL TOTAL MASS [kg]'
TITLE = f'{YLABEL} and {XLABEL} DURING PERIOD ANALYSIS WITH 20 GROUND MOTIONS \n TARGETED STRUCTURAL PERIOD: {DEMAND:.3f} [s]'
COLOR = 'purple'
...
DATA = MDOF(Ke, MAT_TYPE, TOTAL_MASS, ANAL_TYPE)
