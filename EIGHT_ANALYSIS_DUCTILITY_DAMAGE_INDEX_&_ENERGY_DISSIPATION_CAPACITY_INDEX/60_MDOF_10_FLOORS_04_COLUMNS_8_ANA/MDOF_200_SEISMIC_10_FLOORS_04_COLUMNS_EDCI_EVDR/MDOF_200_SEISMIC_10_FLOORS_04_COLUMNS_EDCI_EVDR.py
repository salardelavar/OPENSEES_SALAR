...
#---------------------------------------------------------------------------------------------------------#
# This program performs  Dynamic Analysis on a Multi-Degree-of-Freedom (MDOF) system                      #
# subjected to 200 seismic ground motions. The analysis evaluates the structural response under varying   #
...
#---------------------------------------------------------------------------------------------------------#
# Key Features:                                                                                           #
# - Simulation of MDOF system using OpenSees.                                                             #
...
NUM_SEISMIC = 200                                # Total number for seismic simulation
...
#%%------------------------------------------------------
### OPENSEES FUNCTION
def MDOF(MAT_TYPE, TOTAL_MASS, ANAL_TYPE, i):
    # Initialize model
    ops.wipe()
...
data = MDOF(MAT_TYPE, TOTAL_MASS, ANAL_TYPE, 0)
...
for i in range(NUM_SEISMIC):
    ...
    data = MDOF(MAT_TYPE, TOTAL_MASS, ANAL_TYPE, i)
    ...
    #%% START - Equivalent viscous damping ratio
    ...
    #%% END - Equivalent viscous damping ratio
...
with pd.ExcelWriter("INELASTIC_SEISMIC_SDOF_RESULTS.xlsx", engine="openpyxl") as writer:
...
#%%------------------------------------------------------
# 3D PLOT
X, Y, Z = max_displacement, max_velocity, max_acceleration
XLABEL, YLABEL, ZLABEL = 'Max. Displacement [m]', 'Max. Velocity [m/s]', 'Max. Acceleration [m/s^2]'               
S10.PLOT_CONTOUR_3D_2D_FUN(125, X, Y, Z, XLABEL, YLABEL, ZLABEL)

X, Y, Z = max_displacement, max_edci , max_zeta
XLABEL, YLABEL, ZLABEL = 'Max. Displacement [m]', 'Energy Dissipation Capacity Index [%]', 'Equivalent viscous damping ratio [%]'               
S10.PLOT_CONTOUR_3D_2D_FUN(126, X, Y, Z, XLABEL, YLABEL, ZLABEL)

X, Y, Z = max_velocity, max_edci , max_zeta
XLABEL, YLABEL, ZLABEL = 'Max. Velocity [m/s]', 'Energy Dissipation Capacity Index [%]', 'Equivalent viscous damping ratio [%]'               
S10.PLOT_CONTOUR_3D_2D_FUN(127, X, Y, Z, XLABEL, YLABEL, ZLABEL)

X, Y, Z = max_acceleration, max_edci , max_zeta
XLABEL, YLABEL, ZLABEL = 'Max. Acceleration [m/s^2]', 'Energy Dissipation Capacity Index [%]', 'Equivalent viscous damping ratio [%]'               
S10.PLOT_CONTOUR_3D_2D_FUN(128, X, Y, Z, XLABEL, YLABEL, ZLABEL)

X, Y, Z = max_DI, max_edci , max_zeta
XLABEL, YLABEL, ZLABEL = 'Structural Damage Index [%]', 'Energy Dissipation Capacity Index [%]', 'Equivalent viscous damping ratio [%]'               
...
damage_states = {
'DS1_Slight': (0.15, 0.05),    # Median PGA=0.15g, β=0.05
...
results_df.to_excel('MDOF_SEISMIC_10_FLOORS_04_COLUMNS_EDCI_EVDR_DATA_RESULTS.xlsx', index=False)
#%%------------------------------------------------------  
XLABEL = 'Displacement'
...
X = np.column_stack([max_displacement, max_velocity, max_acceleration])          
Y = np.column_stack([max_zeta, max_edci, max_DI])  
X_LABELS = ['Max. Displacement [m]', 'Max. Velocity [m/s]', 'Max. Acceleration [m/s^2]']
Y_LABELS = ['Equivalent Viscous Damping Ratio [%]', 'Energy Dissipation Capacity Index [%]', 'Ductility Damage Index [%]']
...
param_list = ["DISP", "VEL", "ACC", "EDCI", "DI"]
