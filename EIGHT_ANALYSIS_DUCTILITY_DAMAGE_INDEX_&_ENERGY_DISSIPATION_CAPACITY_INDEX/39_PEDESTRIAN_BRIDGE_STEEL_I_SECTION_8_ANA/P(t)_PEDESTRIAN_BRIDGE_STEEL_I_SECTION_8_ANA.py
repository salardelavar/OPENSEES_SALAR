...
#           COMPREHENSIVE NONLINEAR SEISMIC ASSESSMENT OF A PEDESTRIAN STEEL I SECTION BRIDGE             #
...
###########################################################################################################
"""
Nonlinear Seismic Performance Assessment of a pedestrain bridge steel I section:
An OpenSeesPy Framework for Material and Geometric Nonlinearity Under Static, Cyclic, and Earthquake Loading

This OpenSeesPy script performs rigorous nonlinear static and dynamic analysis of a pedestrain bridge steel I section
...
'https://www.facebook.com/groups/derbyshireandpeakdistrictwalks/posts/1877350526163696/'
'https://twodogsandanawning.co.uk/ice-cream-for-lynnie29th-july-2014/'      
...
import PEDESTRIAN_BRIDGE_STEEL_SECTION_DIFF_WIDTH_HEIGHT_FUN_EXTRA as S11
import EQULIVALENT_VISCOUS_DAMPING_RATIO_FUN as S055
#%%----------------------------------------------------
def PEDESTRIAN_STEEL_ARC_BRIDGE(LENGTH, MAT_TYPE, ELE_TYPE, ANAL_TYPE, TOTAL_MASS):
    # Initialize model
    ops.wipe()
    ...
    ε = 0.03 * LENGTH                # [mm] Bridge deck camber (Imperfection amplitude)
    ...
    SEC_TAG = 1
    Depth, Ele_Mass = S11.PEDESTRIAN_BRIDGE_STEEL_SECTION_DIFF_WIDTH_HEIGHT_FUN_EXTRA(SEC_TAG, STEEL_TYPE, STEEL_DENSITY, plot=True)
    """
    HSec, BSec, cover = 500, 500, 50
    Depth, Ele_Mass = S12.COMPOSITE_REC_CROSS_SHAPED_STEEL_SECTION_FUN_EXTRA(SEC_TAG, HSec, BSec, cover,
                                  STEEL_TYPE, fc, Kc,
                                  STEEL_DENSITY,
                                  CONCRETE_DENSITY, PLOT)
    """
    #Depth, Ele_Mass = S13.COMPOSITE_SECTION_DIFF_WIDTH_HEIGHT_FUN_EXTRA(SEC_TAG, STEEL_TYPE, fc, Kc, CONCRETE_DENSITY, plot=True)
    #Depth, Ele_Mass = S14.RETROFITTED_COMPOSITE_SECTION_DIFF_WIDTH_HEIGHT_FUN_EXTRA(SEC_TAG, STEEL_TYPE, fc, Kc, CONCRETE_DENSITY, plot=True)
    #Depth, Ele_Mass = S16.STEEL_BOX_SECTION_DIFF_WIDTH_HEIGHT_FUN_EXTRA(SEC_TAG, STEEL_TYPE,STEEL_DENSITY, plot=True)
    #Depth, Ele_Mass = S17.STEEL_CORNER_BOX_SECTION_DIFF_WIDTH_HEIGHT_FUN_EXTRA(SEC_TAG, STEEL_TYPE, STEEL_DENSITY, plot=True)
    #Depth, Ele_Mass = S18.STEEL_UNP_BOX_SECTION_DIFF_WIDTH_HEIGHT_FUN_EXTRA(SEC_TAG, STEEL_TYPE, STEEL_DENSITY, plot=True)
    #Depth, Ele_Mass = S19.I_STEEL_PLATE_SECTION_DIFF_WIDTH_HEIGHT_FUN_EXTRA(SEC_TAG, STEEL_TYPE, STEEL_DENSITY, plot=True)    
    #%% CREATE ELEMENTS
    ele_tag = 1
    transfTag = 1
    ...
    if ANAL_TYPE == 'PUSHOVER': # STATIC TIME-HISTORY ANALYSIS
        ...
        DINCR = -0.1    # [mm] Incremental Vertical Displacement
        DMAX = -200.0   # [mm] Max. Vertical Displacement
...
def PLOT_2D(X, Y, Xfit, Yfit, X2, Y2, XLABEL, YLABEL, TITLE, LEGEND01, LEGEND02, LEGEND03, COLOR, Z):
    ...
    if Z == 3:
        ...
        plt.show() 
#%%----------------------------------------------------
LENGTH = 15000.0      # [mm] Bridge Horizental Length
...
DATA = PEDESTRIAN_STEEL_ARC_BRIDGE(LENGTH, MAT_TYPE, ELE_TYPE, ANAL_TYPE, TOTAL_MASS)
...
# Save to Excel
with pd.ExcelWriter("P(t)_PEDESTRIAN_BRIDGE_STEEL_I_SECTION_8_ANA_OUTPUT.xlsx", engine='openpyxl') as writer:
