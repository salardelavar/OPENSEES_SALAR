...
#           COMPREHENSIVE NONLINEAR SEISMIC ASSESSMENT OF STEEL A WATER TOWER : AN OPENSEES               #
...
###########################################################################################################
"""
Nonlinear Seismic Performance Assessment of a Water Tower:
An OpenSeesPy Framework for Material and Geometric Nonlinearity Under Static, Cyclic, and Earthquake Loading

This OpenSeesPy script performs rigorous nonlinear static and dynamic analysis of a Water Tower
...
import CIRCULALR_HOLLOW_PIPE_STEEL_SECTION_FUN as S08
...
#%%----------------------------------------------------
def WATER_TOWER(LENGTH, MAT_TYPE, ELE_TYPE, ANAL_TYPE, TOTAL_MASS):
    # Initialize model
    ops.wipe()
    ...
    #%% SECTION
    SEC_TAG = 1
    Do = 1200.0                   # [mm] Outer diameter
    t = 30.0                      # [mm] Pipe thickness
    ...
    Depth, Ele_Mass = S08.PIPE_HOLLOW_STEEL_SECTION_FUN(SEC_TAG, MAT_TAG, Do, t, numSubdivCirc, numSubdivRad, DENSITY_STEEL, plot=True)
    #%% CREATE ELEMENTS
    ele_tag = 1
    transfTag = 1
    ...
    if ANAL_TYPE == 'PUSHOVER': # STATIC TIME-HISTORY ANALYSIS
        ...
        DMAX = -800.0   # [mm] Max. Displacement
...
def PLOT_2D(X, Y, Xfit, Yfit, X2, Y2, XLABEL, YLABEL, TITLE, LEGEND01, LEGEND02, LEGEND03, COLOR, Z):
    ...
    if Z == 3:
        ...
        plt.show() 
#%%----------------------------------------------------
LENGTH = 15000.0      # [mm] Tower Length
...
DATA = WATER_TOWER(LENGTH, MAT_TYPE, ELE_TYPE, ANAL_TYPE, TOTAL_MASS)
...
# Save to Excel
with pd.ExcelWriter("W(x,t)_WATER_TOWER_8_ANA_OUTPUT.xlsx", engine='openpyxl') as writer:
