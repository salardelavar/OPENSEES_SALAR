...
#           FINITE‑DIFFERENCE NEWTON ITERATION AND OPENSEES VIA 20 SEISMIC GROUND MOTIONS                 #
#---------------------------------------------------------------------------------------------------------#
...
def SDOF(DUCT, MAT_TYPE, TOTAL_MASS, ANAL_TYPE, i):
    ...
    if ANAL_TYPE == 'SEISMIC': # DYNAMIC TIME-HISTORY ANALYSIS
        ...
        duration = 20.0                    # [s] Analysis duration
        dt = 0.01                          # [s] Time step
        GMfact = 9.81                      # [m/s²]standard acceleration of gravity or standard acceleration
...
def PLOT_2D(X, Y, Xfit, Yfit, X2, Y2, XLABEL, YLABEL, TITLE, LEGEND01, LEGEND02, LEGEND03, COLOR, Z):
    ...
    if Z == 3:
        ...
        plt.show() 
#%%----------------------------------------------------
TOTAL_MASS = 500_000.0   # [kg] Total Mass of Structure
#%%----------------------------------------------------
MAT_TYPE = 'INELASTIC'   # 'ELASTIC' OR 'INELASTIC'

# --------------------------------------------------------------------------------------
# FIND THE OPTIMUM VALUE (NEWTON-RAPHSON SOLVER FOR OPTIMAL STRUCTURAL DUCTILITY RATIO
# --------------------------------------------------------------------------------------
...
X = 5.0          # Intial Guess for X -> [m/m] Each Column Ductility Ratio
...
DI_RATIO = []            # COLLECT DUCTILITY DAMAGE INDEX RATIO (SEISMIC DIVDED BY CYCLIC DISPLACEMENT)
REACTION_RATIO = []      # COLLECT REACTION RATIO (SEISMIC DIVDED BY CYCLIC DISPLACEMENT)
DISP_RATIO = []          # COLLECT DISPLACEMENT RATIO (SEISMIC DIVDED BY CYCLIC DISPLACEMENT)
EDVR_RATIO = []          # COLLECT EQULIVALENT VISCOUS DAMPING RATIO (SEISMIC DIVDED BY CYCLIC DISPLACEMENT)
DISP, VELO, ACC = [], [], []
...
for II in range(40, 60):
    ...
    while (RESIDUAL > TOLERANCE):
        ...
        DATA = SDOF(X, MAT_TYPE, TOTAL_MASS, ANAL_TYPE, II)
        ...
        ANAL_TYPE = 'CYCLIC_DISPLACEMENT'
        DATA = SDOF(Xmin, MAT_TYPE, TOTAL_MASS, ANAL_TYPE, II)
        (reaction_CP, disp_CP, DI_CP,
         PERIOD_MIN_CP, PERIOD_MAX_CP) = DATA
        ...
        ANAL_TYPE = 'CYCLIC_DISPLACEMENT'
        DATA = SDOF(Xmax, MAT_TYPE, TOTAL_MASS, ANAL_TYPE, II)
        (reaction_CP, disp_CP, DI_CP,
         PERIOD_MIN_CP, PERIOD_MAX_CP) = DATA
        ...
        if RESIDUAL < TOLERANCE:
            print(f'\t\t Seismic:                               {II+1}')
            print(f'\t\t Optimum X (STRU. DUCTILITY RATIO):     {X:.4f}')
            print(f'\t\t Iteration Counts:                      {IT}')
            print(f'\t\t Convergence Residual:                  {RESIDUAL:.10e}')
        #print(X)
        
    DISP.append(max(disp_SEI)); VELO.append(max(velo_SEI)); ACC.append(max(acc_SEI))
    OPT_DUCT_target.append(X)
    PERIOD_RATIO.append(max(PERIOD_MAX_SEI)/max(PERIOD_MAX_CP))
    DI_RATIO.append(max(DI_SEI)/max(DI_CP))
    REACTION_RATIO.append(max(reaction_SEI)/max(reaction_CP))
    DISP_RATIO.append(max(disp_SEI)/max(disp_CP))
    
    XLABEL = "Displacement [m]"
    YLABEL = "Base Reaction [N]"
    TITLE = "Equivalent viscous damping ratio - Cyclic Displaecment Hysteresis"
    method = 1 # 
    zeta_CP = S055.EQULIVALENT_VISCOUS_DAMPING_RATIO_FUN(disp_CP, reaction_CP, method, XLABEL, YLABEL, TITLE)
    ...
    method = 1 # 
    zeta_SEI = S055.EQULIVALENT_VISCOUS_DAMPING_RATIO_FUN(disp_SEI, reaction_SEI, method, XLABEL, YLABEL, TITLE)
    print(f"Equivalent viscous damping ratio = {zeta_SEI:.4f}")
    
    EDVR_RATIO.append(zeta_SEI/zeta_CP)
    
totaltime = TI.process_time() - starttime
print(f'\nTotal time (s): {totaltime:.4f} \n\n')

XDATA = OPT_DUCT_target
YDATA = DISP
XLABEL = 'OPTIMUM STRUCTURAL DUCTILITY RATIO'
YLABEL = 'MAX. DISPLACEMENT [m]' 
TITLE = f'{YLABEL} and {XLABEL} DURING PERIOD ANALYSIS WITH 20 GROUND MOTIONS \n TARGETED ENERGY DISSIPACATION CAPAICTY INDEX: {DEMAND:.3f}'
COLOR = 'blue'
S010.PLOT_SCATTER(XDATA, YDATA , XLABEL, YLABEL, TITLE, COLOR, LOG = 0, ORDER = 1)
...
YDATA = PERIOD_RATIO
XLABEL = 'OPTIMUM STRUCTURAL DUCTILITY RATIO'
YLABEL = 'Tmax(SEI) / Tmax(CP)' # 'PERIOD RATIO (SEISMIC DIVDED BY CYCLIC DISPLACEMENT)'
TITLE = f'{YLABEL} and {XLABEL} DURING PERIOD ANALYSIS WITH 20 GROUND MOTIONS \n TARGETED ENERGY DISSIPACATION CAPAICTY INDEX: {DEMAND:.3f}'
COLOR = 'purple'
S010.PLOT_SCATTER(XDATA, YDATA , XLABEL, YLABEL, TITLE, COLOR, LOG = 0, ORDER = 1)

XDATA = OPT_DUCT_target
YDATA = REACTION_RATIO
XLABEL = 'OPTIMUM STRUCTURAL DUCTILITY RATIO'
YLABEL = 'REACTIONmax(SEI) / REACTIONmax(CP)' # 'REACTION RATIO (SEISMIC DIVDED BY CYCLIC DISPLACEMENT)'
TITLE = f'{YLABEL} and {XLABEL} DURING PERIOD ANALYSIS WITH 20 GROUND MOTIONS \n TARGETED ENERGY DISSIPACATION CAPAICTY INDEX: {DEMAND:.3f}'
COLOR = 'red'
S010.PLOT_SCATTER(XDATA, YDATA , XLABEL, YLABEL, TITLE, COLOR, LOG = 0, ORDER = 1)
...
YDATA = DISP_RATIO
XLABEL = 'OPTIMUM STRUCTURAL DUCTILITY RATIO'
YLABEL = 'DISPmax(SEI) / DISPmax(CP)' # 'DISPLACEMENT RATIO (SEISMIC DIVDED BY CYCLIC DISPLACEMENT)'
TITLE = f'{YLABEL} and {XLABEL} DURING PERIOD ANALYSIS WITH 20 GROUND MOTIONS \n TARGETED ENERGY DISSIPACATION CAPAICTY INDEX: {DEMAND:.3f}'
COLOR = 'blue'
S010.PLOT_SCATTER(XDATA, YDATA , XLABEL, YLABEL, TITLE, COLOR, LOG = 0, ORDER = 1)
...
YDATA = DI_RATIO
XLABEL = 'OPTIMUM STRUCTURAL DUCTILITY RATIO'
YLABEL = 'DImax(SEI) / DImax(CP)' # 'DUCTILITY DAMAGE INDEX (SEISMIC DIVDED BY CYCLIC DISPLACEMENT)'
TITLE = f'{YLABEL} and {XLABEL} DURING PERIOD ANALYSIS WITH 20 GROUND MOTIONS \n TARGETED ENERGY DISSIPACATION CAPAICTY INDEX: {DEMAND:.3f}'
COLOR = 'green'
S010.PLOT_SCATTER(XDATA, YDATA , XLABEL, YLABEL, TITLE, COLOR, LOG = 0, ORDER = 1)
...
YDATA = EDVR_RATIO
XLABEL = 'OPTIMUM STRUCTURAL DUCTILITY RATIO'
YLABEL = 'EDVR(SEI) / EDVR(CP)' # 'EDVR (SEISMIC DIVDED BY CYCLIC DISPLACEMENT)'
TITLE = f'{YLABEL} and {XLABEL} DURING PERIOD ANALYSIS WITH 20 GROUND MOTIONS \n TARGETED ENERGY DISSIPACATION CAPAICTY INDEX: {DEMAND:.3f}'
COLOR = 'brown'
S010.PLOT_SCATTER(XDATA, YDATA , XLABEL, YLABEL, TITLE, COLOR, LOG = 0, ORDER = 1)
...
# 3D PLOT
X, Y, Z = DISP_RATIO, DI_RATIO, OPT_DUCT_target
XLABEL, YLABEL, ZLABEL = 'DISPmax(SEI) / DISPmax(CP)', 'DImax(SEI) / DImax(CP)', 'OPTIMUM STRUCTURAL DUCTILITY RATIO'               
S11.PLOT_CONTOUR_3D_2D_FUN(120, X, Y, Z, XLABEL, YLABEL, ZLABEL)

X, Y, Z = EDVR_RATIO, DI_RATIO, OPT_DUCT_target
XLABEL, YLABEL, ZLABEL = 'EDVR(SEI) / EDVR(CP)', 'DImax(SEI) / DImax(CP)', 'OPTIMUM STRUCTURAL DUCTILITY RATIO'               
S11.PLOT_CONTOUR_3D_2D_FUN(121, X, Y, Z, XLABEL, YLABEL, ZLABEL)

X, Y, Z = DISP, DI_RATIO, OPT_DUCT_target
XLABEL, YLABEL, ZLABEL = 'MAX. DISPLACEMENT [m]', 'DImax(SEI) / DImax(CP)', 'OOPTIMUM STRUCTURAL DUCTILITY RATIO'               
S11.PLOT_CONTOUR_3D_2D_FUN(120, X, Y, Z, XLABEL, YLABEL, ZLABEL)

X, Y, Z = VELO, DI_RATIO, OPT_DUCT_target
XLABEL, YLABEL, ZLABEL = 'MAX. VELOCITY  [m/s]', 'DImax(SEI) / DImax(CP)', 'OPTIMUM STRUCTURAL DUCTILITY RATIO'               
S11.PLOT_CONTOUR_3D_2D_FUN(121, X, Y, Z, XLABEL, YLABEL, ZLABEL)

X, Y, Z = ACC, DI_RATIO, OPT_DUCT_target
XLABEL, YLABEL, ZLABEL = 'MAX. ACCELERATION [m/s^2]', 'DImax(SEI) / DImax(CP)', 'OPTIMUM STRUCTURAL DUCTILITY RATIO'               
S11.PLOT_CONTOUR_3D_2D_FUN(121, X, Y, Z, XLABEL, YLABEL, ZLABEL)
...
X = np.column_stack([DISP, VELO, ACC])          
Y = np.column_stack([EDVR_RATIO, OPT_DUCT_target, DI_RATIO])  
X_LABELS = ['Max. Displacement [m]', 'Max. Velocity [m/s]', 'Max. Acceleration [m/s^2]']
Y_LABELS = ['EDVR RATIO', 'OPTIMUM STR. DUCTILITY RATIO', 'DI RATIO']
...
DATA = SDOF(DUCT, MAT_TYPE, TOTAL_MASS, ANAL_TYPE, II)
