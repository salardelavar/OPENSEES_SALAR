...
for i, II in enumerate(range(nK)):
    ...
    for j, JJ in enumerate(range(nD)):
        ...
        TITLE = "Equivalent viscous damping ratio - Free-vibration Analysis Hysteresis"
        method = 1 # 
        zeta = S055.EQULIVALENT_VISCOUS_DAMPING_RATIO_FUN(reaction_FV, disp_FV, method, XLABEL, YLABEL, TITLE)
        print(f"Equivalent viscous damping ratio = {zeta:.4f}")
        """
        
        # Store the values
        WW_matrix[i, j] = damping_ratio_FV  # CHOOSE THIS
        #WW_matrix[i, j] = zeta               # OR CHOOSE THIS
