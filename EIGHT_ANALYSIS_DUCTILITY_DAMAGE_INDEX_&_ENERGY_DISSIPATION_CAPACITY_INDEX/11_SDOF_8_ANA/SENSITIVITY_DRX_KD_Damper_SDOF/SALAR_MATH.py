...
def HISROGRAM_BOXPLOT(X, HISTO_COLOR, LABEL):
    ...
    #print(f'Skewness: {SKEW :.6e}')
    #print(f'kurtosis: {KURT :.6e}')
...
def RELIABILITY_ANALYSIS(base_reaction, num_sim, mean_capacity, std_dev_capacity, TITLE):
    """
    ...
    if len(base_reaction) != num_sim:
        raise ValueError(f"Length of {TITLE} must match the number of simulations (num_sim).")
    ...
    import matplotlib.pyplot as plt
    plt.figure(figsize=(10, 6))
    plt.hist(element_capacity, bins=50, color='blue', alpha=0.7, label="R: Capacity")
    plt.hist(base_reaction, bins=50, color='red', alpha=0.7, label=f"D: {TITLE}")
...
def MULTIPLE_REGRESSION(df):
    ...
    X = sm.add_constant(df[['displacement_y',
                            'rotation_z',
                            'base_axial',
                            'base_shear',
                            'base_moment',
    ...
    model = sm.OLS(df['displacement_y'], X).fit()
...
def RANDOM_FOREST(df, threshold_displacement):
    ...
    df['Safety_label'] = (df['displacement_x'] <= threshold_displacement).astype(int)
    ...
    data = {
        ...
        'temperature':Max_Thermal,
        'yield_strength_rebar':fy,
        'compressive_strength_concrete':fcp,
        'distributed_load':distributed_load,
        'Strain_hardening_ratio':b,
    }
