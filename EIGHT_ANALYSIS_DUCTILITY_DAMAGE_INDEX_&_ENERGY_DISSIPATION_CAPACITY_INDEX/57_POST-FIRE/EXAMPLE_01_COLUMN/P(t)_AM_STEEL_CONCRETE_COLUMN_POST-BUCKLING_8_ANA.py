...
# COMPREHENSIVE NONLINEAR SEISMIC ASSESSMENT OF MULTI-MODE POST-BUCKLING PHENOMENA IN A POST-FIRE STEEL   #
# COLUMN WITH SEMI-RIGID CONNECTIONS: AN OPENSEES FRAMEWORK FOR STATIC PUSHOVER, CYCLIC DEGRADATION,      #
#                       STATIC TIME-HISTORY, AND DYNAMIC TIME-HISTORY ANALYSES                            #
#---------------------------------------------------------------------------------------------------------#
# NONLINEAR ANALYSIS OF POST-FIRE STEEL COLUMN STRUCTURES SUBJECTED TO PUSHOVER, IMPACT, AND SEISMIC LOADS# 
#---------------------------------------------------------------------------------------------------------#
#                        THERMAL EFFECTS ON STEEL MATERIAL STRESS-STRAIN REALTION                         #
#---------------------------------------------------------------------------------------------------------#
...
###########################################################################################################
"""
Assume we have a structure where, after a fire, certain regions of
 the structural members have lost their initial material properties
 (yield strength and modulus of elasticity), and the entire column
 member has undergone lateral buckling. Now, we assess the strength
 of this structure by performing eight different analyses.
...
import AFTER_FIRE_01_STEEL_SECTION as S11
import AFTER_FIRE_02_STEEL_SECTION as S12
import AFTER_FIRE_03_STEEL_SECTION as S13
import AFTER_FIRE_04_STEEL_SECTION as S14
...
def POST_BUCKLING_COLUMN(LENGTH, MAT_TYPE, ELE_TYPE, ANAL_TYPE, TOTAL_MASS, MODE_NUM, SEMI_RIGID):
    ...
    #Depth, Ele_Mass = S11.AFTER_FIRE_01_STEEL_SECTION_DIFF_WIDTH_HEIGHT_FUN_EXTRA_TWO(SEC_TAG, STEEL_TYPE, fc, Kfc, CONCRETE_DENSITY, plot=True)
    #Depth, Ele_Mass = S12.AFTER_FIRE_02_STEEL_SECTION_DIFF_WIDTH_HEIGHT_FUN_EXTRA_TWO(SEC_TAG, STEEL_TYPE, fc, Kfc, CONCRETE_DENSITY, plot=True)
    Depth, Ele_Mass = S13.AFTER_FIRE_03_STEEL_SECTION_DIFF_WIDTH_HEIGHT_FUN_EXTRA_TWO(SEC_TAG, STEEL_TYPE, fc, Kfc, CONCRETE_DENSITY, plot=True)
    #Depth, Ele_Mass = S14.AFTER_FIRE_04_I_STEEL_PLATE_SECTION_DIFF_WIDTH_HEIGHT_FUN_EXTRA(SEC_TAG, STEEL_TYPE, STEEL_DENSITY, plot=True)
    #%% STEEL SECTION
    ...
    #%% FOR SMALL BEAM ELEMENT
    ops.node(N+4, BL, LENGTH)         # NODES FOR RIGID ELEMENT 
    #ops.rigidLink('-beam', N+1, N+4) # RIGID ELEMENT
    ops.element('elasticBeamColumn', 5000, N+1, N+4, SEC_TAG, transfTag,'-mass', Ele_Mass)
    #%%--------------------------------------
    ...
    if ANAL_TYPE == 'PUSHOVER': # STATIC TIME-HISTORY ANALYSIS
        ...
        DMAX = -15.0     # [mm] Max. Displacement
        ops.integrator('DisplacementControl', load_node, IDctrlDOF, DINCR)
        ops.analysis('Static')
        ...
        for step in range(Nsteps):
            ...
            reactionY.append(ops.nodeReaction(1, 2))                        # AXIAL BASE REACTION
            reactionZ.append(ops.nodeReaction(1, 3))                        # MOMENT BASE REACTION
            disp_mid.append(ops.nodeDisp(middle_node, 2))                   # DISPLACEMENT NODE
            dispX.append(ops.nodeDisp(middle_node, 1))                      # DISPLACEMENT NODE IN X DIR 
            dispY.append(ops.nodeDisp(middle_node, 2))                      # DISPLACEMENT NODE IN Y DIR 
    ...
    if ANAL_TYPE == 'CYCLIC_DISPLACEMENT': # STATIC TIME-HISTORY ANALYSIS
        ...
        for target_disp in disp_protocol:
            current_disp = ops.nodeDisp(load_node, 2) # DISPALCEMENT APPLIED IN NODE IN X DIR.
            dU = target_disp - current_disp
            ops.integrator('DisplacementControl', load_node, IDctrlDOF, dU)
            OK = ops.analyze(1)
            S02.ANALYSIS(OK, 1, MAX_TOLERANCE, MAX_ITERATIONS)
            ops.reactions()
            reactionX.append(ops.nodeReaction(1, 1))                        # SHEAR BASE REACTION
    ...
    if ANAL_TYPE == 'DYNAMIC_EXTERNAL_TIME-DEPENDENT_LOADING': # DYNAMIC TIME-HISTORY ANALYSIS
        ...
        while stable == 0 and current_time < duration:
            ...
            dispX.append(ops.nodeDisp(middle_node, 1))            # DISPLACEMENT NODE IN X DIR 
            dispY.append(ops.nodeDisp(middle_node, 2))            # DISPLACEMENT NODE IN Y DIR 
            veloX.append(ops.nodeVel(middle_node, 1))             # VELOCITY NODE
            veloY.append(ops.nodeVel(middle_node, 2))             # VELOCITY NODE
            accX.append(ops.nodeAccel(middle_node, 1))            # ACCELERATION NODE
            accY.append(ops.nodeAccel(middle_node, 2))            # ACCELERATION NODE
    ...
    if ANAL_TYPE == 'FREE-VIBRATION': # DYNAMIC TIME-HISTORY ANALYSIS
        ...
        if IU == True:
            ...
            ops.setNodeDisp(load_node, 2, u0, '-commit') # INFO LINK: https://openseespydoc.readthedocs.io/en/latest/src/setNodeDisp.html
        if IV == True:
            # Define initial velocity
            ops.setNodeVel(load_node, 2, v0, '-commit')  # INFO LINK: https://openseespydoc.readthedocs.io/en/stable/src/setNodeVel.html
        if IA == True:
            # Define initial  acceleration
            ops.setNodeAccel(load_node, 2, a0, '-commit') # INFO LINK: https://openseespydoc.readthedocs.io/en/latest/src/setNodeAccel.html
        ...
        DATA = (time, reactionX, reactionY, reactionZ, disp_mid,
                ...
                veloX, veloY,
                accX, accY,
                stiffness, PERIOD, damping_ratioX, damping_ratioY,
                np.array(PERIOD_MIN), np.array(PERIOD_MAX),
                STRESSb, STRAINb, STRESSt, STRAINt, CURVATURE, NA, CD)
...
TITLE = 'Base Shear Reaction and Displacement of Structure During Pushover Analysis'
COLOR = 'black'
SEMILOGY = False
...
TITLE = 'Base Axial Reaction and Displacement of Structure During Pushover Analysis'
COLOR = 'black'
SEMILOGY = False
PLOT(XDATA, YDATA, TITLE, XLABEL, YLABEL, COLOR, SEMILOGY)
...
XLABEL = 'Displacement in X Dir. [mm]'
YLABEL = 'Base Shear Reaction [N]'
TITLE = 'Base Shear Reaction and Displacement of Structure During Cyclic-Displacement Analysis'
COLOR = 'black'
SEMILOGY = False
PLOT(XDATA, YDATA, TITLE, XLABEL, YLABEL, COLOR, SEMILOGY)
...
XLABEL = 'Displacement in Y Dir. [mm]'
YLABEL = 'Base Axial Reaction [N]'
TITLE = 'Base Axial Reaction and Displacement of Structure During Cyclic-Displacement Analysis'
COLOR = 'black'
SEMILOGY = False
PLOT(XDATA, YDATA, TITLE, XLABEL, YLABEL, COLOR, SEMILOGY)
...
method = 1 # 
zeta = S055.EQULIVALENT_VISCOUS_DAMPING_RATIO_FUN(dispX_CP, reactionX_CP, method, XLABEL, YLABEL, TITLE)
...
TITLE = "Equivalent viscous damping ratio - Cyclic Displacement Hysteresis"
method = 1 # 
zeta = S055.EQULIVALENT_VISCOUS_DAMPING_RATIO_FUN(dispY_CP, reactionY_CP, method, XLABEL, YLABEL, TITLE)
print(f"Equivalent viscous damping ratio = {zeta:.4f}")
...
XLABEL = 'Displacement in X Dir. [mm]'
YLABEL = 'Base Shear Reaction [N]'
TITLE = 'Base Shear Reaction and Displacement of Structure During Static External Time-dependent Loading Analysis'
COLOR = 'black'
SEMILOGY = False
PLOT(XDATA, YDATA, TITLE, XLABEL, YLABEL, COLOR, SEMILOGY)
...
XLABEL = 'Displacement in Y Dir. [mm]'
YLABEL = 'Base Axial Reaction [N]'
TITLE = 'Base Axial Reaction and Displacement of Structure During Static External Time-dependent Loading Analysis'
COLOR = 'black'
SEMILOGY = False
PLOT(XDATA, YDATA, TITLE, XLABEL, YLABEL, COLOR, SEMILOGY)
...
XLABEL = 'Displacement in X Dir. [mm]'
YLABEL = 'Base Shear Reaction [N]'
TITLE = 'Base Shear Reaction and Displacement of Structure During Dynamic External Time-dependent Loading Analysis'
COLOR = 'black'
SEMILOGY = False
PLOT(XDATA, YDATA, TITLE, XLABEL, YLABEL, COLOR, SEMILOGY)
...
XLABEL = 'Displacement in Y Dir. [mm]'
YLABEL = 'Base Axial Reaction [N]'
TITLE = 'Base Axial Reaction and Displacement of Structure During Dynamic External Time-dependent Loading Analysis'
COLOR = 'black'
SEMILOGY = False
PLOT(XDATA, YDATA, TITLE, XLABEL, YLABEL, COLOR, SEMILOGY)
...
veloX_FV, veloY_FV,
accX_FV, accY_FV,
stiffness_FV, PERIOD_FV, damping_ratio_FV_X, damping_ratio_FV_Y,
...
XLABEL = 'Displacement in X Dir. [mm]'
YLABEL = 'Base Shear Reaction [N]'
TITLE = 'Base Shear Reaction and Displacement of Structure During Free-vibration Analysis\n Damping Ratio in X Dir: {damping_ratio_FV_X:.3e} [%]'
COLOR = 'black'
SEMILOGY = False
PLOT(XDATA, YDATA, TITLE, XLABEL, YLABEL, COLOR, SEMILOGY)
...
XLABEL = 'Displacement in Y Dir. [mm]'
YLABEL = 'Base Axial Reaction [N]'
TITLE = 'Base Axial Reaction and Dispalcement of Structure During Free-vibration Analysis\n Damping Ratio in Y Dir: {damping_ratio_FV_Y:.3e} [%]'
COLOR = 'black'
SEMILOGY = False
PLOT(XDATA, YDATA, TITLE, XLABEL, YLABEL, COLOR, SEMILOGY)
...
# PLOT ELEMENTS MOMENT FORCE
YLABEL = 'Element Moment Force [N.m]'  
...
method = 1 # 
zeta = S055.EQULIVALENT_VISCOUS_DAMPING_RATIO_FUN(dispX_FV, reactionX_FV, method, XLABEL, YLABEL, TITLE)
print(f"Equivalent viscous damping ratio = {zeta:.4f}")
...
XLABEL = 'Displacement in X Dir. [mm]'
YLABEL = 'Base Shear Reaction [N]'
TITLE = 'Base Shear Reaction and Displacement of Structure During Seismic Analysis'
COLOR = 'black'
SEMILOGY = False
PLOT(XDATA, YDATA, TITLE, XLABEL, YLABEL, COLOR, SEMILOGY)
...
XLABEL = 'Displacement in Y Dir. [mm]'
YLABEL = 'Base Axial Reaction [N]'
TITLE = 'Base Axial Reaction and Dispalcement of Structure During Seismic Analysis'
COLOR = 'black'
SEMILOGY = False
PLOT(XDATA, YDATA, TITLE, XLABEL, YLABEL, COLOR, SEMILOGY)
...
method = 1 # 
zeta = S055.EQULIVALENT_VISCOUS_DAMPING_RATIO_FUN(dispX_SEI, reactionX_SEI, method, XLABEL, YLABEL, TITLE)
print(f"Equivalent viscous damping ratio = {zeta:.4f}")
...
TITLE = "Equivalent viscous damping ratio - Seismic Hysteresis"
method = 1 # 
zeta = S055.EQULIVALENT_VISCOUS_DAMPING_RATIO_FUN(dispY_SEI, reactionY_SEI, method, XLABEL, YLABEL, TITLE)
print(f"Equivalent viscous damping ratio = {zeta:.4f}")
...
# ----------------------------------------
XX = np.abs(dispY_PUSH); YY = np.abs(reactionY_PUSH); # ABSOLUTE VALUE
SLOPE_NODE = 10
...
COLOR = 'black'
PLOT_2D(np.abs(dispY_PUSH), np.abs(reactionY_PUSH), X, Y, X, Y, XLABEL, YLABEL, TITLE, LEGEND01, LEGEND02, LEGEND03, COLOR='black', Z=2) 
#print(f'\t\t Ductility Ratio: {Y[2]/Y[1]:.4f}')
