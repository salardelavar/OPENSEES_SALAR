...
def BEAM_ONE_ELEMENT_SIMPLY_SUPPORTED(LENGTH, MAT_TYPE, ELE_TYPE, ANAL_TYPE, TOTAL_MASS):
    ...
    if ANAL_TYPE == 'STATIC': 
        ...
        ops.reactions()
        reaction.append(ops.nodeReaction(1, 2)+ops.nodeReaction(5, 2))                         # SHEAR BASE REACTION
        disp_mid.append(ops.nodeDisp(center_node, 2))                   # DISPLACEMENT NODE 03 
        dispX.append(ops.nodeDisp(center_node, 1))                      # DISPLACEMENT NODE 03 IN X DIR 
        dispY.append(ops.nodeDisp(center_node, 2))                      # DISPLACEMENT NODE 03 IN Y DIR 
    ...
    if ANAL_TYPE == 'DYNAMIC_EXTERNAL_TIME-DEPENDENT_LOADING': # DYNAMIC TIME-HISTORY ANALYSIS
        ...
        while stable == 0 and current_time < duration:
            ...
            dispX.append(ops.nodeDisp(center_node, 1))            # DISPLACEMENT NODE 03 IN X DIR 
            dispY.append(ops.nodeDisp(center_node, 2))            # DISPLACEMENT NODE 03 IN Y DIR 
            veloX.append(ops.nodeVel(center_node, 1))             # VELOCITY NODE 03
            veloY.append(ops.nodeVel(center_node, 2))             # VELOCITY NODE 03
            accX.append(ops.nodeAccel(center_node, 1))            # ACCELERATION NODE 03
            accY.append(ops.nodeAccel(center_node, 2))            # ACCELERATION NODE 03
...
# Save to Excel
with pd.ExcelWriter("P(t)_BEAM_ONE_ELEMENT_OUTPUT.xlsx", engine='openpyxl') as writer:
