...
#           COMPREHENSIVE NONLINEAR SEISMIC ASSESSMENT OF A FRAME STRUCTURE : AN OPENSEES FRAMEWORK       #
...
###########################################################################################################
"""
Nonlinear Seismic Performance Assessment of a Frame:
An OpenSeesPy Framework for Material and Geometric Nonlinearity Under Static, Cyclic, and Earthquake Loading

This OpenSeesPy script performs rigorous nonlinear static and dynamic analysis of a Frame
...
#%%----------------------------------------------------
def FRAME_TRUSS(MAT_TYPE, TOTAL_MASS):
    # Initialize model
    ops.wipe()
    ...
    # Add nodes
    ops.node(1, 0.0, 0.0)
    ops.node(2, 0.5*L, 0.5*L)
    ops.node(3, L, 0.0)#-1.0e-8
    ...
    ele_mass = AREA * DENSITY_STEEL
    # Geometric transformation
    transfTag = 1
    #ops.geomTransf('Linear', transfTag)
    #ops.geomTransf('PDelta', transfTag)
    ops.geomTransf('Corotational', transfTag)
    ops.element('elasticBeamColumn', 100, 1, 2, AREA, Es, Iz, transfTag, '-mass', ele_mass)
    ops.element('elasticBeamColumn', 200, 2, 3, AREA, Es, Iz, transfTag, '-mass', ele_mass)
    ...
    ops.element('corotTruss', Ele_Tag_03, 1, 3, AREA, MAT_TAG, '-rho',AREA*DENSITY_STEEL , '-doRayleigh', 1)
    
    center_node = 3 # Target Node
    ...
    if ANAL_TYPE == 'STATIC_EXTERNAL_TIME-DEPENDENT_LOADING': # STATIC TIME-HISTORY ANALYSIS
        ...
        ops.load(center_node, 1.0, 0.0, 0.0) # P(t) = P0 exp(-0.05wt) sin(wt) 
    ...
    if ANAL_TYPE == 'FREE-VIBRATION': # DYNAMIC TIME-HISTORY ANALYSIS
        ...
        u0 = -0.01                         # [m] Initial displacement
        v0 = 0.150                         # [m/s] Initial velocity
...
DATA = FRAME_TRUSS(MAT_TYPE, TOTAL_MASS)
...
X, HISTO_COLOR, LABEL = dispX_CP, 'cyan', 'DISPLACEMENT FROM SEISMIC ANALYSIS'
S09.HISROGRAM_BOXPLOT(X, HISTO_COLOR, LABEL)
X, HISTO_COLOR, LABEL = reactionX_CP, 'purple', 'REACTION FROM SEISMIC ANALYSIS'
S09.HISROGRAM_BOXPLOT(X, HISTO_COLOR, LABEL)
...
X, HISTO_COLOR, LABEL = dispX_SEI, 'cyan', 'DISPLACEMENT FROM SEISMIC ANALYSIS'
S09.HISROGRAM_BOXPLOT(X, HISTO_COLOR, LABEL)
X, HISTO_COLOR, LABEL = reactionX_SEI, 'purple', 'REACTION FROM SEISMIC ANALYSIS'
S09.HISROGRAM_BOXPLOT(X, HISTO_COLOR, LABEL)
...
# Save to Excel
with pd.ExcelWriter("FRAME_TRUSS_OUTPUT.xlsx", engine='openpyxl') as writer:
