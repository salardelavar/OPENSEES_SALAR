...
# COMPREHENSIVE NONLINEAR SEISMIC ASSESSMENT OF A CONCRETE SLAB SUPERSTRUCTURE SIMPILY SUPPORTED SHORT    #
#           SPAN BRIDGE : AN OPENSEES FRAMEWORK FOR STATIC PUSHOVER, CYCLIC DEGRADATION,                  #
...
This OpenSeesPy script performs rigorous nonlinear static and dynamic analysis of a simpily supported concrete slab short span bridge
...
# YOUTUBE: Design of Slab Bridges
'https://www.youtube.com/watch?v=RX-WImcb73Y'
# YOUTUBE: Design of Bridge Deck Slab
'https://www.youtube.com/watch?v=KGWP2x5FvGo'
...
import CONCRETE_SLAB_SECTION_FUN_EXTRA as S14
import SECTION_CRACK_DEPTH_FUN as S18
import EQULIVALENT_VISCOUS_DAMPING_RATIO_FUN as S055
#%%----------------------------------------------------
def SHORT_SPAN_SLAB_CONCRETE_BRIDGE(LENGTH, MAT_TYPE, ELE_TYPE, ANAL_TYPE, TOTAL_MASS):
    # Initialize model
    ops.wipe()
    ...
    Depth, Ele_Mass = S14.CONCRETE_SLAB_SECTION_FUN(SEC_TAG, STEEL_TYPE, fc, Kfc,
                               Bsec, Hsec, cover,
    ...
    if ANAL_TYPE == 'PUSHOVER': # STATIC TIME-HISTORY ANALYSIS
        ...
        DMAX = -10.0    # [mm] Max. Vertical Displacement
    ...
    if ANAL_TYPE == 'CYCLIC_DISPLACEMENT': # STATIC TIME-HISTORY ANALYSIS
        IDctrlDOF = 2   # 1: Horizental Dispalcement - 2: Vertical Dispalcement
        DMAX = -2.50    # [mm] Max. Vertical Displacement
        n_points = 10000
    ...
    if ANAL_TYPE == 'SEISMIC': # DYNAMIC TIME-HISTORY ANALYSIS
        ...
        SSF_X = 0.05                       # Seismic Acceleration Scale Factor in X Direction
        SSF_Y = 0.05                       # Seismic Acceleration Scale Factor in Y Direction
...
DATA = SHORT_SPAN_SLAB_CONCRETE_BRIDGE(LENGTH, MAT_TYPE, ELE_TYPE, ANAL_TYPE, TOTAL_MASS)
...
# Save to Excel
with pd.ExcelWriter("P(t)_BRIDGE_CONCRETE_SLAB_SUPERSTRUCTURE_SIMPLY_SUPPORTED_OUTPUT.xlsx", engine='openpyxl') as writer:
