...
#           COMPREHENSIVE NONLINEAR SEISMIC ASSESSMENT OF A CONCRETE COLUMN : AN OPENSEES                 #
...
###########################################################################################################
"""
Nonlinear Seismic Performance Assessment of a concrete column:
An OpenSeesPy Framework for Material and Geometric Nonlinearity Under Static, Cyclic, and Earthquake Loading

This OpenSeesPy script performs rigorous nonlinear static and dynamic analysis of a concrete column
...
import CONCRETE_I_SECTION_FUN_EXTRA as S08
import CONCRETE_RECTANGULAR_HOLLOW_SECTION_FUN as S11
import CONCRETE_T_SECTION_FUN_EXTRA as S12
import CONCRETE_CIRCULALR_HOLLOW_PIPE_SECTION_FUN as S13
import CONCRETE_REC_SECTION_FUN_EXTRA as S14
import CONCRETE_I_SECTION_DIFF_FLANGE_FUN_EXTRA as S15
import CONCRETE_SECTION_DIFF_WIDTH_HEIGHT_FUN_EXTRA as S16
import STEEL_SECTION_DIFF_WIDTH_HEIGHT_FUN_EXTRA as S17
import SECTION_CRACK_DEPTH_FUN as S18
import EQULIVALENT_VISCOUS_DAMPING_RATIO_FUN as S055
import CONCRETE_RECTANGULAR_HOLLOW_SECTION_FUN_TWO as S19
import CONCRETE_CIRCULALR_HOLLOW_PIPE_SECTION_FUN_TWO as S20
#%%----------------------------------------------------
def CONCRETE_COLUMN(LENGTH, MAT_TYPE, ELE_TYPE, ANAL_TYPE, TOTAL_MASS):
    # Initialize model
    ops.wipe()
    ...
    #%% GEOMETRY DEFINITION
    ops.node(1, 0.0, (0/6)*LENGTH)
    ops.node(2, 0.0, (1/6)*LENGTH)
    ops.node(3, 0.0, (2/6)*LENGTH)
    ops.node(4, 0.0, (3/6)*LENGTH)
    ops.node(5, 0.0, (4/6)*LENGTH)
    ops.node(6, 0.0, (5/6)*LENGTH)
    ops.node(7, 0.0, (6/6)*LENGTH)
    #%% SUPPORTS
    ops.fix(1, 1, 1, 1)      # bot  - fixed
    
    #%% SECTION
    # I CONCRETE SECTION
    bf = 1000.0     # [mm] Flange width
    tf = 100.0      # [mm] Flange thickness
    h  = 600.0      # [mm] Total height
    tw = 200.0      # [mm] Web thickness
    ...
    """
    Depth, Ele_Mass = S08.CONCRETE_I_SECTION_FUN(SEC_TAG, STEEL_TYPE, fc, Kfc, bf, tf, h, tw,
                                       nFib, CONCRETE_DENSITY, 
                                       plot=True)
    
    """
    """
    b_o, h_o, b_i, h_i = 1000.0, 1000.0, 700.0, 700.0 # [mm]
    Depth, Ele_Mass = S11.CONCRETE_RECTANGULAR_HOLLOW_SECTION_FUN(SEC_TAG, STEEL_TYPE, fc, Kfc,
                               b_o, h_o, b_i, h_i,
                               nFib, CONCRETE_DENSITY,
                               plot=True)
    ...
    """
    b_o, h_o, b_i, h_i = 1000.0, 1000.0, 700.0, 700.0 # [mm]
    Depth, Ele_Mass = S19.CONCRETE_RECTANGULAR_HOLLOW_SECTION_FUN(
    SEC_TAG,
    STEEL_TYPE,
    fc,
    Kfc,
    ...
    """
    """
    Depth, Ele_Mass = S12.CONCRETE_T_SECTION_FUN(SEC_TAG, MAT_TAG, STEEL_TYPE, fc, Kfc,
                               bf, tf, h, tw,
                               nFib, CONCRETE_DENSITY,
                               plot=True)
    """
    """
    Do = 700.0                    # [mm] Outer diameter
    t = 100.0                     # [mm] thickness
    numSubdivCirc = 32            # circumferential divisions
    numSubdivRad = 4              # radial divisions
    Depth, Ele_Mass = S13.CONCRETE_CIRCULALR_HOLLOW_PIPE_SECTION_FUN(SEC_TAG, STEEL_TYPE, Do, t,
    ...
    numSubdivCirc = 32            # circumferential divisions
    numSubdivRad = 4              # radial divisions
    Depth, Ele_Mass = S20.CONCRETE_CIRCULAR_HOLLOW_PIPE_SECTION_FUN(
        SEC_TAG, STEEL_TYPE, Do, t,
        numSubdivCirc, numSubdivRad,
        fc, Kfc,
        CONCRETE_DENSITY,
        nRebars=24,
    ...
    """
    # REC. CONCRETE SECTION
    Bsec = 400.0     # [mm] width
    Hsec = 600.0     # [mm] height
    cover = 50.0     # [mm]      
    fc = 30.0        # [N/mm²] Concrete Compressive Strength
    Kfc = 1.24       # ratio of confined to unconfined concrete strength
    SEC_TAG = 1
    ...
    nFib = 40
    CONCRETE_DENSITY = 2500/1e9      # [kg/m^3] -> [kg/mm^3] Concrete Material Density
    Depth, Ele_Mass = S14.CONCRETE_REC_SECTION_FUN(SEC_TAG, STEEL_TYPE, fc, Kfc,
                               Bsec, Hsec, cover,
                               nFib, CONCRETE_DENSITY,
                               plot=True)
    """
    """
    # I CONCRETE SECTION WITH DIFFERENT FLANGES
    bfT = 1000.0     # [mm] Top Flange width
    tfT = 100.0      # [mm] Top Flange thickness
    bfB = 500.0      # [mm] Bottom Flange width
    tfB = 200.0      # [mm] Bottom Flange thickness
    h  = 600.0       # [mm] Total height
    Depth, Ele_Mass = S15.CONCRETE_I_SECTION_DIFF_FLANGE_FUN(SEC_TAG, STEEL_TYPE, fc, Kfc,
                           bfT, tfT, bfB, tfB, h, tw,
                           nFib, CONCRETE_DENSITY,
                           plot=True)
    """
    """
    Depth, Ele_Mass = S16.CONCRETE_SECTION_DIFF_WIDTH_HEIGHT_FUN_EXTRA(SEC_TAG, STEEL_TYPE, fc, Kfc,
                                                                       nFib, CONCRETE_DENSITY,
                                                                       plot=True)
    """
    """
    STEEL_DENSITY = 7850/1e9
    Depth, Ele_Mass = S17.STEEL_SECTION_DIFF_WIDTH_HEIGHT_FUN_EXTRA(SEC_TAG, STEEL_TYPE,
                                                                    nFib, STEEL_DENSITY,
                                                                    plot=True)
    """
    #%% CREATE ELEMENTS
    ...
    if ANAL_TYPE == 'PUSHOVER': # STATIC TIME-HISTORY ANALYSIS
        ...
        DMAX = -2500.0  # [mm] Max. Displacement
        ...
        for step in range(Nsteps):
            ...
            if ELE_TYPE == 'nonlinearBeamColumn': 
                ...
                na, cd = S18.SECTION_CRACK_DEPTH_FUN(center_node-1, SEC_TAG,
                                        -0.004,          # TOP FIBER MATERIAL ULTIMATE STRAIN FOR CRACK DEPTH EVALUTION
    ...
    if ANAL_TYPE == 'CYCLIC_DISPLACEMENT': # STATIC TIME-HISTORY ANALYSIS
        ...
        for target_disp in disp_protocol:
            current_disp = ops.nodeDisp(center_node, 1) # DISPALCEMENT APPLIED IN NODE IN X DIR.
            ...
            reactionX.append(ops.nodeReaction(1, 1))                        # SHEAR BASE REACTION
            reactionY.append(ops.nodeReaction(1, 2))                        # AXIAL BASE REACTION
            reactionZ.append(ops.nodeReaction(1, 3))                        # MOMENT BASE REACTION            disp_mid.append(ops.nodeDisp(center_node, 1))                   # DISPLACEMENT NODE 07 
            disp_mid.append(ops.nodeDisp(center_node, 1))                   # DISPLACEMENT NODE 07
            dispX.append(ops.nodeDisp(center_node, 1))                      # DISPLACEMENT NODE 07 IN X DIR 
            dispY.append(ops.nodeDisp(center_node, 2))                      # DISPLACEMENT NODE 07 IN Y DIR 
            # Store elements force, strain and stress
...
def PLOT_2D(X, Y, Xfit, Yfit, X2, Y2, XLABEL, YLABEL, TITLE, LEGEND01, LEGEND02, LEGEND03, COLOR, Z):
    ...
    if Z == 3:
        ...
        plt.show() 
#%%----------------------------------------------------
LENGTH = 15000.0      # [mm] Column Length
...
DATA = CONCRETE_COLUMN(LENGTH, MAT_TYPE, ELE_TYPE, ANAL_TYPE, TOTAL_MASS)
...
method = 1 # 
zeta = S055.EQULIVALENT_VISCOUS_DAMPING_RATIO_FUN(disp_mid_CP, reactionX_CP, method, XLABEL, YLABEL, TITLE)
...
method = 2 # 
zeta = S055.EQULIVALENT_VISCOUS_DAMPING_RATIO_FUN(disp_mid_FV, reactionX_FV, method, XLABEL, YLABEL, TITLE)
print(f"Equivalent viscous damping ratio = {zeta:.4f}")
...
method = 1 # 
zeta = S055.EQULIVALENT_VISCOUS_DAMPING_RATIO_FUN(disp_mid_SEI, reactionX_SEI, method, XLABEL, YLABEL, TITLE)
print(f"Equivalent viscous damping ratio = {zeta:.4f}")
...
# --------------------------------------
XX = np.abs(disp_mid_PUSH); YY = np.abs(reactionX_PUSH); # ABSOLUTE VALUE
SLOPE_NODE = 10
...
Ed_SEI, fig_SEI = DISSIPATED_ENERGY_FUN_WITH_PLOT(
    disp_mid_SEI, reactionX_SEI, method = 2, 
...
Ed_CP, fig_CP = DISSIPATED_ENERGY_FUN_WITH_PLOT(
    disp_mid_CP, reactionX_CP, method = 2,
...
# Save to Excel
with pd.ExcelWriter("W(x,t)_CONCRETE_COLUMN_8_ANA_OUTPUT.xlsx", engine='openpyxl') as writer:
