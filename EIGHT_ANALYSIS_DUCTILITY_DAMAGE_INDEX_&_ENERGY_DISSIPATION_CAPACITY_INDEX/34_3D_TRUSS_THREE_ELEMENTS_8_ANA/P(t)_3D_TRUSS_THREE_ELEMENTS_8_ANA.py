...
#        COMPREHENSIVE NONLINEAR SEISMIC ASSESSMENT OF THREE ELEMENTS 3D TRUSS : AN OPENSEES              #
...
###########################################################################################################
"""
Nonlinear Seismic Performance Assessment of Three Elements 3D Truss:
An OpenSeesPy Framework for Material and Geometric Nonlinearity Under Static, Cyclic, and Earthquake Loading

This OpenSeesPy script performs rigorous nonlinear static and dynamic analysis of hree Elements 3D Truss
...
#%%----------------------------------------------------
def THREE_ELEMENTS_3D_TRUSS(LENGTH, AREA, MAT_TYPE, ELE_TYPE, ANAL_TYPE, TOTAL_MASS):
    # Initialize model
    ops.wipe()
    ...
    #%% GEOMETRY DEFINITION
    ops.node(1, LENGTH, 0.0, 0.0)
    ops.node(2, 0.0, 0.0, LENGTH) # CENTER NODE
    ops.node(3, -0.5 * LENGTH, np.sqrt(3) / 2 * LENGTH, 0.0)
    ops.node(4, -0.5 * LENGTH, -np.sqrt(3) / 2 * LENGTH, 0.0)
    #%% SUPPORTS
    ops.fix(1, 1, 1, 1)      # left  - pinned
    ops.fix(3, 1, 1, 1)      # right - pinned
    ops.fix(4, 1, 1, 1)      # right - pinned
    #%% CREATE ELEMENTS
    ele_tag = 1
    ...
    if ELE_TYPE == 'corotTruss':
        ...
        ops.element('corotTruss', 2, 3, 2, AREA, MAT_TAG, '-rho', ele_mass, '-doRayleigh', 1)
        ops.element('corotTruss', 3, 4, 2, AREA, MAT_TAG, '-rho', ele_mass, '-doRayleigh', 1)
    if ELE_TYPE == 'Truss': 
        ...
        ops.element('Truss', 2, 3, 2, AREA, MAT_TAG, '-rho', ele_mass, '-doRayleigh', 1)
        ops.element('Truss', 3, 4, 2, AREA, MAT_TAG, '-rho', ele_mass, '-doRayleigh', 1)
    ele_tag = 3 + 1
    #%%--------------------------------------
    """
    ...
    for ii in range(1, tag):
        ops.load(ii, 0.0, 0.0, -ENW)   # [N] downward
        ops.mass(ii, ENW, ENW, ENW)
        
    TS_TAG = 2
    PATT_TAG = 2
    ops.timeSeries('Linear', TS_TAG)
    ops.pattern('Plain', PATT_TAG, TS_TAG)     
    ops.load(center_node, 0.0, 0.0, -1.0)
    ...
    if ANAL_TYPE == 'STATIC': 
        ...
        ops.reactions()
        reaction.append(ops.nodeReaction(1, 3)+ops.nodeReaction(3, 3)+ops.nodeReaction(4, 3))  # SHEAR BASE REACTION        
        disp_mid.append(ops.nodeDisp(center_node, 3))                   # DISPLACEMENT NODE 02 
        ...
        dispZ.append(ops.nodeDisp(center_node, 3))                      # DISPLACEMENT NODE 02 IN Z DIR 
        ...
        for node_id in node_displacementsX.keys():    
            ...
            node_displacementsY[node_id].append(ops.nodeDisp(node_id, 3))
        else:
    ...
    if ANAL_TYPE == 'PUSHOVER': # STATIC TIME-HISTORY ANALYSIS
        IDctrlDOF = 3   # 1: Horizental Dispalcement - 2: Vertical Dispalcement
        DINCR = -0.01   # [mm] Incremental Vertical Displacement
        DMAX = -100.0   # [mm] Max. Vertical Displacement
        ...
        for step in range(Nsteps):
            ...
            S02.ANALYSIS(OK, 1, MAX_TOLERANCE, MAX_ITERATIONS)
            ops.reactions()
            reaction.append(ops.nodeReaction(1, 3)+
                            ops.nodeReaction(3, 3)+
                            ops.nodeReaction(4, 3))  # SHEAR BASE REACTION            
            disp_mid.append(ops.nodeDisp(center_node, 3))                   # DISPLACEMENT NODE 02 
            dispX.append(ops.nodeDisp(center_node, 1))                      # DISPLACEMENT NODE 02 IN X DIR 
            dispY.append(ops.nodeDisp(center_node, 2))                      # DISPLACEMENT NODE 02 IN Y DIR 
            dispZ.append(ops.nodeDisp(center_node, 3))                      # DISPLACEMENT NODE 02 IN Z DIR 
            ...
            for node_id in node_displacementsX.keys():    
                node_displacementsX[node_id].append(ops.nodeDisp(node_id, 1))
                node_displacementsY[node_id].append(ops.nodeDisp(node_id, 2))     
                node_displacementsZ[node_id].append(ops.nodeDisp(node_id, 3)) 
    ...
    if ANAL_TYPE == 'CYCLIC_DISPLACEMENT': # STATIC TIME-HISTORY ANALYSIS
        ...
        for target_disp in disp_protocol:
            current_disp = ops.nodeDisp(center_node, 3) # DISPALCEMENT APPLIED IN MIDDLE NOD IN X DIR.
    ...
    if ANAL_TYPE == 'STATIC_EXTERNAL_TIME-DEPENDENT_LOADING': # STATIC TIME-HISTORY ANALYSIS
        ...
        ops.load(2, 0.0, 0.0, -1.0) # P(t) = P0 exp(-0.05wt) sin(wt) 
    ...
    if ANAL_TYPE == 'FREE-VIBRATION': # DYNAMIC TIME-HISTORY ANALYSIS
        ...
        if IU == True:
            ...
            ops.setNodeDisp(center_node, 3, u0, '-commit') # INFO LINK: https://openseespydoc.readthedocs.io/en/latest/src/setNodeDisp.html
        if IV == True:
            # Define initial velocity
            ops.setNodeVel(center_node, 3, v0, '-commit')  # INFO LINK: https://openseespydoc.readthedocs.io/en/stable/src/setNodeVel.html
        if IA == True:
            # Define initial  acceleration
            ops.setNodeAccel(center_node, 3, a0, '-commit') # INFO LINK: https://openseespydoc.readthedocs.io/en/latest/src/setNodeAccel.html
...
DATA = THREE_ELEMENTS_3D_TRUSS(LENGTH, AREA, MAT_TYPE, ELE_TYPE, ANAL_TYPE, TOTAL_MASS)
(PERIOD_MIN_X, PERIOD_MAX_X) = DATA
print('Structure First Period:  ', PERIOD_MIN_X)
print('Structure Second Period: ', PERIOD_MAX_X) 

S01.PLOT_3D_TRUSS(deformed_scale=1.0)
#%%----------------------------------------------------
# STATIC ANALYSIS
#ELE_TYPE = 'Truss'      # MATERIAL NONLINEARITY
ELE_TYPE = 'corotTruss'  # MATERIAL AND GEOMETRIC NONLINEARITIES
...
TITLE = "elements  force vs Time for Truss Element During Dynamic External Time-dependent Loading Analysis"
PLOT_FORCES(time_ETDLD, ele_axialforce_ETDLD, YLABEL, TITLE) # ELEMENTS AXIAL FORCE
# PLOT ELEMENTS STRESS
YLABEL = 'Element Stress [N/mm^2]'  
TITLE = "elements  Stress vs Time for Truss Element During Dynamic External Time-dependent Loading Analysis"
PLOT_FORCES(time_ETDLD, ele_stress_ETDLD, YLABEL, TITLE) # ELEMENTS STRESS
# PLOT ELEMENTS STRAIN
YLABEL = 'Element Strain [mm]'  
TITLE = "elements  Strain vs Time for Truss Element During Dynamic External Time-dependent Loading Analysis"
...
TITLE = "elements Damage-index [%] vs Time for Truss Element During Dynamic External Time-dependent Loading Analysis"
...
# PLOT ELEMENTS AXIAL FORCE
YLABEL = 'Element Axial Force [N]'
TITLE = "elements  force vs Time for Truss Element During Free-vibration Analysis"
PLOT_FORCES(time_FV, ele_axialforce_FV, YLABEL, TITLE) # ELEMENTS AXIAL FORCE
# PLOT ELEMENTS STRESS
YLABEL = 'Element Stress [N/mm^2]'  
TITLE = "elements  Stress vs Time for Truss Element During Free-vibration Analysis"
PLOT_FORCES(time_FV, ele_stress_FV, YLABEL, TITLE) # ELEMENTS STRESS
# PLOT ELEMENTS STRAIN
YLABEL = 'Element Strain [mm]'  
TITLE = "elements  Strain vs Time for Truss Element During Free-vibration Analysis"
PLOT_FORCES(time_FV, ele_strain_FV, YLABEL, TITLE) # ELEMENTS AXIAL STRAIN
# PLOT ELEMENTS DAMAGE-INDEX
YLABEL = 'Element Damage-index [%]'  
TITLE = "elements Damage-index [%] vs Time for Truss Element During Free-vibration Analysis"
...
# PLOT ELEMENTS AXIAL FORCE
YLABEL = 'Element Axial Force [N]'
TITLE = "elements  force vs Time for Truss Element During Seismic Analysis"
PLOT_FORCES(time_SEI, ele_axialforce_SEI, YLABEL, TITLE) # ELEMENTS AXIAL FORCE
# PLOT ELEMENTS STRESS
YLABEL = 'Element Stress [N/mm^2]'  
TITLE = "elements  Stress vs Time for Truss Element During Seismic Analysis"
PLOT_FORCES(time_SEI, ele_stress_SEI, YLABEL, TITLE) # ELEMENTS STRESS
# PLOT ELEMENTS STRAIN
YLABEL = 'Element Strain [mm]'  
TITLE = "elements  Strain vs Time for Truss Element During Seismic Analysis"
PLOT_FORCES(time_SEI, ele_strain_SEI, YLABEL, TITLE) # ELEMENTS AXIAL STRAIN
# PLOT ELEMENTS DAMAGE-INDEX
YLABEL = 'Element Damage-index [%]'  
TITLE = "elements Damage-index [%] vs Time for Truss Element During Seismic Analysis"
...
# Save to Excel
with pd.ExcelWriter("P(t)_3D_TRUSS_THREE_ELEMENTS_OUTPUT.xlsx", engine='openpyxl') as writer:
