def PLOT_3D_TRUSS(deformed_scale=1.0):
    """
    Plot 3D Truss geometry with undeformed (black) and deformed (red dashed) shapes from OpenSees model.
    ...
    for i, ele in enumerate(ops.getEleTags()):
        ...
        x1, y1, z1 = node_coords[node1]
        x2, y2, z2 = node_coords[node2]
        ux1, uy1, uz1 = ops.nodeDisp(node1)
        ux2, uy2, uz2 = ops.nodeDisp(node2)
        ax.plot(
    ...
    ax.set_title(f"3D Truss: Undeformed vs Deformed (Scale = {deformed_scale:.2f})", fontsize=14)
    ax.legend()
    ax.grid(True)
