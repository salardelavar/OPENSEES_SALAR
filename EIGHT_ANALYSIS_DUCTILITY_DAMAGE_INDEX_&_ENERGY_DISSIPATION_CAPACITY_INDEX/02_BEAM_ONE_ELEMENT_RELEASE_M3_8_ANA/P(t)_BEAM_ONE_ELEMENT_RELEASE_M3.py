...
#       COMPREHENSIVE NONLINEAR SEISMIC ASSESSMENT OF STEEL A BEAM ELEMENT : AN OPENSEES FRAMEWORK FOR    #
...
###########################################################################################################
"""
Nonlinear Seismic Performance Assessment of a Beam Element:
An OpenSeesPy Framework for Material and Geometric Nonlinearity Under Static, Cyclic, and Earthquake Loading

This OpenSeesPy script performs rigorous nonlinear static and dynamic analysis of a Beam Element
...
import STEEL_FIBER_SECTION as S08
...
import DOUBLE_I_STEEL_SECTION_DIFF_WIDTH_HEIGHT_FUN_EXTRA as S11
import I_STEEL_PLATE_SECTION_DIFF_WIDTH_HEIGHT_FUN_EXTRA as S12
import STEEL_BOX_SECTION_DIFF_WIDTH_HEIGHT_FUN_EXTRA as S13
import STEEL_CORNER_BOX_SECTION_DIFF_WIDTH_HEIGHT_FUN_EXTRA as S14
import STEEL_SECTION_DIFF_WIDTH_HEIGHT_FUN_EXTRA as S15
import STEEL_UNP_BOX_SECTION_DIFF_WIDTH_HEIGHT_FUN_EXTRA as S16
import CROSS_SHAPED_STEEL_SECTION_DIFF_WIDTH_HEIGHT_FUN_EXTRA as S17
import PLATE_DOUBLE_I_STEEL_SECTION_DIFF_WIDTH_HEIGHT_FUN_EXTRA as S18
#%%----------------------------------------------------
def BEAM_ONE_ELEMENT(LENGTH, MAT_TYPE, ELE_TYPE, ANAL_TYPE, TOTAL_MASS):
    # Initialize model
    ops.wipe()
    ...
    #%% SUPPORTS
    ops.fix(1, 1, 1, 1)      # left  - fixed
    ops.fix(2, 0, 1, 1)      # right - rolled-fixed
    
    # SECTION
    DENSITY_STEEL = 7850/1e9      # [kg/m^3] -> [kg/mm^3] Steel Material Density
    SEC_TAG = 1

    #Depth, Ele_Mass = S11.STEEL_SECTION_DIFF_WIDTH_HEIGHT_FUN_EXTRA(SEC_TAG, MAT_TYPE, DENSITY_STEEL, plot=True)
    Depth, Ele_Mass = S12.I_STEEL_PLATE_SECTION_DIFF_WIDTH_HEIGHT_FUN_EXTRA(SEC_TAG, MAT_TYPE, DENSITY_STEEL, plot=True)
    #Depth, Ele_Mass = S13.STEEL_BOX_SECTION_DIFF_WIDTH_HEIGHT_FUN_EXTRA(SEC_TAG, MAT_TYPE, DENSITY_STEEL, plot=True)
    #Depth, Ele_Mass = S14.STEEL_CORNER_BOX_SECTION_DIFF_WIDTH_HEIGHT_FUN_EXTRA(SEC_TAG, MAT_TYPE, DENSITY_STEEL, plot=True)
    #Depth, Ele_Mass = S15.STEEL_SECTION_DIFF_WIDTH_HEIGHT_FUN_EXTRA(SEC_TAG, MAT_TYPE, DENSITY_STEEL, plot=True)
    #Depth, Ele_Mass = S16.STEEL_UNP_BOX_SECTION_DIFF_WIDTH_HEIGHT_FUN_EXTRA(SEC_TAG, MAT_TYPE, DENSITY_STEEL, plot=True)
    #Depth, Ele_Mass = S17.STEEL_SECTION_DIFF_WIDTH_HEIGHT_FUN_EXTRA(SEC_TAG, MAT_TYPE, DENSITY_STEEL, plot=True)
    #Depth, Ele_Mass = S18.STEEL_SECTION_DIFF_WIDTH_HEIGHT_FUN_EXTRA(SEC_TAG, MAT_TYPE, DENSITY_STEEL, plot=True)
    #%% CREATE ELEMENTS
    ele_tag = 1
    transfTag = 1
    ...
    if ELE_TYPE == 'elasticBeamColumn':
        ops.element('elasticBeamColumn', 1, 1, 2, numIntgrPts, SEC_TAG, transfTag,'-mass', Ele_Mass, '-release', 3)
    if ELE_TYPE == 'nonlinearBeamColumn': 
        ops.element('nonlinearBeamColumn', 1, 1, 2, numIntgrPts, SEC_TAG, transfTag,'-mass', Ele_Mass, '-release', 3) 
    #%%--------------------------------------
    """
    ...
    ops.load(center_node, 1.0, 0.0, 0.0)   # [N] downward
    ...
    if ANAL_TYPE == 'STATIC': 
        ...
        for ele_id in ele_axialforce.keys(): 
            ele_axialforce[ele_id].append(ops.eleResponse(ele_id, 'force')[0])       # [N] ELEMENT FORCE
...
DATA = BEAM_ONE_ELEMENT(LENGTH, MAT_TYPE, ELE_TYPE, ANAL_TYPE, TOTAL_MASS)
...
PLOT_FORCES(time_ETDLD, ele_force_ETDLD, YLABEL, TITLE) # Beam Element - ELEMENTS AXIAL FORCE
# PLOT ELEMENTS STRESS
YLABEL = 'Element Stress [N/mm^2]'  
TITLE = "elements  Stress in X Dir. vs Time for Beam Element During Dynamic External Time-dependent Loading Analysis"
#PLOT_FORCES(time_ETDLD, ele_stress_ETDLD, YLABEL, TITLE) # Beam Element - ELEMENTS STRESS
# PLOT ELEMENTS STRAIN
YLABEL = 'Element Strain [mm]'  
TITLE = "elements  Strain in X Dir. vs Time for Beam Element During Dynamic External Time-dependent Loading Analysis"
#PLOT_FORCES(time_ETDLD, ele_strain_ETDLD, YLABEL, TITLE) # Beam Element - ELEMENTS AXIAL FORCE
# PLOT ELEMENTS DAMAGE-INDEX
YLABEL = 'Element Damage-index [%]'  
TITLE = "elements Damage-index [%] in X Dir. vs Time for Beam Element During Dynamic External Time-dependent Loading Analysis"
#PLOT_FORCES(time_ETDLD, ele_strain_ETDLD, YLABEL, TITLE) # Beam Element - ELEMENTS AXIAL FORCE
...
PLOT_FORCES(time_FV, ele_force_FV, YLABEL, TITLE) # Beam Element - ELEMENTS AXIAL FORCE
# PLOT ELEMENTS STRESS
YLABEL = 'Element Stress [N/mm^2]'  
TITLE = "elements  Stress in X Dir. vs Time for Beam Element During Free-vibration Analysis"
#PLOT_FORCES(time_FV, ele_stress_FV, YLABEL, TITLE) # Beam Element - ELEMENTS STRESS
# PLOT ELEMENTS STRAIN
YLABEL = 'Element Strain [mm]'  
TITLE = "elements  Strain in X Dir. vs Time for Beam Element During Free-vibration Analysis"
#PLOT_FORCES(time_FV, ele_strain_FV, YLABEL, TITLE) # Beam Element - ELEMENTS AXIAL FORCE
# PLOT ELEMENTS DAMAGE-INDEX
YLABEL = 'Element Damage-index [%]'  
TITLE = "elements Damage-index [%] in X Dir. vs Time for Beam Element During Free-vibration Analysis"
#PLOT_FORCES(time_FV, ele_strain_FV, YLABEL, TITLE) # Beam Element - ELEMENTS AXIAL FORCE
...
# PLOT ELEMENTS STRESS
YLABEL = 'Element Stress [N/mm^2]'  
TITLE = "elements  Stress in X Dir. vs Time for Beam Element During Seismic Analysis"
#PLOT_FORCES(time_SEI, ele_stress_SEI, YLABEL, TITLE) # Beam Element - ELEMENTS STRESS
# PLOT ELEMENTS STRAIN
YLABEL = 'Element Strain [mm]'  
TITLE = "elements  Strain in X Dir. vs Time for Beam Element During Seismic Analysis"
#PLOT_FORCES(time_SEI, ele_strain_SEI, YLABEL, TITLE) # Beam Element - ELEMENTS AXIAL FORCE
# PLOT ELEMENTS DAMAGE-INDEX
YLABEL = 'Element Damage-index [%]'  
TITLE = "elements Damage-index [%] in X Dir. vs Time for Beam Element During Seismic Analysis"
#PLOT_FORCES(time_SEI, ele_strain_SEI, YLABEL, TITLE) # Beam Element - ELEMENTS AXIAL FORCE
...
# Save to Excel
with pd.ExcelWriter("BEAM_ONE_ELEMENT_RELEASE_M3_OUTPUT.xlsx", engine='openpyxl') as writer:
