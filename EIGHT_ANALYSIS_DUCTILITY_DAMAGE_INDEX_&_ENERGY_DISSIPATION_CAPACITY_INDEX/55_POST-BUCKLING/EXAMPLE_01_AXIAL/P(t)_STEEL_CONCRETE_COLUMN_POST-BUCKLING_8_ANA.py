...
def POST_BUCKLING_COLUMN(LENGTH, MAT_TYPE, ELE_TYPE, ANAL_TYPE, TOTAL_MASS, MODE_NUM, SEMI_RIGID):
    ...
    if ANAL_TYPE == 'SEISMIC': # DYNAMIC TIME-HISTORY ANALYSIS
        ...
        while stable == 0 and current_time < duration:
            ...
            veloX.append(ops.nodeVel(middle_node, 1))             # VELOCITY NODE
            veloY.append(ops.nodeVel(middle_node, 2))             # VELOCITY NODE
            accX.append(ops.nodeAccel(middle_node, 1))            # ACCELERATION NODE 07
            accY.append(ops.nodeAccel(middle_node, 2))            # ACCELERATION NODE 07
            stiffness.append(np.abs(reactionX[-1]) / np.abs(disp_mid[-1]))
            OMEGA.append(np.sqrt(stiffness[-1]/TOTAL_MASS))
            PERIOD.append((np.pi * 2) / OMEGA[-1])
            # Store elements force, strain and stress
...
stiffness_ETDLD, PERIOD_ETDLD, damping_ratio_ETDLD_X, damping_ratio_ETDLD_Y,
...
method = 1 # 
zeta = S055.EQULIVALENT_VISCOUS_DAMPING_RATIO_FUN(dispY_ETDLD, reactionY_ETDLD, method, XLABEL, YLABEL, TITLE)
print(f"Equivalent viscous damping ratio = {zeta:.4f}")
...
XLABEL = 'Displacement in X Dir. [mm]'
YLABEL = 'Base Shear Reaction [N]'
TITLE = f'Base Shear Reaction and Displacement of Structure During Free-vibration Analysis\n Damping Ratio in X Dir: {damping_ratio_FV_X:.3e} [%]'
COLOR = 'black'
SEMILOGY = False
PLOT(XDATA, YDATA, TITLE, XLABEL, YLABEL, COLOR, SEMILOGY)
...
XLABEL = 'Displacement in Y Dir. [mm]'
YLABEL = 'Base Axial Reaction [N]'
TITLE = f'Base Axial Reaction and Dispalcement of Structure During Free-vibration Analysis\n Damping Ratio in Y Dir: {damping_ratio_FV_Y:.3e} [%]'
COLOR = 'black'
SEMILOGY = False
PLOT(XDATA, YDATA, TITLE, XLABEL, YLABEL, COLOR, SEMILOGY)
...
(time_SEI, reactionX_SEI, reactionY_SEI, reactionZ_SEI, disp_mid_SEI,
 ...
 veloX_SEI, veloY_SEI,
 accX_SEI, accY_SEI,
 stiffness_SEI, PERIOD_SEI, damping_ratio_SEI_X, damping_ratio_SEI_Y,
