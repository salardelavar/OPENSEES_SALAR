...
#COMPREHENSIVE NONLINEAR SEISMIC ASSESSMENT OF A SIMPLY SUPPORTED POST-TENSION CABLE ELEMENT : AN OPENSEES#
#FRAMEWORK FOR STATIC PUSHOVER, CYCLIC DEGRADATION, STATIC TIME-HISTORY AND DYNAMIC TIME-HISTORY ANALYSIS #
#---------------------------------------------------------------------------------------------------------#
#                    EFFECTS OF POST-TENSION CABLE WITH INTIAL STRAIN OR INTIAL STRESS                    #
#---------------------------------------------------------------------------------------------------------#
...
###########################################################################################################
"""
Nonlinear Seismic Performance Assessment of a Cable Element:
An OpenSeesPy Framework for Material and Geometric Nonlinearity Under Static, Cyclic, and Earthquake Loading

This OpenSeesPy script performs rigorous nonlinear static and dynamic analysis of a Cable Element
...
"""
# BOOK: Advanced Analysis And Design Of Steel Frames-Guo-Qiang Li-WILEY-2007
# BOOK: Analysis and Design of Steel and Composite Structures-Qing Quan Liang-CRC Press-2015
# BOOK: Handbook of strcutural steel Connection Design and Detail_Akbar R Tamboli
# BOOK: Non-Linear Static and Cyclic Analysis of Steel Frames with Semi-Rigid Connections-S.L. Chan and P.P.T. Chui-Elsevier-2000
# BOOK: Plastic Design and Second-Order Analysis of Steel Frames-W. F. Chen, I. Sohal-Springer New York-1995
# BOOK: Seismic Design of Steel Structures-Victor Gioncu & Federico Mazzolani-CRC Press-2014    
...
#%%----------------------------------------------------
def POST_TENSION_CABLE_ONE_ELEMENT_SIMPLY_SUPPORTED(LENGTH, AREA, MAT_TYPE, ELE_TYPE, ANAL_TYPE, TOTAL_MASS):
    # Initialize model
    ops.wipe()
    ...
    ele_mass = AREA * DENSITY_STEEL # [kg/mm]
    ...
    if MAT_TYPE == 'INELASTIC':
        Fy = 1200.0			    # [N/mm²] Steel yield stress
        Es = 200000.0		    # [N/mm²] Modulus of steel
        ey = Fy/Es			    # [mm/mm] Steel yield strain
        Fu = 1.1273*Fy          # [N/mm²] Steel Ultimate Strength
        esu = 0.06              # [mm/mm] Steel Ultimate Strain
        Esh = (Fu - Fy)/(esu - ey)
        Bs = Esh / Es           # strain-hardening ratio 
        pinchX = 1.0            # Pinching factor in X direction
        pinchY = 1.0            # Pinching factor in Y direction
        damage1 = 0.0           # Damage due to ductility
        damage2 = 0.0           # Damage due to energy
        beta = 0.0              # Stiffness degradation parameter
        ops.uniaxialMaterial('Hysteretic', MAT_TAG, Fy, ey,
                                                    Fu, esu,
                                                    0.2*Fu, 1.1*esu,
                                                    -Fy, -ey,
                                                    -Fu, -esu,
    ...
    # PRESTRAIN CABLE
    MAT_BASE = 2              # Prestrain Cable Section Tag
    P_pre = 500000.0          # [N] AXIAL PRESSURE ON CABLE
    sigma0 = P_pre / AREA     # [N/mm²] AXIAL STRESS
    eps0 = sigma0 / Es        # [mm/mm] AXIAL STRAIN - Initial strain (Tension Postive Value - Compression Negaive Value)
    # uniaxialMaterial InitStrainMaterial $matTag $otherTag $initStrain
    #ops.uniaxialMaterial('InitStrainMaterial', MAT_BASE, MAT_TAG, eps0)  
    # uniaxialMaterial InitStressMaterial $matTag $otherTag $initStress
    ops.uniaxialMaterial('InitStressMaterial', MAT_BASE, MAT_TAG, sigma0)
    #%% GEOMETRY DEFINITION 
    ops.node(1, 0.0, 0.0)
    
    ops.node(2, (1/6)*LENGTH, -(1.5/600)*LENGTH)
    ops.node(3, (2/6)*LENGTH, -(2.5/600)*LENGTH)
    ops.node(4, (3/6)*LENGTH, -(3.0/600)*LENGTH)
    ops.node(5, (4/6)*LENGTH, -(2.5/600)*LENGTH)
    ops.node(6, (5/6)*LENGTH, -(1.5/600)*LENGTH)
    """
    ops.node(2, (1/6)*LENGTH, 0.0)
    ops.node(3, (2/6)*LENGTH, 0.0)
    ops.node(4, (3/6)*LENGTH, 0.0)
    ops.node(5, (4/6)*LENGTH, 0.0)
    ops.node(6, (5/6)*LENGTH, 0.0)
    """
    ops.node(7, (6/6)*LENGTH, 0.0)
    #%% SUPPORTS
    ops.fix(1, 1, 1)      # left  - pinned
    ops.fix(7, 1, 1)      # right - pinned
    #%% CREATE ELEMENTS
    ele_tag = 1
    ...
    if ELE_TYPE == 'corotTruss':
        ...
        ops.element('corotTruss', 1, 1, 2, AREA, MAT_BASE, '-rho', ele_mass, '-doRayleigh', 1)
        ops.element('corotTruss', 2, 2, 3, AREA, MAT_BASE, '-rho', ele_mass, '-doRayleigh', 1)
        ops.element('corotTruss', 3, 3, 4, AREA, MAT_BASE, '-rho', ele_mass, '-doRayleigh', 1)
        ops.element('corotTruss', 4, 4, 5, AREA, MAT_BASE, '-rho', ele_mass, '-doRayleigh', 1)
        ops.element('corotTruss', 5, 5, 6, AREA, MAT_BASE, '-rho', ele_mass, '-doRayleigh', 1)
        ops.element('corotTruss', 6, 6, 7, AREA, MAT_BASE, '-rho', ele_mass, '-doRayleigh', 1)
    ...
    if ELE_TYPE == 'Truss': 
        ...
        ops.element('Truss', 1, 1, 2, AREA, MAT_BASE, '-rho', ele_mass, '-doRayleigh', 1)
        ops.element('Truss', 2, 2, 3, AREA, MAT_BASE, '-rho', ele_mass, '-doRayleigh', 1)
        ops.element('Truss', 3, 3, 4, AREA, MAT_BASE, '-rho', ele_mass, '-doRayleigh', 1)
        ops.element('Truss', 4, 4, 5, AREA, MAT_BASE, '-rho', ele_mass, '-doRayleigh', 1)
        ops.element('Truss', 5, 5, 6, AREA, MAT_BASE, '-rho', ele_mass, '-doRayleigh', 1)
        ops.element('Truss', 6, 6, 7, AREA, MAT_BASE, '-rho', ele_mass, '-doRayleigh', 1)
    ...
    if ANAL_TYPE == 'STATIC': 
        ...
        ops.reactions()
        reaction.append(ops.nodeReaction(1, 2)+ops.nodeReaction(7, 2))  # SHEAR BASE REACTION        
        disp_mid.append(ops.nodeDisp(center_node, 2))                   # DISPLACEMENT NODE 02 
        dispX.append(ops.nodeDisp(center_node, 1))                      # DISPLACEMENT NODE 04 IN X DIR 
        dispY.append(ops.nodeDisp(center_node, 2))                      # DISPLACEMENT NODE 04 IN Y DIR 
    ...
    if ANAL_TYPE == 'PUSHOVER': # STATIC TIME-HISTORY ANALYSIS
        ...
        DMAX = -1000.0  # [mm] Max. Vertical Displacement
        ...
        for step in range(Nsteps):
            ...
            ops.reactions()
            reaction.append(ops.nodeReaction(1, 2)+ops.nodeReaction(7, 2))  # SHEAR BASE REACTION            
            disp_mid.append(ops.nodeDisp(center_node, 2))                   # DISPLACEMENT NODE 04 
            dispX.append(ops.nodeDisp(center_node, 1))                      # DISPLACEMENT NODE 04 IN X DIR 
            dispY.append(ops.nodeDisp(center_node, 2))                      # DISPLACEMENT NODE 04 IN Y DIR 
    ...
    if ANAL_TYPE == 'DYNAMIC_EXTERNAL_TIME-DEPENDENT_LOADING': # DYNAMIC TIME-HISTORY ANALYSIS
        ...
        while stable == 0 and current_time < duration:
            ...
            dispX.append(ops.nodeDisp(center_node, 1))            # DISPLACEMENT NODE 04 IN X DIR 
            dispY.append(ops.nodeDisp(center_node, 2))            # DISPLACEMENT NODE 04 IN Y DIR 
            veloX.append(ops.nodeVel(center_node, 1))             # VELOCITY NODE 04
            veloY.append(ops.nodeVel(center_node, 2))             # VELOCITY NODE 04
            accX.append(ops.nodeAccel(center_node, 1))            # ACCELERATION NODE 04
            accY.append(ops.nodeAccel(center_node, 2))            # ACCELERATION NODE 04
    ...
    if ANAL_TYPE == 'SEISMIC': # DYNAMIC TIME-HISTORY ANALYSIS
        ...
        SSF_X = 0.50                       # Seismic Acceleration Scale Factor in X Direction
        SSF_Y = 0.50                       # Seismic Acceleration Scale Factor in Y Direction
...
def PLOT_2D(X, Y, Xfit, Yfit, X2, Y2, XLABEL, YLABEL, TITLE, LEGEND01, LEGEND02, LEGEND03, COLOR, Z):
    ...
    if Z == 3:
        ...
        plt.show() 
#%%----------------------------------------------------
LENGTH = 10000.0          # [mm] Beam Length
DIA = 120.0               # [mm] Cable Diameter
AREA  = np.pi*(DIA**2)/4  # [mm^2] Section Area for diagonal elements
...
DATA = POST_TENSION_CABLE_ONE_ELEMENT_SIMPLY_SUPPORTED(LENGTH, AREA, MAT_TYPE, ELE_TYPE, ANAL_TYPE, TOTAL_MASS)
...
method = 1 # 
zeta = S055.EQULIVALENT_VISCOUS_DAMPING_RATIO_FUN(disp_mid_CP, reaction_CP, method, XLABEL, YLABEL, TITLE)
...
TITLE = "elements  force in X Dir. vs Time for Cable Element During Dynamic External Time-dependent Loading Analysis"
PLOT_FORCES(time_ETDLD, ele_axialforce_ETDLD, YLABEL, TITLE) # Cable Element - ELEMENTS AXIAL FORCE
# PLOT ELEMENTS STRESS
YLABEL = 'Element Stress [N/mm^2]'  
TITLE = "elements  Stress in X Dir. vs Time for Cable Element During Dynamic External Time-dependent Loading Analysis"
PLOT_FORCES(time_ETDLD, ele_stress_ETDLD, YLABEL, TITLE) # Cable Element - ELEMENTS STRESS
# PLOT ELEMENTS STRAIN
YLABEL = 'Element Strain [mm]'  
TITLE = "elements  Strain in X Dir. vs Time for Cable Element During Dynamic External Time-dependent Loading Analysis"
PLOT_FORCES(time_ETDLD, ele_strain_ETDLD, YLABEL, TITLE) # Cable Element - ELEMENTS AXIAL FORCE
# PLOT ELEMENTS DAMAGE-INDEX
YLABEL = 'Element Damage-index [%]'  
TITLE = "elements Damage-index [%] in X Dir. vs Time for Cable Element During Dynamic External Time-dependent Loading Analysis"
PLOT_FORCES(time_ETDLD, ele_strain_ETDLD, YLABEL, TITLE) # Cable Element - ELEMENTS AXIAL FORCE

# PLOT NODAL DISPALEMENTS
PLOT_DISPLAEMENTS(time_ETDLD, node_displacementsX_ETDLD, TITLE = "Node Displacements in X Dir. vs Time for Cable Element During Dynamic External Time-dependent Loading Analysis") # Cable Element IN X DIR.
PLOT_DISPLAEMENTS(time_ETDLD, node_displacementsY_ETDLD, TITLE = "Node Displacements in Y Dir. vs Time for Cable Element During Dynamic External Time-dependent Loading Analysis") # Cable Element IN Y DIR.
...
# PLOT ELEMENTS AXIAL FORCE
YLABEL = 'Element Axial Force [N]'
TITLE = "elements  force in X Dir. vs Time for Cable Element During Free-vibration Analysis"
PLOT_FORCES(time_FV, ele_axialforce_FV, YLABEL, TITLE) # Cable Element - ELEMENTS AXIAL FORCE
# PLOT ELEMENTS STRESS
YLABEL = 'Element Stress [N/mm^2]'  
TITLE = "elements  Stress in X Dir. vs Time for Cable Element During Free-vibration Analysis"
PLOT_FORCES(time_FV, ele_stress_FV, YLABEL, TITLE) # Cable Element - ELEMENTS STRESS
# PLOT ELEMENTS STRAIN
YLABEL = 'Element Strain [mm]'  
TITLE = "elements  Strain in X Dir. vs Time for Cable Element During Free-vibration Analysis"
PLOT_FORCES(time_FV, ele_strain_FV, YLABEL, TITLE) # Cable Element - ELEMENTS AXIAL FORCE
# PLOT ELEMENTS DAMAGE-INDEX
YLABEL = 'Element Damage-index [%]'  
TITLE = "elements Damage-index [%] in X Dir. vs Time for Cable Element During Free-vibration Analysis"
PLOT_FORCES(time_FV, ele_strain_FV, YLABEL, TITLE) # Cable Element - ELEMENTS AXIAL FORCE

# PLOT NODAL DISPALEMENTS
PLOT_DISPLAEMENTS(time_FV, node_displacementsX_FV, TITLE = "Node Displacements in X Dir. vs Time for Cable Element During Free-vibration Analysis") # Cable Element IN X DIR.
PLOT_DISPLAEMENTS(time_FV, node_displacementsY_FV, TITLE = "Node Displacements in Y Dir. vs Time for Cable Element During Free-vibration Analysis") # Cable Element IN Y DIR.
...
method = 2 # 
zeta = S055.EQULIVALENT_VISCOUS_DAMPING_RATIO_FUN(disp_mid_FV, reaction_FV, method, XLABEL, YLABEL, TITLE)
print(f"Equivalent viscous damping ratio = {zeta:.4f}")
...
# PLOT ELEMENTS AXIAL FORCE
YLABEL = 'Element Axial Force [N]'
TITLE = "elements  force in X Dir. vs Time for Cable Element During Seismic Analysis"
PLOT_FORCES(time_SEI, ele_axialforce_SEI, YLABEL, TITLE) # Cable Element - ELEMENTS AXIAL FORCE
# PLOT ELEMENTS STRESS
YLABEL = 'Element Stress [N/mm^2]'  
TITLE = "elements  Stress in X Dir. vs Time for Cable Element During Seismic Analysis"
PLOT_FORCES(time_SEI, ele_stress_SEI, YLABEL, TITLE) # Cable Element - ELEMENTS STRESS
# PLOT ELEMENTS STRAIN
YLABEL = 'Element Strain [mm]'  
TITLE = "elements  Strain in X Dir. vs Time for Cable Element During Seismic Analysis"
PLOT_FORCES(time_SEI, ele_strain_SEI, YLABEL, TITLE) # Cable Element - ELEMENTS AXIAL FORCE
# PLOT ELEMENTS DAMAGE-INDEX
YLABEL = 'Element Damage-index [%]'  
TITLE = "elements Damage-index [%] in X Dir. vs Time for Cable Element During Seismic Analysis"
PLOT_FORCES(time_SEI, ele_strain_SEI, YLABEL, TITLE) # Cable Element - ELEMENTS AXIAL FORCE

# PLOT NODAL DISPALEMENTS
PLOT_DISPLAEMENTS(time_SEI, node_displacementsX_SEI, TITLE = "Node Displacements in X Dir. vs Time for Cable Element During Seismic Analysis") # Cable Element IN X DIR.
PLOT_DISPLAEMENTS(time_SEI, node_displacementsY_SEI, TITLE = "Node Displacements in Y Dir. vs Time for Cable Element During Seismic Analysis") # Cable Element IN Y DIR.
...
method = 1 # 
zeta = S055.EQULIVALENT_VISCOUS_DAMPING_RATIO_FUN(disp_mid_SEI, reaction_SEI, method, XLABEL, YLABEL, TITLE)
print(f"Equivalent viscous damping ratio = {zeta:.4f}")
...
LEGEND03 = 'Undefined'
TITLE = f'Last Data of Base-Shear Reaction-Displacement Analysis - Ductility Ratio: {X[2]/X[1]:.4f} - Over Strength Factor: {Y[2]/Y[1]:.4f}'
COLOR = 'black'
...
# Save to Excel
with pd.ExcelWriter("W(x,t)_POST-TENSION_CABLE_SIMPLY_SUPPORTED_OUTPUT.xlsx", engine='openpyxl') as writer:
