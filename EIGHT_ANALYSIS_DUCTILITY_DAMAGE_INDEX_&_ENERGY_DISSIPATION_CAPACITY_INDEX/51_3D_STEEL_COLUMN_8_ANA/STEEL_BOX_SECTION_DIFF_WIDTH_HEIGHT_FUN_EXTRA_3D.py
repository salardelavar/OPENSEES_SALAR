def STEEL_BOX_SECTION_DIFF_WIDTH_HEIGHT_FUN_EXTRA(secTag, STEEL_TYPE,STEEL_DENSITY, plot=True):
    ...
    mat_layers = [
        (PT, CD+2*PT, CD+0.5*PT, 0.5*CD, NFY, NFX, steelITag, 'cyan'),              # 01 -> BOX
        (PT, CD+2*PT, 0.0-0.5*PT, 0.5*CD, NFY, NFX, steelITag, 'cyan'),             # 02 -> BOX
        (CD, PT, 0.5*CD, CD+0.5*PT, NFY, NFX, steelITag, 'cyan'),                   # 03 -> BOX   
        (CD, PT, 0.5*CD, -0.5*PT, NFY, NFX, steelITag, 'cyan'),                     # 04 -> BOX
    ]
    ...
    if plot:
        ...
        ax.set_title(f'Steel Box section with rebars and plates (Numbers shown) - Section Tag: {secTag}')
