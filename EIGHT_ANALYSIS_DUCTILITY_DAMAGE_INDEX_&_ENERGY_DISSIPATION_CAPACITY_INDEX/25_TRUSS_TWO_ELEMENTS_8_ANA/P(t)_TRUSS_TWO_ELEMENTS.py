...
#          COMPREHENSIVE NONLINEAR SEISMIC ASSESSMENT OF TWO ELEMENTS TRUSS : AN OPENSEES                 #
...
###########################################################################################################
"""
Nonlinear Seismic Performance Assessment of Two Elements Truss:
An OpenSeesPy Framework for Material and Geometric Nonlinearity Under Static, Cyclic, and Earthquake Loading

This OpenSeesPy script performs rigorous nonlinear static and dynamic analysis of Two Elements Truss
...
#%%----------------------------------------------------
def TWO_ELEMENTS_TRUSS(LENGTH, AREA, MAT_TYPE, ELE_TYPE, ANAL_TYPE, TOTAL_MASS):
    # Initialize model
    ops.wipe()
    ...
    #%% GEOMETRY DEFINITION
    ops.node(1, 0.0, 0.0)
    ops.node(2, (1/2)*LENGTH, (1/2)*LENGTH)
    ops.node(3, (2/2)*LENGTH, 0.0)
    #%% SUPPORTS
    ops.fix(1, 1, 1)      # left  - pinned
    ops.fix(3, 1, 1)      # right - pinned
    #%% CREATE ELEMENTS
    ele_tag = 1
    ...
    if ANAL_TYPE == 'STATIC': 
        ...
        ops.reactions()
        reaction.append(ops.nodeReaction(1, 2)+ops.nodeReaction(3, 2))  # SHEAR BASE REACTION        
    ...
    if ANAL_TYPE == 'STATIC_EXTERNAL_TIME-DEPENDENT_LOADING': # STATIC TIME-HISTORY ANALYSIS
        ...
        ops.load(2, 0.0, -1.0) # P(t) = P0 exp(-0.05wt) sin(wt) 
...
DATA = TWO_ELEMENTS_TRUSS(LENGTH, AREA, MAT_TYPE, ELE_TYPE, ANAL_TYPE, TOTAL_MASS)
...
# Save to Excel
with pd.ExcelWriter("P(t)_TRUSS_TWO_ELEMENTS_OUTPUT.xlsx", engine='openpyxl') as writer:
