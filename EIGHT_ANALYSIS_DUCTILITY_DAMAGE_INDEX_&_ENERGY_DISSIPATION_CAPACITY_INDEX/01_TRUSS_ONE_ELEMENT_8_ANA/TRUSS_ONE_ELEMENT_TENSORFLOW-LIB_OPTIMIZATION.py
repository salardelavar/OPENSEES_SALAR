...
#      COMPREHENSIVE NONLINEAR SEISMIC ASSESSMENT OF A STEEL TRUSS ELEMENT : AN OPENSEES FRAMEWORK FOR    #
...
def TRUSS_ONE_ELEMENT(LENGTH, AREA, MAT_TYPE,
    ...
    if ELE_TYPE == 'corotTruss':
        ...
        ops.element('corotTruss', ele_tag, 1, 2, AREA, MAT_TAG, '-rho',AREA*0.00000785 , '-doRayleigh', 1)
    if ELE_TYPE == 'Truss': 
        ...
        ops.element('Truss', ele_tag, 1, 2, AREA, MAT_TAG, '-rho', AREA*0.00000785 , '-doRayleigh', 1)
        ele_tag += 1
    ...
    if ANAL_TYPE == 'PUSHOVER': # STATIC TIME-HISTORY ANALYSIS
        ...
        DMAX = -15.0    # [mm] Max. Vertical Displacement
...
#%% FIND BEST TRUSS ELEMENT SECTION AREA WITH STRUCTURAL PERIOD OPTIMIZATION:
    
# Analysis Durations:
starttime = TI.process_time()
    
# =========================================
# PHASE 1: GENERATE DATASET (SAMPLING)
# =========================================

N_SAMPLES = 300
TARGET_P = 5.10    # [s] Structural Period
TOLERANCE = 1e-2
ITMAX = 3000
...
AREA_samples = np.linspace(AREA_MIN, AREA_MAX, N_SAMPLES).reshape(-1, 1)
PERIOD_samples = []

for A in AREA_samples:
    DATA = TRUSS_ONE_ELEMENT(
        LENGTH, float(A),
        MAT_TYPE, ELE_TYPE, ANAL_TYPE, TOTAL_MASS
    )
    PERIOD_MAXi = np.max(DATA[11]) # WE ARE LOOKING FOR MAX. PERIOD FOR PLASTIC RANGE OF STRUCTURE
    PERIOD_samples.append(np.max(PERIOD_MAXi))

PERIOD_samples = np.array(PERIOD_samples).reshape(-1, 1)
...
model.fit(AREA_samples, PERIOD_samples, epochs=800, verbose=0)

# =========================================
# PHASE 3: OPTIMIZATION (FAST & DIFFERENTIABLE)
# =========================================

X = tf.Variable([[AREA]], dtype=tf.float32)
...
for IT in range(ITMAX):
    ...
    with tf.GradientTape() as tape:
        P = model(X)
        loss = tf.square(P - TARGET_P)

    grad = tape.gradient(loss, X)
    optimizer.apply_gradients([(grad, X)])

    print(f"IT {IT:3d} | AREA = {X.numpy()[0,0]:.6f} | PERIOD = {P.numpy()[0,0]:.6f}")

    if tf.abs(grad)[0,0] < TOLERANCE:
        break

AREA_OPT = X.numpy()[0,0]

# =========================================
# PHASE 4: VERIFY WITH OPENSEES
# =========================================
DATA = TRUSS_ONE_ELEMENT(
    LENGTH, AREA_OPT,
    MAT_TYPE, ELE_TYPE, ANAL_TYPE, TOTAL_MASS
...
PERIOD_VERIFY = np.max(DATA[11])

print("\n=================================")
print(f"OPTIMAL AREA (ML):     {AREA_OPT:.6f}")
print(f"TARGET PERIOD:        {TARGET_P}")
print(f"VERIFIED PERIOD:      {PERIOD_VERIFY:.6f}")
print(f"RELATIVE ERROR [%]:   {100*(PERIOD_VERIFY-TARGET_P)/TARGET_P:.3f}")
