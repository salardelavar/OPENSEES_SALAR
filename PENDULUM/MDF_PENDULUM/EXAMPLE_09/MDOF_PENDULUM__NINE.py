...
def EXTERNAL_TIME_DEPENDENT_LOAD_MDOF(MAT_TYPE, duration, dt):
    ...
    # Add nodes
    ops.node(1, 0.0, 0.0)
    ops.node(2, 1.5*X, 0.0)#-1.0e-8
    ops.node(3, 0.5*X, -0.5*X)
    ops.node(4, 1.0*X, -0.5*X)

    
    
    # Define mass to node 3 and 4
    ops.mass(3, M, M)
    ops.mass(4, M, M)
    ...
    # K∆(P-∆)
    Ele_Tag_01, Ele_Tag_02, Ele_Tag_03 = 100, 200, 300
    ...
    ops.element('corotTruss', Ele_Tag_01, 1, 3, AREA, MAT_TAG, '-rho', AREA * DENSITY_STEEL , '-doRayleigh', 1)
    ops.element('corotTruss', Ele_Tag_02, 2, 4, AREA, MAT_TAG, '-rho', AREA * DENSITY_STEEL , '-doRayleigh', 1)
    ops.element('corotTruss', Ele_Tag_03, 3, 4, AREA*0.1, MAT_TAG, '-rho', AREA*0.1 * DENSITY_STEEL , '-doRayleigh', 1)
    ...
    while stable == 0 and current_time < duration:
        ...
        ops.reactions()
        reactionX.append(ops.nodeReaction(1, 1)+ops.nodeReaction(2, 1)) # BASE REACTION IN X DIR.
        reactionY.append(ops.nodeReaction(1, 2)+ops.nodeReaction(2, 2)) # BASE REACTION IN Y DIR.
        dispX.append(ops.nodeDisp(3, 1))         # DISPLACEMENT IN X DIR.  
        dispY.append(ops.nodeDisp(3, 2))         # DISPLACEMENT IN Y DIR.
        velX.append(ops.nodeVel(3, 1))           # VELOCITY IN X DIR.
        velY.append(ops.nodeVel(3, 2))           # VELOCITY IN Y DIR.
        accelX.append(ops.nodeAccel(3, 1))       # ACCELERATION IN X DIR.
        accelY.append(ops.nodeAccel(3, 2))       # ACCELERATION IN Y DIR.
...
# Print the Model
#printModel()
ops.printModel("-JSON", "-file", "MDOF_PENDULUM_NINE.json")
#%%-------------------------------------------------------------------------------
# EXCEL OUTPUT
import pandas as pd
...
# Save to Excel
with pd.ExcelWriter("MDOF_PENDULUM_NINE_OUTPUT.xlsx", engine='openpyxl') as writer:
