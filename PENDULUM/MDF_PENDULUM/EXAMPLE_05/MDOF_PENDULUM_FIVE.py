...
FY = 85000.0                      # [N.m] Yield Moment of Structure
FU = 1.1 * FY                     # [N.m] Ultimate Moment of Structure
Ke = 4500000.0                    # [N.m/rad] Spring Elastic Stiffness
DY = FY / Ke                      # [rad] Yield Rotation
DSU = 0.36                        # [rad] Ultimate Rotation
Ksh = (FU - FY) / (DSU - DY)      # [N.m/rad] Rotation Hardening Modulus
Kp = FU / DSU                     # [N.m/rad] Spring Plastic Stiffness
...
def EXTERNAL_TIME_DEPENDENT_03(m, acc_record, DT, DT_time): # P(t) = -m * ug**(t)
    ...
    return DT_load
#%% ---------------------------------------------
def EXTERNAL_TIME_DEPENDENT_LOAD_MDOF(SPRING_TYPE, MAT_TYPE, duration, dt):
    # Create model
    ops.wipe()
    # Create a 2D model with 3 DOF per node
    ops.model('Basic', '-ndm', 2, '-ndf', 3)
    
    L1 = 0.5 # [m] Element Length
    L2 = 1.0 # [m] Element Length
    
    # Add nodes
    ops.node(1, 0.0, -0.5*L2)
    ops.node(2, 0.0, -L2)
    
    ops.node(3, L1, 0.0)
    ops.node(4, L1, -L2)
    
    ops.node(11, 0.0, -0.5*L2)#-1.0e-8
    ops.node(22, L1, 0.0)     #-1.0e-8
    ...
    ops.element('elasticBeamColumn', Ele_Tag_01, 1, 2, AREA, Es, Iz, transfTag, '-mass', 0.0001)
    #INFO LINK: https://openseespydoc.readthedocs.io/en/latest/src/elasticBeamColumn.html
    
    Ele_Tag_02 = 200
    transfTag = 2
    ops.geomTransf('Corotational', transfTag)
    ops.element('elasticBeamColumn', Ele_Tag_02, 3, 4, AREA, Es, Iz, transfTag, '-mass', 0.0001)
    ...
    ops.element('corotTruss', Ele_Tag_03, 2, 4, AREA, MAT_TAG, '-rho',AREA*0.00785 , '-doRayleigh', 1)
        
    # Define material and element - Kθ(M-θ)
    if SPRING_TYPE == 'ELASTIC': 
        ...
        ops.element('zeroLength', 20, 22, 3,'-mat', 1,'-dir', 3) # INFO LINK: https://openseespydoc.readthedocs.io/en/latest/src/ZeroLength.html
    ...
    if SPRING_TYPE == 'INELASTIC':
        ...
        ops.element('zeroLength', 20, 22, 3,'-mat', 1, 2,'-dir', 3, 3)
...
DATA = EXTERNAL_TIME_DEPENDENT_LOAD_MDOF(SPRING_TYPE, MAT_TYPE, duration, dt)
...
S07.PLOT_2D_FRAME(deformed_scale=1)  # Adjust scale factor as needed

SPRING_TYPE = 'INELASTIC' # Kθ(M-θ) - 'ELASTIC' OR 'INELASTIC' 
...
# Print the Model
#printModel()
ops.printModel("-JSON", "-file", "MDF_PENDULUM_FIVE.json")
#%%-------------------------------------------------------------------------------
# EXCEL OUTPUT
import pandas as pd
...
# Save to Excel
with pd.ExcelWriter("MDOF_PENDULUM_FIVE_OUTPUT.xlsx", engine='openpyxl') as writer:
