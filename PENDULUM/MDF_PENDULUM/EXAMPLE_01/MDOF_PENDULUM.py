...
----------------------------------------------------------------------------
This code performs nonlinear time-history analysis of a 2D moment-resisting frame
 with lumped mass at the top, subjected to artificial seismic excitation.
 The structure is modeled using elastic beam-column elements and zero-length
 springs at intermediate nodes to represent either elastic or inelastic behavior.
 For the inelastic case, a uniaxial HystereticSM material defines a multi-linear
 backbone curve with hardening and degradation, capturing stiffness and strength
 deterioration under cyclic loading. Rayleigh damping is applied via a viscous material
 combined in parallel with the hysteretic spring. An artificial ground acceleration record
 is generated using spectral matching (S04 function) and applied as an equivalent inertial
 force (P(t) = -m·üg). Time integration uses the Newmark-β method with Newton-Raphson
 iteration. Modal properties (eigenvalues, periods) are extracted at each step, and the
 instantaneous period is computed from tangent stiffness, revealing period elongation due
 to inelasticity. Damping ratios are estimated from displacement decay. Output includes
 displacement, velocity, acceleration, base reactions, and force components (inertia, damping, spring)
 for both elastic and inelastic models.
 Plots compare elastic vs. inelastic response:
force-displacement hysteresis, stiffness degradation, period elongation,
 and energy dissipation (damping force–velocity loops).
 The analysis quantifies how inelastic action reduces stiffness, lengthens period,
 and increases hysteretic damping, leading to reduced seismic forces
 but larger residual displacements—fundamental concepts in performance-based
...
FY = 8500.0                       # [N] Yield Force of Structure
FU = 1.1 * FY                     # [N] Ultimate Force of Structure
Ke = 450000.0                     # [N/m] Spring Elastic Stiffness
...
def EXTERNAL_TIME_DEPENDENT_LOAD_MDOF(SPRING_TYPE, duration, dt):
    ...
    # Add nodes
    ops.node(1, 0.0, 0.0)
    ops.node(2, 0.0, 0.25*L)
    ops.node(3, 0.0, 0.5*L)
    ops.node(4, 0.0, L)
    
    ops.node(11, 0.0, 0.0)#-1.0e-8
    ops.node(22, 0.0, 0.5*L)
    
    # Define mass to node 4
    ops.mass(4, M, M, 0.0)
    ...
    ops.element('elasticBeamColumn', 200, 2, 3, AREA, Es, Iz, transfTag, '-mass', 0.0001)
    ops.element('elasticBeamColumn', 300, 3, 4, AREA, Es, Iz, transfTag, '-mass', 0.0001)
    
    # Define material and element
    if SPRING_TYPE == 'ELASTIC': 
        ops.uniaxialMaterial('Elastic', 1, Ke, C)
        ops.element('zeroLength', 10, 11, 1,'-mat', 1, 1,'-dir', 1, 2) # INFO LINK: https://openseespydoc.readthedocs.io/en/latest/src/ZeroLength.html
        ops.element('zeroLength', 20, 22, 3,'-mat', 1, 1,'-dir', 1, 2)
    if SPRING_TYPE == 'INELASTIC':
        ...
        ops.element('zeroLength', 10, 11, 2,'-mat', 1, 2, 1, 2,'-dir', 1, 1, 1, 1)
        ops.element('zeroLength', 20, 22, 3,'-mat', 1, 2, 1, 2,'-dir', 2, 2, 2, 2)
    ...
    while stable == 0 and current_time < duration:
        ...
        ops.reactions()
        reactionX.append(ops.nodeReaction(2, 1)) # BASE REACTION IN X DIR.
        reactionY.append(ops.nodeReaction(2, 2)) # BASE REACTION IN Y DIR.
        dispX.append(ops.nodeDisp(4, 1))         # DISPLACEMENT IN X DIR.  
        dispY.append(ops.nodeDisp(4, 2))         # DISPLACEMENT IN Y DIR.
        velX.append(ops.nodeVel(4, 1))           # VELOCITY IN X DIR.
        velY.append(ops.nodeVel(4, 2))           # VELOCITY IN Y DIR.
        accelX.append(ops.nodeAccel(4, 1))       # ACCELERATION IN X DIR.
        accelY.append(ops.nodeAccel(4, 2))       # ACCELERATION IN Y DIR.
    ...
    # OUTPUTED DATA
    DATA = (time, reactionX, dispX, velX, accelX, reactionY, dispY, velY, accelY,
            stiffnessX, stiffnessY, PERIOD, damping_ratioX, damping_ratioY,
            FDx, FSx, FIx, FDy, FSy, FIy)
...
(time, reactionXE, dispXE, velXE, accelXE, reactionYE, dispYE, velYE, accelYE,
 stiffnessXE, stiffnessYE, periodE, E_damping_ratioXE, E_damping_ratioYE,
 FDXe, FSXe, FIXe, FDYe, FSYe, FIYe) = DATA

S02.PERIOD_FUN(dispXE, dt)

SPRING_TYPE = 'INELASTIC'
DATA = EXTERNAL_TIME_DEPENDENT_LOAD_MDOF(SPRING_TYPE, duration, dt)
(time, reactionXI, dispXI, velXI, accelXI, reactionYI, dispYI, velYI, accelYI,
 stiffnessXI, stiffnessYI, periodI, E_damping_ratioXI, E_damping_ratioYI,
 FDXi, FSXi, FIXi, FDYi, FSYi, FIYi) = DATA
...
# Print the Model
#printModel()
ops.printModel("-JSON", "-file", "MDF_PENDULUM.json")
#%%-------------------------------------------------------------------------------
# EXCEL OUTPUT
import pandas as pd
...
# Save to Excel
with pd.ExcelWriter("MDOF_PENDULUM_OUTPUT.xlsx", engine='openpyxl') as writer:
