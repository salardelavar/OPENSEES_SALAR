...
M3 = 1.5                # [kg] Mass of Node 03
M4 = 7.0                # [kg] Mass of Node 04
...
W3 = M1 * g             # [N] Weight of Node 03
W4 = M2 * g             # [N] Weight of Node 04
...
def PENDULUM_MDOF(MAT_TYPE, duration, dt, DR):
    ...
    ops.element('corotTruss', 3, 4, 5, AREA, MAT_TAG, '-rho', AREA*0.00785 , '-doRayleigh', 1)
    ops.element('corotTruss', 4, 5, 6, AREA, MAT_TAG, '-rho', AREA*0.00785 , '-doRayleigh', 1)
    
    ops.element('corotTruss', 5, 7, 2, AREA, MAT_TAG, '-rho', AREA*0.00785 , '-doRayleigh', 1)
    ops.element('corotTruss', 6, 2, 5, AREA, MAT_TAG, '-rho', AREA*0.00785 , '-doRayleigh', 1)
    ops.element('corotTruss', 7, 5, 8, AREA, MAT_TAG, '-rho', AREA*0.00785 , '-doRayleigh', 1)
    ...
    while stable == 0 and current_time < duration:
        ...
        ops.reactions()
        reactionX.append(ops.nodeReaction(1, 1)+ops.nodeReaction(4, 1)) # BASE REACTION IN X DIR.
        reactionY.append(ops.nodeReaction(1, 2)+ops.nodeReaction(4, 2)) # BASE REACTION IN Y DIR.
        ...
        OMEGA.append(np.sqrt(stiffnessX[-1]/(M1+M2+M3+M4)))
        PERIOD.append((np.pi * 2) / OMEGA[-1])
        # IN EACH STEP STRUCTURAL PERIOD WILL BE CALCULATED
        PERIODmin, PERIODmax = S03.EIGENVALUE_ANALYSIS(4, PLOT=True)
        #PERIODmin, PERIODmax = S07.RAYLEIGH_DAMPING(4, 0.5*DR, DR, 0, 1)
        PERIOD_MIN.append(PERIODmin)
        PERIOD_MAX.append(PERIODmax)
        Ci = 2 * DR * (2*np.pi/PERIODmax) * (M1+M2+M3+M4) # Damping COEFFICIENT - UPDATED AND CHANGES IN EACH STEP
...
# Print the Model
#printModel()
ops.printModel("-JSON", "-file", "MDOF_PENDULUM_FOUR.json")
#%%-------------------------------------------------------------------------------
# EXCEL OUTPUT
import pandas as pd
...
# Save to Excel
with pd.ExcelWriter("MDOF_PENDULUM_FOUR_OUTPUT.xlsx", engine='openpyxl') as writer:
