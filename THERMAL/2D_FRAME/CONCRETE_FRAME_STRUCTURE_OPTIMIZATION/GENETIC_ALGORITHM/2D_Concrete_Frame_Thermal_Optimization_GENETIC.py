...
#----------------------------------------------------------------------------------#
#                  OPTIMIZATION ALOGORITHM: GENETIC ALGORITHM METHOD               #
#----------------------------------------------------------------------------------#
...
3. Analysis Setup: The analysis uses static load control with thermal increments, and the Genetic Algorithm ensures convergence.
...
X = RD            # [mm^2] Intial Guess for Rebar Diameter
...
# ---------------------------------------------------------------------------
# FIND THE OPTIMUM VALUE (GENETIC ALGORITHM FOR OPTIMAL REBAR DIAMETER)
# ---------------------------------------------------------------------------

def GENETIC_ALGORIHM_OPTIMIZER(B, H, COVER, DEMAND_NODE, DEMAND, TOLERANCE, ITMAX, X_bounds, pop_size=50, mutation_rate=0.1, crossover_rate=0.8):
    ...
    for generation in range(ITMAX):
        ...
        for X in population:
            ...
            print('F:   ', F)
            residual = np.abs(F)
            fitness.append(residual)
            
            # Update best solution if improved
            if residual < best_residual:
                best_residual = residual
                best_X = X
                # Early exit if tolerance met
                if best_residual <= TOLERANCE:
                    print(f"\t\t Convergence achieved at generation {generation}")
                    return best_X, best_residual, generation + 1
        
        # Selection: Tournament selection
        parents = []
        for _ in range(pop_size):
            # Randomly select 2 candidates and pick the fitter one
            candidates = np.random.choice(len(population), 2)
        ...
        for i in range(0, pop_size, 2):
            parent1 = parents[i]
            parent2 = parents[i+1] if (i+1) < pop_size else parent1
        ...
        # Mutation: Gaussian perturbation with bounds enforcement
        offspring = np.array(offspring[:pop_size])  # Ensure correct population size
        for i in range(len(offspring)):
            if np.random.rand() < mutation_rate:
                mutation = np.random.normal(scale=0.1*(X_bounds[1]-X_bounds[0]))
                offspring[i] = np.clip(offspring[i] + mutation, X_bounds[0], X_bounds[1])
        
        # Form new population
        population = offspring.copy()
        
        # Progress monitoring
        print(f'Generation {generation + 1}: Best X = {best_X:.4f}, Residual = {best_residual:.4e} \n\n')

    return best_X, best_residual, generation, temp, dispX, dispY, mid_node

#--------------------------------------------------------------------

# GA Parameters (adjust based on problem knowledge)
X_bounds = (6.0, 60.0)   # [mm] bounds for rebar diameter
...
# Run GA optimization
best_diameter, final_residual, generations_used, temp, dispX, dispY, mid_node = GENETIC_ALGORIHM_OPTIMIZER(
    B, H, COVER, DEMAND_NODE, DEMAND,
    TOLERANCE, ITMAX, X_bounds, pop_size, mutation_rate, crossover_rate
)

# Results output
print(f'\n\t\t Optimum Rebar Diameter: {best_diameter:.4f} mm')
print(f'\t\t Total Generations:      {generations_used}')
