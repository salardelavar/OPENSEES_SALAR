...
"""
Models and Analyzes a 2D Concrete Frame subjected to Thermal and Distributed Loads using OpenSees an Find optimum Steel Rebar Diameter
wth Newthon-raphson Method 
Key points:  
...
from CONCRETE_FIBERTHERMAL_SECTION import R_RECTANGULAR_CONCRETE_SECTION_REBAR_FF
...
distributed_load = -0.003  # [kN/mm] Distributed load
#--------------------------------------------------------------------
...
#--------------------------------------------------------------------
# Pushover Analysis of Thermal Loading
def TEMP_ANAL(B, H, COVER, RD, NUM_B, NUM_H, NUM_LAYERS):
    ...
    #Depth = R_RECTANGULAR_CONCRETE_SECTION(secTag, B, H, NUM_B, NUM_H, matTag01)
    # Concrete Thermal Fiber Section with Rebars 
    Depth = R_RECTANGULAR_CONCRETE_SECTION_REBAR_FF(secTag, B, H, COVER, RD, NUM_B, NUM_H, NUM_LAYERS,matTag02, matTag01, PLOT=False)

    #print(' Thermal Sections Done.')
    #--------------------------------------------------------------------
    ...
    biTag = 1
    ops.beamIntegration('Lobatto', biTag, secTag, numIntegrationPoints)
    #--------------------------------------------------------------------
    ...
    for i in range(num_stories):
        for j in range(num_bays + 1):  # Vertical elements (columns)
            ...
            ops.element('dispBeamColumnThermal', element_id, node1, node2, secTag, transfTag)
    ...
    ops.recorder('Element', '-file', 'STRESS_STRAIN_BOT.txt', '-time', '-ele', 6, 'section', secTag, 'fiber', -DD, 0, 'stressStrain')
    ops.recorder('Element', '-file', 'STRESS_STRAIN_TOP.txt', '-time', '-ele', 6, 'section', secTag, 'fiber', +DD, 0, 'stressStrain')
    #--------------------------------------------------------------------
    # Perform analysis
    temp = []
    dispX = []
...
"""
# ###########################
#  GENETIC ALOGORITHM METHOD 
# ###########################

import numpy as np
import random

def FITNESS_FUNCION(X, B, H, COVER, NUM_B, NUM_H, NUM_LAYERS, DEMAND):
    # Calculate the fitness of a candidate solution.
    temp, dispX, dispY, mid_node = TEMP_ANAL(B, H, COVER, X, NUM_B, NUM_H, NUM_LAYERS)
    SUPPLY = np.max(np.abs(dispX))
    F = SUPPLY - DEMAND
    return -np.abs(F)  # The closer F is to zero, the better the fitness

def GENETIC_ALOGORITHM(B, H, COVER, NUM_B, NUM_H, NUM_LAYERS, DEMAND,
                      population_size=20, generations=100,
                      mutation_rate=0.1, crossover_rate=0.8):

    # Genetic Algorithm to find the optimal rebar diameter.
    # Initialize population
    population = np.random.uniform(low=1.0, high=50.0, size=population_size)  # Random initial diameters
    best_solution = None
    best_fitness = -np.inf
    ...
    for generation in range(generations):
        ...
        fitness = np.array([FITNESS_FUNCION(X, B, H, COVER, NUM_B, NUM_H, NUM_LAYERS, DEMAND) for X in population])
        ...
        fitness_sum = np.sum(fitness - fitness.min() + 1e-6)  # Avoid division by zero
        probabilities = (fitness - fitness.min() + 1e-6) / fitness_sum
        ...
        for _ in range(population_size // 2):
            if random.random() < crossover_rate:
                ...
                child2 = 0.5 *(par3entparent1 -parentt2);
...
# Define parameters
#B = 100           # Example: Width of the section
#H = 200           # Example: Height of the section
#COVER = 25        # Example: Concrete cover
#NUM_B = 4         # Number of bars in width
#NUM_H = 4         # Number of bars in height
#NUM_LAYERS = 2    # Number of rebar layers
DEMAND = 0.00005   # [mm] Demand Lateral Displacement
...
best_diameter = GENETIC_ALOGORITHM(
    B, H, COVER, NUM_B, NUM_H, NUM_LAYERS, DEMAND,
...
# Print results
print(f"Optimal Rebar Diameter: {best_diameter:.4f}")
...
DEMAND = 0.00005  # [mm] Demand Lateral Displacement
X = RD            # Intial Guess for Rebar Diameter
ESP = 1e-10        # Finite difference derivative Convergence Tolerance
...
while (RESIDUAL > TOLERANCE):
    ...
    Xmin = X - ESP
    temp, dispX, dispY, mid_node = TEMP_ANAL(B, H, COVER, Xmin, NUM_B, NUM_H, NUM_LAYERS)
    ...
    Xmax = X + ESP
    temp, dispX, dispY, mid_node = TEMP_ANAL(B, H, COVER, Xmax, NUM_B, NUM_H, NUM_LAYERS)
    ...
    # CONTROLLING -------------------
    if np.abs(DF) < TOLERANCE:  # Avoid division by zero or near-zero values
        print("Derivative too small; non-convergence.")
        break
    if IT == ITMAX:
