...
#----------------------------------------------------------------------------------#
#            OPTIMIZATION ALOGORITHM: DECISION TREE REGRESSION METHOD              #
#----------------------------------------------------------------------------------#
...
3. Analysis Setup: The analysis uses static load control with thermal increments, and the Decision Tree algorithm ensures convergence.
...
NUM_SAMPLES = 200       # Number of samples for data generation
TOLERANCE = 1e-6        # Convergence Tolerance

# -----------------------------------------------------------------------------
# FIND THE OPTIMUM VALUE (DECISION TREE REGRESSION FOR OPTIMAL REBAR DIAMETER)
# -----------------------------------------------------------------------------

def DECISION_TREE_OPTIMIZER(B, H, COVER, DEMAND_NODE, DEMAND, TOLERANCE, X_bounds, NUM_SAMPLES):
    ...
    for X in X_samples:
        ...
        print(f'\n Iteration {it+1} - Rebar Diameter: {X} (mm) - Max. Disp: {SUPPLY} (mm)\n\n')
        it += 1
        disp_samples.append(SUPPLY)
    ...
    regressor.fit(X_samples, disp_samples)
    
    # Fine grid search within bounds
    fine_diameters = np.linspace(X_bounds[0], X_bounds[1], 1000).reshape(-1,1)
    predicted_displacements = regressor.predict(fine_diameters)
    residuals = np.abs(predicted_displacements - DEMAND)
    #print(f'Residuals:  {residuals} \n\n')
    
    # Find diameter with minimum residual
    min_index = np.argmin(residuals)
    best_diameter = fine_diameters[min_index][0]
    #print(f'Best Diameter:  {best_diameter} \n\n')
    final_residual = residuals[min_index]
    
    return best_diameter, final_residual, temp, dispX, dispY, mid_node

# Analysis Durations:
starttime = TI.process_time()

# Run Decision Tree optimization
best_diameter, final_residual, temp, dispX, dispY, mid_node = DECISION_TREE_OPTIMIZER(
    B, H, COVER, DEMAND_NODE, DEMAND,
    TOLERANCE, X_bounds, NUM_SAMPLES
)
