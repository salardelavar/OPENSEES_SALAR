...
fy = 0.4               # [kN/mm^2] Yield strength of steel section
...
def R_RECTANGULAR_CONCRETE_SECTION_REBAR_FF(secTag, B, H, COVER, RD, NUM_B, NUM_H, NUM_LAYERS, matTag_concrete, matTag_steel):
    ...
    # Calculate rebar layer positions
    layer_spacing = H / (NUM_LAYERS + 1)
    rebar_positions = [-H / 2 + (layer + 1) * layer_spacing for layer in range(NUM_LAYERS)]
...
RD = 12       # [mm] Diameter of the rebar
...
# Concrete Thermal Fiber Section with Rebars 
NUM_LAYERS = 6 # Number of Rebar fiber layers
