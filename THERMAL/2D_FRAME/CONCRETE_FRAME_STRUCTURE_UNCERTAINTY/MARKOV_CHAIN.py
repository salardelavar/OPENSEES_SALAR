...
def MARKOV_CHAIN(FILE_TF, file_path, DATA):
    """
    Reads displacement data from a file, calculates the Markov Transition Matrix, and visualizes it as a directed graph.

    Parameters:
        file_path (str): Path to the file containing displacement data.
    ...
    Q1 = 85  #np.quantile(data, 0.05)  # No Damage Level
    Q2 = 90  #np.quantile(data, 0.10)  # Minor Damage Level
    Q3 = 95  #np.quantile(data, 0.50)  # Moderate Damage Level
    Q4 = 100 #np.quantile(data, 0.80)  # Severe Damage Level
