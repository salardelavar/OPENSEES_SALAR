...
These values are approximate and can vary based on factors such as moisture content, compaction, and the presence of organic material. For accurate assessments, it's best to conduct site-specific geotechnical investigations.

Contact problems in finite element analysis (FEA) are quite fascinating and complex! They involve the interaction between two or more bodies that come into contact with each other. Here's a brief overview:

What are Contact Problems?
Contact problems occur when two or more bodies come into contact and interact with each other. This interaction can involve forces, displacements, and stresses at the contact interface. Examples include metal forming, vehicle crashes, and gear systems.

Lateral Springs in Contact Problems
Lateral springs are used in FEA to model the contact behavior between bodies. They provide a way to simulate the stiffness and damping properties of the contact interface. Here's how they work:
...
mass_values = [100.0, 500.0]  # [kg] mass values for node 2
...
def INTERACTION_ANALYSIS(MASS_ONE, DAMPING, SOIL_DAMPING, periodTF, LINEAR, SPRING_KIND, CONTACT, CONTACT_DISP):
    ...
    displacement = np.array(displacement_n2x)
    ...
    return modal_period, time, displacement_n1x, displacement_n1y, displacement_n2x, displacement_n2y, velocity_n1, velocity_n2, acceleration_n1, acceleration_n2, spring_force_H, spring_force_V, spring_force_M, ele_force, delta
#%%------------------------------------------------
### SOIL-STRUCTURE INTERACTION WITH TWO MASSES:
starttime = time.process_time()
# Perform analysis
period_undamped, time_undamped, displacement_n1x_undamped, displacement_n1y_undamped, displacement_n2x_undamped, displacement_n2y_undamped, velocity_n1_undamped, velocity_n2_undamped, acceleration_n1_undamped, acceleration_n2_undamped, spring_force_H_undamped, spring_force_V_undamped, spring_force_M_undamped, ele_force_undamped, delta_undamped = INTERACTION_ANALYSIS(MASS_ONE=False, DAMPING=False, SOIL_DAMPING=True, periodTF=False, LINEAR=False, SPRING_KIND=4, CONTACT=True, CONTACT_DISP=0.2)
period_damped, time_damped, displacement_n1x_damped, displacement_n1y_damped, displacement_n2x_damped, displacement_n2y_damped, velocity_n1_damped, velocity_n2_damped, acceleration_n1_damped, acceleration_n2_damped, spring_force_H_damped, spring_force_V_damped, spring_force_M_damped, ele_force_damped, delta_damped = INTERACTION_ANALYSIS(MASS_ONE=False, DAMPING=True, SOIL_DAMPING=True, periodTF=False, LINEAR=False, SPRING_KIND=4, CONTACT=True, CONTACT_DISP=0.2)
...
# Solve for x
solution = fsolve(EQUATION, x0, args=(delta_undamped))
print(f"Exact Solution - Undamped Structure: {solution[0]:.8e}")
solution = fsolve(EQUATION, x0, args=(delta_damped))
