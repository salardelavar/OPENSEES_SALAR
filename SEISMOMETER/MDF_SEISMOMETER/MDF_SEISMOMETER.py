...
#                   MODELING OF SEISMOMETER (DISPLACEMENT METER) MDOF STRUCTURE USING OPENSEES                       #
#                                                   P(t) = -m * ug**(t)                                              #
#--------------------------------------------------------------------------------------------------------------------#
...
1. This OpenSeesPy script models a Multi-Degree-of-Freedom (MDOF) structure's response
 to time-dependent loading, comparing elastic and inelastic material behavior.

2. The structure consists of elastic beam-column elements with a zero-length spring
 element at the base that can be either linear elastic or hysteretic nonlinear.

3. The hysteretic material follows a specified backbone curve with hardening behavior
 and pinching effects to simulate realistic energy dissipation during cyclic loading.

4. External excitation is applied as force loading derived from artificial earthquake
 acceleration records, simulating ground motion effects.

5. The analysis uses Newmark time integration with Newton-Raphson iteration to solve
 the dynamic equations of motion.

6. Key response parameters are tracked including displacement, velocity, acceleration,
 base reactions, stiffness, and period variation over time.

7. Damping ratios are calculated using logarithmic decrement from response peaks,
 accounting for nonlinear behavior.

8. The code evaluates three fundamental forces: damping force (fD), spring force (fS),
 and inertia force (fI) for both material models.

9. Results include comparison plots of force-displacement relationships, time-history
 responses, and period evolution during loading.

10. Output data is saved in both JSON format for model state and Excel format for
 post-processing and validation of structural performance under dynamic excitation.
...
def EXTERNAL_TIME_DEPENDENT_LOAD_MDOF(SPRING_TYPE, duration, dt):
    ...
    ops.node(11, 0.5*L, 0.0)#-1.0e-8
    #ops.node(22, 0.5*L, 0.0)
    
    # Define mass to node 3
    ops.mass(3, 0.0, M, 0.0)
    ...
    if SPRING_TYPE == 'ELASTIC': 
        ...
        ops.element('zeroLength', 10, 11, 2,'-mat', 1,'-dir', 2) # INFO LINK: https://openseespydoc.readthedocs.io/en/latest/src/ZeroLength.html
    if SPRING_TYPE == 'INELASTIC':
        ...
        ops.element('zeroLength', 10, 11, 2,'-mat', 1, 2,'-dir', 2, 2)
    ...
    while stable == 0 and current_time < duration:
        ...
        ops.reactions()
        reaction.append(ops.nodeReaction(1, 2)) # BASE REACTION
        disp.append(ops.nodeDisp(3, 2))         # DISPLACEMENT  
        vel.append(ops.nodeVel(3, 2))           # VELOCITY
        accel.append(ops.nodeAccel(3, 2))       # ACCELERATION
...
# Print the Model
#printModel()
ops.printModel("-JSON", "-file", "MDF_SEISMOMETER.json")
#%%-------------------------------------------------------------------------------
# EXCEL OUTPUT
import pandas as pd
...
# Save to Excel
with pd.ExcelWriter("MDF_SEISMOMETER_OUTPUT.xlsx", engine='openpyxl') as writer:
