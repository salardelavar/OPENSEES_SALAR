...
#                                (RELEASE MOMENT IN FIRST AND LAST ELEEMENT)                              #
#---------------------------------------------------------------------------------------------------------#
...
def POST_BUCKLING(IA, IU, IV, MODE_NUM):
    ...
    ops.fix(N+1, 0, 0, 1)  # Top node: free u_x, free u_y, fix theta
    #------------------------------------------------------------------------------------------------
    ...
    for i in range(1, N+1):
        if i == 1 or i == N:
            ops.element('nonlinearBeamColumn', i, i, i+1, numIntgrPts, secTag, transfTag,'-mass', Ele_Mass, '-release', 3)
        else:
    ...
    # OUTPUTED DATA
    DATA = (axial_disp, rotation, lateral_disp,
            axial_velocity, lateral_velocity,
            axial_acce, lateral_acce, axial_force_BOT,
...
# Displacement plot
plt.subplot(6, 1, 4)
plt.plot(time, lateral_disp_00, linewidth=1.5, label=f'MODE 00 - Damping Ratio: {damping_ratio_00:.3e} %')
...
# Velocity plot
plt.subplot(6, 1, 5)
plt.plot(time, lateral_velocity_00, linewidth=1.5)
...
# Acceleration plot
plt.subplot(6, 1, 6)
plt.plot(time, lateral_acce_00, linewidth=1.5)
...
# Save to Excel
with pd.ExcelWriter("MULTI-MODE-POST_BUCKLING_STEEL_COLUMN_NONLINEAR_FV_LATERAL_RELEASE_M3_OUTPUT.xlsx", engine='openpyxl') as writer:
