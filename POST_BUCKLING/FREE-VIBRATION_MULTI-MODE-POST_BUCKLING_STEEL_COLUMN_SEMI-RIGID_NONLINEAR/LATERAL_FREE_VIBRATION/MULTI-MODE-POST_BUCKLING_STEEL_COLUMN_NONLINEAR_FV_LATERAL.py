...
#    INVESTIGATION OF FREE-VIBRATION ANALYSIS WITH LATERAL DISPLACEMENT OF MULTI-MODE POST-BUCKLING       #
#  PHENOMENA STEEL COLUMNS USING OPENSEES CONSIDERING THE GEOMETRIC AND MATERIAL PROPERTIES NONLINEARITY  #
#---------------------------------------------------------------------------------------------------------#
...
# 8. BUCKLING BEHAVIOR CAPTURE: LATERAL DISPLACEMENTS AT THE TOP-HEIGHT NODE INDICATE POST-BUCKLING       #
# DEFORMATION.                                                                                            #
...
def POST_BUCKLING(IA, IU, IV, MODE_NUM):
    ...
    #ops.fix(N+1, 0, 0, 0)  # Top node: fix u_x, free u_y, free theta
    #------------------------------------------------------------------------------------------------
    """
    ...
    if IU == True:
        ...
        ops.setNodeDisp(N+1, 1, u0, '-commit') # INFO LINK: https://openseespydoc.readthedocs.io/en/latest/src/setNodeDisp.html
    if IV == True:
        # Define initial velocity
        ops.setNodeVel(N+1, 1, v0, '-commit')  # INFO LINK: https://openseespydoc.readthedocs.io/en/stable/src/setNodeVel.html
    if IA == True:
        # Define initial  acceleration
        ops.setNodeAccel(N+1, 1, a0, '-commit') # INFO LINK: https://openseespydoc.readthedocs.io/en/latest/src/setNodeAccel.html
    ...
    damping_ratio = S06.DAMPING_RATIO(lateral_disp)   
    # OUTPUTED DATA
...
# Displacement plot
plt.subplot(6, 1, 4)
plt.plot(time, lateral_disp_01, linewidth=1.5, label=f'MODE 01 - Damping Ratio: {damping_ratio_01:.3e} %')
plt.plot(time, lateral_disp_02, linewidth=1.5, label=f'MODE 02 - Damping Ratio: {damping_ratio_02:.3e} %')
plt.plot(time, lateral_disp_03, linewidth=1.5, label=f'MODE 03 - Damping Ratio: {damping_ratio_03:.3e} %')
plt.plot(time, lateral_disp_04, linewidth=1.5, label=f'MODE 04 - Damping Ratio: {damping_ratio_04:.3e} %')
plt.title('Lateral Displacement vs Time', fontsize=12, pad=10)
plt.ylabel('Lateral Displacement (mm)', fontsize=10)
plt.grid(alpha=0.3)
plt.legend(loc='upper right', framealpha=1)

# Velocity plot
plt.subplot(6, 1, 5)
plt.plot(time, lateral_velocity_01, linewidth=1.5)
plt.plot(time, lateral_velocity_02, linewidth=1.5)
plt.plot(time, lateral_velocity_03, linewidth=1.5)
plt.plot(time, lateral_velocity_04, linewidth=1.5)
plt.title('Lateral Velocity vs Time', fontsize=12, pad=10)
plt.ylabel('Lateral Velocity (mm/s)', fontsize=10)
plt.grid(alpha=0.3)

# Acceleration plot
plt.subplot(6, 1, 6)
plt.plot(time, lateral_acce_01, linewidth=1.5)
plt.plot(time, lateral_acce_02, linewidth=1.5)
plt.plot(time, lateral_acce_03, linewidth=1.5)
plt.plot(time, lateral_acce_04, linewidth=1.5)
plt.title('Lateral Acceleration vs Time', fontsize=12, pad=10)
plt.ylabel('Lateral Acceleration (mm/s²)', fontsize=10)
plt.grid(alpha=0.3)
...
plt.plot(axial_disp_01, np.abs(axial_load_01)/PY, linewidth=1, label=f"Mode 01 - Axial Disp: {np.abs(axial_disp_01[-1]) : .3e}")
plt.plot(axial_disp_02, np.abs(axial_load_02)/PY, linewidth=1, label=f"Mode 02 - Axial Disp: {np.abs(axial_disp_02[-1]) : .3e}")
plt.plot(axial_disp_03, np.abs(axial_load_03)/PY, linewidth=1, label=f"Mode 03 - Axial Disp: {np.abs(axial_disp_03[-1]) : .3e}")
plt.plot(axial_disp_04, np.abs(axial_load_04)/PY, linewidth=1, label=f"Mode 04 - Axial Disp: {np.abs(axial_disp_04[-1]) : .3e}")
plt.xlabel('Axial displacement in Y Dir. at top-height (mm)')
...
plt.plot(lateral_disp_01, shear_load_01, linewidth=1, label=f"Mode 01 - Lateral Disp: {np.abs(lateral_disp_01[-1]) : .3e}")
plt.plot(lateral_disp_02, shear_load_02, linewidth=1, label=f"Mode 02 - Lateral Disp: {np.abs(lateral_disp_02[-1]) : .3e}")
plt.plot(lateral_disp_03, shear_load_03, linewidth=1, label=f"Mode 03 - Lateral Disp: {np.abs(lateral_disp_03[-1]) : .3e}")
plt.plot(lateral_disp_04, shear_load_04, linewidth=1, label=f"Mode 04 - Lateral Disp: {np.abs(lateral_disp_04[-1]) : .3e}")
plt.xlabel('Lateral displacement in X Dir. at top-height (mm)')
#plt.ylabel('Axial compressive load (N)')
...
plt.plot(rotation_01, np.abs(moment_load_01), linewidth=1, label=f"Mode 01 - Moment: {np.abs(moment_load_01[-1]) : .3e}")
plt.plot(rotation_02, np.abs(moment_load_02), linewidth=1, label=f"Mode 02 - Moment: {np.abs(moment_load_02[-1]) : .3e}")
plt.plot(rotation_03, np.abs(moment_load_03), linewidth=1, label=f"Mode 03 - Moment: {np.abs(moment_load_03[-1]) : .3e}")
plt.plot(rotation_04, np.abs(moment_load_04), linewidth=1, label=f"Mode 04 - Moment: {np.abs(moment_load_04[-1]) : .3e}")
...
# Save to Excel
with pd.ExcelWriter("MULTI-MODE-POST_BUCKLING_STEEL_COLUMN_NONLINEAR_FV_LATERAL_OUTPUT.xlsx", engine='openpyxl') as writer:
