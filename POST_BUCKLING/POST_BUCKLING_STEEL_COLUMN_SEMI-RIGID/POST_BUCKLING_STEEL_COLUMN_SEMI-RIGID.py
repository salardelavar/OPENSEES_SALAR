...
secTag = 1
fy = 240                      # [N/mm²] Yield strength of steel section
fu = 1.5 * fy                 # [N/mm²] Ultimate strength of steel section
Es = 200000                   # [N/mm²] Modulus of elasticity of steel section
...
matTag = 1
ops.uniaxialMaterial('Steel01', matTag, fy, Es, b) # Steel with bilinear kinematic hardening Material with effect
#------------------------------------------------------------------------------------------------
...
rotat_inc = 0.000001  # Rotation increment (rad)
...
plt.plot(axial_disp, np.abs(axial_load)/PY, 'black', label=f"Post Buckling Load: {np.abs(axial_load[-1]) : .3f}", linewidth=4)
...
plt.plot(lateral_disp, np.abs(axial_load)/PY, 'black', linewidth=4)
...
plt.plot(rotation, np.abs(moment_load), 'black', linewidth=4)
