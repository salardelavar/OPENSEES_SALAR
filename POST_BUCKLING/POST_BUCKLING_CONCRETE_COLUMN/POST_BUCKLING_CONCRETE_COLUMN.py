...
I = (BSec * HSec**3)/12
AREA = BSec * HSec       # COLUMN SECTION AREA
FFC = AREA * abs(fcp)    # ULTIMATE AXIAL CAPACITY
...
plt.plot(axial_disp, np.abs(axial_load)/FFC, 'black', label=f"Post Buckling Load: {np.abs(axial_load[-1]) : .3f}", linewidth=4)
...
plt.ylabel('Axial compressive load / (fc x Ag)')
