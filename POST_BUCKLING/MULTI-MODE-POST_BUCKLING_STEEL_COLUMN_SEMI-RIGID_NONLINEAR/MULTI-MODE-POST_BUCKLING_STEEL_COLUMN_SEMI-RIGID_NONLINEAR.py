...
def POST_BUCKLING(MODE_NUM):
    ...
    ops.uniaxialMaterial('Hysteretic', matTag_S, MY, TY, MU, TSU, 0.43*MU, 1.13*TSU, -MY, -TY, -MU, -TSU, -0.43*MU, -1.07*TSU, pinchX, pinchY, damage1, damage2, beta)
    ...
    return axial_disp, rotation, lateral_disp, axial_load, shear_load, moment_load, PY  
...
(axial_disp_01, rotation_01, lateral_disp_01,
 axial_load_01, shear_load_01, moment_load_01, PY) = DATA 
...
(axial_disp_02, rotation_02, lateral_disp_02,
 axial_load_02, shear_load_02, moment_load_02, PY) = DATA 
...
(axial_disp_03, rotation_03, lateral_disp_03,
 axial_load_03, shear_load_03, moment_load_03, PY) = DATA 
...
(axial_disp_04, rotation_04, lateral_disp_04,
 axial_load_04, shear_load_04, moment_load_04, PY) = DATA 
...
plt.plot(lateral_disp_01, np.abs(axial_load_01)/PY, linewidth=4, label=f"Mode 01")
plt.plot(lateral_disp_02, np.abs(axial_load_02)/PY, linewidth=4, label=f"Mode 02")
plt.plot(lateral_disp_03, np.abs(axial_load_03)/PY, linewidth=4, label=f"Mode 03")
plt.plot(lateral_disp_04, np.abs(axial_load_04)/PY, linewidth=4, label=f"Mode 04")
...
plt.plot(rotation_01, np.abs(moment_load_01), linewidth=4, label=f"Mode 01")
plt.plot(rotation_02, np.abs(moment_load_02), linewidth=4, label=f"Mode 02")
plt.plot(rotation_03, np.abs(moment_load_03), linewidth=4, label=f"Mode 03")
plt.plot(rotation_04, np.abs(moment_load_04), linewidth=4, label=f"Mode 04")
...
# Save to Excel
# --------------------------
with pd.ExcelWriter("PostBuckling_Modes.xlsx", engine='openpyxl') as writer:
...
print("MULTI-MODE-POST_BUCKLING_STEEL_COLUMN_SEMI-RIGID_NONLINEAR_OUTPUT.xlsx")
#%%------------------------------------------------------------------------------------------------