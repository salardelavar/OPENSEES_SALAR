...
for i in range(nx):
    for j in range(ny):
        ...
        spring_node = tag + nodes
        spring_tags[(i, j)] = spring_node
        ops.node(spring_node, x, y, 0.0)
        ...
        #ops.fix(spring_node, 1, 1, 1, 1, 1, 1) # fix base soil nodes -> High Strength Soil
        # Create zero-length element for vertical spring
        ops.element("zeroLength", spring_node, spring_node + NZ, spring_node, "-mat", 1, "-dir", 3)
        spring_tag += 1
        tag += 1
