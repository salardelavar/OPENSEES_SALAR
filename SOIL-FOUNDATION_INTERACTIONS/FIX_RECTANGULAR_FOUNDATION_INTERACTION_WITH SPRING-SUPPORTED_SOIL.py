...
for i in range(1, nx - 1):
    for j in range(1, ny - 1):
        ...
        A[idx, index(i - 1, j)] += E * H**3 / (12 * (1 - nu**2)) / dx**4
        A[idx, index(i + 1, j)] += E * H**3 / (12 * (1 - nu**2)) / dx**4
        A[idx, idx] -= 2 * E * H**3 / (12 * (1 - nu**2)) / dx**4

        # Contributions from bending in y-direction
        A[idx, index(i, j - 1)] += E * H**3 / (12 * (1 - nu**2)) / dy**4
        A[idx, index(i, j + 1)] += E * H**3 / (12 * (1 - nu**2)) / dy**4
        A[idx, idx] -= 2 * E * H**3 / (12 * (1 - nu**2)) / dy**4
...
# Corrected OpenSees Boundary Conditions
ops.wipe()
...
for i in range(nx):
    for j in range(ny):
        ...
        if i == 0 or i == nx - 1 or j == 0 or j == ny - 1:  # Only boundary nodes are fixed
