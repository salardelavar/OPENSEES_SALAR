...
# COMPARATIVE FREE-VIBRATION AN PUSHOVER ANALYSIS OF A SDOF STRUCTURE: ELASTIC VS INELASTIC RESPONSE USING OPENSEES  #
#                                AND EVALUATION OF DISSIPATED ENERGY CAPACITY INDEX                                  #
#--------------------------------------------------------------------------------------------------------------------#
#                              THIS PROGRAM WRITTEN BY SALAR DELAVAR GHASHGHAEI (QASHQAI)                            #
#                                       EMAIL: salar.d.ghashghaei@gmail.com                                          #
######################################################################################################################
"""
Energy Dissipation Capacity of Structural Components means the ability of parts of a structure (like beams, columns, braces, or dampers) to absorb and dissipate energy during dynamic loads, especially earthquakes.
It mainly occurs through inelastic deformation, cracking, yielding, and hysteresis behavior.
Components with high energy dissipation reduce seismic demand on the whole structure.
This capacity helps prevent sudden brittle failure and increases structural safety.
It is often evaluated using hysteresis loops from cyclic loading tests or analyses.
Good energy dissipation leads to higher ductility and better seismic performance.
----------------------------------------------------------------------------
...
"""
# BOOK: Energy Dissipation Capacity of Structural Components
'https://link.springer.com/chapter/10.1007/978-3-031-97129-7_13'
# PAPER: Experimental Quantification on the Residual Seismic Capacity of Damaged RC Column Members    
'https://www.researchgate.net/publication/331245046_Experimental_Quantification_on_the_Residual_Seismic_Capacity_of_Damaged_RC_Column_Members'
...
#%%-------------------------------------------------------------------------------
# Define parameters
M = [150.0, 100.0, 250.0, 350.0]   # [kg] Mass of each node
...
duration = 6.0                     # [s] Analysis duration
...
stiffness_factor_4 = 0.5
FY1_4, DY1_4 = 2000.0 * stiffness_factor_4, 0.008
FY2_4, DY2_4 = 3500.0 * stiffness_factor_4, 0.04
FY3_4, DY3_4 = 1800.0 * stiffness_factor_4, 0.10
FY4_4, DY4_4 = 1800.0 * stiffness_factor_4, 0.20
FY5_4, DY5_4 = 300.0 * stiffness_factor_4, 0.30
FY6_4, DY6_4 = 150.0 * stiffness_factor_4, 0.45
...
FY1n_4, DY1n_4 = -2000.0 * stiffness_factor_4, -0.008
FY2n_4, DY2n_4 = -3500.0 * stiffness_factor_4, -0.18
FY3n_4, DY3n_4 = -1800.0 * stiffness_factor_4, -0.70
...
plt.show()
#%%-------------------------------------------------------------------------------
def ANALYSIS_SDOF(ANA_KIND, DMAX, DINCR, IA, IU, IV, SPRING_KIND, M, C, K_elastic, KP, KN, u0, v0, a0, duration, dt):
    # Create model
    ops.wipe()
    ...
    if SPRING_KIND == 'ELASTIC': 
        ...
        ops.element('zeroLength', 1, 1, 2,'-mat', 11, 22, 33, 44, '-dir', 1, 1, 1, 1) # ELEMENT 01
    ...
    if SPRING_KIND == 'INELASTIC': 
        ...
        ops.element('zeroLength', 1, 1, 2,'-mat', 11, 311, 22, 322, 33, 333, 44, 344,'-dir', 1, 1, 1, 1, 1, 1, 1, 1) # ELEMENT 01
...
DATA = ANALYSIS_SDOF(ANA_KIND, DMAX, DINCR, IA, IU, IV, SPRING_KIND, M, C, K_elastic, KP, KN, u0, v0, a0, duration, dt)
...
# Print the Model
#printModel()
ops.printModel("-JSON", "-file", "FREE-VIBRATION_&_PUSHOVER_SDOF.json")
#%%-------------------------------------------------------------------------------
# PUSHOVER
SPRING_KIND = 'ELASTIC'
ANA_KIND = 'PUSHOVER'
