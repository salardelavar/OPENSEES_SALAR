...
# MOMENTS, CURVATURE, DUCTILITY RATIOS, AND STIFFNESS THROUGH 10,000 RANDOMIZED SIMULATIONS OF WELD MATERIAL         #
...
# CORRELATION HEATMAPS) TO ASSESS CONFIDENCE INTERVALS, SKEWNESS, AND PARAMETER SENSITIVITY.                         #
# THE OUTPUT CHARACTERIZES STRUCTURAL RELIABILITY AND PROBABILISTIC BEHAVIOR OF CONNECTIONS                          #
# FOR UNCERTAINTY-AWARE DESIGN.                                                                                      #
#--------------------------------------------------------------------------------------------------------------------#
...
def MOMENT_CURVATURE(secTag):
    ...
    # Define materials for nonlinear welded steel connection
    # --------------------------------------------------------
    # STEEL WELD MATERIALS
    # Weld reinforcing steel
    fy = random.uniform(980, 1000)     # Yield stress for weld
    ey = random.uniform(0.0049, 0.005) # Yield strain for weld
    esu = random.uniform(0.10, 0.12)   # Ultimate strain for weld
    fu = 1.1818 * fy                   # Ultimate stress for weld
    E = fy / ey                        # Young's modulus for weld
    Esh = (fu - fy) / (esu - ey)
    b = Esh / E
    
    """
    kh = np.array([[ey, fy], [esu, fu], [1.1*esu, 0.2*fu], [1.2*esu, 0.1*fu]])
    MatTag = 2	#  Tag of previously defined material (UniaxialMaterial tag for a FiberSection or NDMaterial tag for use in an NDFiberSection)
    # uniaxialMaterial MultiLinear $matTag $u1 $f1 $u2 $f2 $u3 $f3 $u4 $f4
    ops.uniaxialMaterial('MultiLinear', MatTag, *kh.flatten()) # Horizontal spring
    """
    ...
    ops.uniaxialMaterial('HystereticSM', MatTag, '-posEnv', *KP.flatten(), '-negEnv', *KN.flatten(), '-pinch', 1, 1, '-damage', 0.1, 0.01, '-beta', 0,'-defoLimitStates', 0.01, -0.01, 0.02, -0.02, '-forceLimitStates', fy, -fy, fu, -fu,'printInput')
    ...
    # Plate reinforcing steel M24 (Class 10.9)
    fyP = random.uniform(390, 395)           # Yield stress for plate
    eyP = random.uniform(0.001875, 0.001899) # Yield strain for plate
    ...
    # Define cross-section for nonlinear welded steel connection
    # ------------------------------------------------------------

    # Set some parameters
    t = random.uniform(9, 11)            # [mm] Weld Thickness
    tf = random.uniform(18, 22)          # [mm] I Section Flange Thickness
    tw = random.uniform(23, 25)          # [mm] I Section Web Thickness
    bf = random.uniform(210, 220)        # [mm] I Section Flange Length
    H = random.uniform(535, 540)         # [mm] I Section Total Height
    hw = H -  2 * tf                     # [mm] I Section Web Height
    L1 = bf + 2 * t                      # [mm] Weld Length for Top of Flange
    L2 = hw - 50                         # [mm] Weld Length along the Web
    L3 = 0.5 * bf - 50                   # [mm] Weld Length for Bottom of Flange
    
    # Check for Error
    for _ in range(1):
        if L1 <= 0 or L2 <= 0 or L3 <= 0:
            print('\n\n Weld length could not be negative value \n\n')
            break
    
    numSubdivY = 20	# Number of subdivisions (fibers) in the local y direction.
    numSubdivZ = 5	# Number of subdivisions (fibers) in the local z direction.
    ...
    ops.section('Fiber', 1)
    # INFO LINK: https://opensees.berkeley.edu/wiki/index.php/Patch_Command
    #patch rect $matTag $numSubdivY $numSubdivZ $yI $zI $yJ $zJ 
    ops.patch('rect', MatTag, numSubdivY, numSubdivZ, -y1 - t, z1, y1 + t, z1 + t)                   # LAYER 01
    #ops.patch('rect', MatTag, numSubdivY, numSubdivZ, -y1 - t, z1 - tf - t, -y1 - t + L3, z1 - tf)   # LAYER 02
    #ops.patch('rect', MatTag, numSubdivY, numSubdivZ, y1 + t - L3, z1 - tf - t, y1 + t , z1 - tf)    # LAYER 03
    ops.patch('rect', MatTag, numSubdivY, numSubdivZ, -y1 - t, z1 - tf, -y1, z1)                     # LAYER 04
    ops.patch('rect', MatTag, numSubdivY, numSubdivZ, y1, z1 - tf, y1 + t, z1)                       # LAYER 05
    ops.patch('rect', MatTag, numSubdivY, numSubdivZ, -0.5* hw - t, 0, -0.5* hw, 0.5 * L2)           # LAYER 06
    ops.patch('rect', MatTag, numSubdivY, numSubdivZ, 0.5* hw, 0, 0.5* hw + t, 0.5 * L2)             # LAYER 07
    ops.patch('rect', MatTag, numSubdivY, numSubdivZ, -0.5* hw - t, 0, -0.5* hw, -0.5 * L2)          # LAYER 08
    ops.patch('rect', MatTag, numSubdivY, numSubdivZ, 0.5* hw, 0, 0.5* hw + t, -0.5 * L2)            # LAYER 09
    ops.patch('rect', MatTag, numSubdivY, numSubdivZ, -y1 - t, -z1 + tf + t, -y1 - t + L3, -z1 + tf) # LAYER 10
    ops.patch('rect', MatTag, numSubdivY, numSubdivZ, y1 + t - L3, -z1 + tf, y1 + t, -z1 + tf + t)   # LAYER 11
    #ops.patch('rect', MatTag, numSubdivY, numSubdivZ, -y1 - t, -z1, -y1, -z1 + tf)                   # LAYER 12
    #ops.patch('rect', MatTag, numSubdivY, numSubdivZ, y1 + t, -z1, -y1, -z1 + tf)                    # LAYER 13
    ops.patch('rect', MatTag, numSubdivY, numSubdivZ, -y1 - t, -z1 - t, y1 + t, -z1)                 # LAYER 14

    # Estimate yield curvature
    # (Assuming no axial load and only top and bottom steel)
    d = H + t      # d -- from cover to section height + weld
    epsy = ey      # steel weld yield strain
    Ky = epsy / (0.5 * d)
    DR = 20        # Target ductility for analysis
    maxK = DR * Ky
    ...
    ops.integrator('LoadControl', 1/numIncr)
...
results_df.to_excel('MOMENT_CURVATURE_UNCERTAINTY_STEEL_WELDED_CONNECTION_RESULTS.xlsx', index=False)
#------------------------------------------------------------------------------------------------  
# ------------------------------------------
