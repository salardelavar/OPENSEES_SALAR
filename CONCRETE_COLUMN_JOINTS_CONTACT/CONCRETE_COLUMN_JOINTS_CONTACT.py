...
#          #         WITH AND WITHOUT EXTERNAL OBJECT CONTACT EFFECTS (CONTACT PROBLEMS)       #
#          #-----------------------------------------------------------------------------------#
...
Contact problems in finite element analysis (FEA) are quite fascinating and complex!
 They involve the interaction between two or more bodies that come into contact with each other. Here's a brief overview:

What are Contact Problems?
Contact problems occur when two or more bodies come into contact and interact with each other.
 This interaction can involve forces, displacements, and stresses at the contact interface.
 Examples include metal forming, vehicle crashes, and gear systems.

Lateral Springs in Contact Problems
Lateral springs are used in FEA to model the contact behavior between bodies. 
They provide a way to simulate the stiffness and damping properties of the contact interface. 
...
def PUSHOVER_ANALYSIS(COL, PX, PY, MZ, max_disp, disp_incr,
    ...
    P_Y_D = 12                     # [mm] Yield displacement
    P_K = P_Y / P_Y_D              # [N/mm] Axial Rigidity Stiffness             
    ...
    #ops.uniaxialMaterial('Hysteretic', 100, P_Y, P_Y_D, P_U, P_eult_D, 0.2*P_U, 1.05*P_eult_D, -P_Y, -P_Y_D, -P_U, -0.5*P_eult_D, -0.2*P_U, -1.0*P_eult_D, 0.5, 0.5, 0.0, 0.0, 0.1)
    
    # BETA: ROTATIONAL SPRING 
    M_Y = 57530 * Bcol01 * Hcol01  # [N.mm] Yield moment capacity
    M_Y_R = 0.0523                 # [rad] Yield Rotation
    M_K = M_Y / M_Y_R              # [N.mm/rad] Rotational Flextural Rigidity Stiffness             
    ...
    #ops.uniaxialMaterial('Hysteretic', 200, M_Y, M_Y_R, M_U, M_eult_R, 0.2*M_U, 1.05*M_eult_R, -M_Y, -M_Y_R, -M_U, -0.5*M_eult_R, -0.2*M_U, -1.0*M_eult_R, 0.5, 0.5, 0.0, 0.0, 0.1)
...
### CONTACT:
CONTACT = False    # True: HAVE CONTACT - False: DO NOT HAVE CONTACT
...
PLOT_2D(X, Y, XLABEL, YLABEL, TITLE, COLOR)
#%%-------------------------------------------------------------
# %% Plot 2D Frame Shapes for Nonlinear Static Analysis (Pushover)
...
def DYNAMIC_ANALYSIS(MASS_X, MASS_Y, MASS_Z, COL, Bcol01, Hcol01, Bcol02, Hcol02, COVER, REBAR_DIA, CONTACT, CONTACT_DISP, Damping_Ratio, dt, Duration):
    ...
    return TIME, drift, displacements_x, displacements_y, rotations, velocityX, accelerationX, velocityY, accelerationY, base_shears, base_axials, base_moments, delta
...
CONTACT = False    # True: HAVE CONTACT - False: DO NOT HAVE CONTACT
CONTACT_DISP = 450 # [mm] CONTACT DISPLACEMENT
...
TIME, drift_x, displacements_x, displacements_y, rotations, velocityX, accelerationX, velocityY, accelerationY, base_shears, base_axials, base_moments, delta = DATA
