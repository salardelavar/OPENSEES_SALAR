...
u0 = 1.7     # [mm] Initial displacement applied to node 2
...
NONLINEAR = 'False' # 'True' -> Active Nonlinear Behaviour for main Strucure (Spring)
DY = 0.0001        # [mm] Yield Displacement
FY = 110.0         # [N] Yield Force
DU = 3.200         # [mm] Ultimate Displacement
FU = 423.0         # [N] Ultimate Force

if NONLINEAR == 'True' and np.abs(u0) > DY:
...
def ANALYSIS_SDOF_AMD():
    ...
    while stable == 0 and current_time < duration:
        ...
        Rayleigh_Force = ops.nodeResponse(2, 1, 8) # INFO LINK: https://openseespydoc.readthedocs.io/en/latest/src/nodeResponse.html
        # YOU CAN SEE force_damper AND Rayleigh_Force is SAME RESULTS
        total_reaction = -(force_spring + force_damper)
        base_reaction.append(total_reaction)
        control_force.append(F_control)
        RayleighForces.append(Rayleigh_Force)

    #ops.wipe()
    return time, displacement, velocity, acceleration, base_reaction, control_force, RayleighForces, PERIOD
#------------------------------------------------------------------------------------------------
# %% Run analysis
starttime = ti.process_time()
time, displacement, velocity, acceleration, base_reaction, control_force, RayleighForces, PERIOD = ANALYSIS_SDOF_AMD()
totaltime = ti.process_time() - starttime
print(f'\nTotal time (s): {totaltime:.4f} \n\n')
#------------------------------------------------------------------------------------------------
# %% PLOT FORCE-DISPALCEMENT RELATION FOR NONLINEAR SPRING
# Process positive cycle data
positive_cycle = [
    FY, DY,
    FU, DU,
    0.2 * FU, 1.1 * DU,
...
import PLOT_FORCE_DISP_FUN as PFDF
if NONLINEAR == 'True':
    PFDF.PLOT_FORCE_DISP(positive_cycle, negative_cycle)   
#------------------------------------------------------------------------------------------------
...
PLOT_DATA(time, RayleighForces, RAY_Z, 'Time [s]', 'Rayleigh Force [N]', 
          f'Time vs Control Force - MAX. ABS: {RAY_Z[-1]:.3f}')
#------------------------------------------------------------------------------------------------
...
# Print the Model
#printModel()
ops.printModel("-JSON", "-file", "MASS_DAMPER_SDOF.json")
#%%----------------------------------------------------------------------------------------------
