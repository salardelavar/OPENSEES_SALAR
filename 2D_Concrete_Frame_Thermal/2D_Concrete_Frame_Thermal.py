...
Nstep = 700                # Number of incremental steps
Incr_Temp = 0.001           # Incremental temperature step
#--------------------------------------------------------------------
# Define model
...
def R_RECTANGULAR_CONCRETE_SECTION_REBAR(secTag, B, H, NUM_B, NUM_H, matTag_concrete, matTag_steel):
    ...
    for y_loc in rebar_positions:
        x_loc_1 = -B / 4  # First rebar in the layer
        x_loc_2 = B / 4   # Second rebar in the layer
...
# Concrete Thermal Fiber Section with Rebars 
Depth = R_RECTANGULAR_CONCRETE_SECTION_REBAR(secTag, B, H, NUM_B, NUM_H, matTag02, matTag01)
#--------------------------------------------------------------------
...
# Plot frame shapes
PLOT_FRAME(deformed_scale=40)  # Adjust scale factor as needed
