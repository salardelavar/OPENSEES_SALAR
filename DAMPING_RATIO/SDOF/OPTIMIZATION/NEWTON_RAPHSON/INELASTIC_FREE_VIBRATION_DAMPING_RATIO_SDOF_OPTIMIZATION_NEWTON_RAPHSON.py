...
#C = 0.85                      # Intial Guess Damping coefficient (N/(m/s)^alpha)
...
while (RESIDUAL > TOLERANCE):
    # X -------------------------------------------------------
    time, displacement, velocity, acceleration, base_reaction, DI, T, XI, delta = ANALYSIS_DYN_SDOF(X)
    print(f'XI: {XI:.8e}')
    F = XI - X
    print('F: ', F)
    # Xmin -------------------------------------------------------
    XMIN = X - ESP  
    time, displacement, velocity, acceleration, base_reaction, DI, T, XImin, delta = ANALYSIS_DYN_SDOF(XMIN)
    Fmin = XImin - XMIN
    print('Fmin: ', Fmin)
    # Xmax -------------------------------------------------------
    XMAX = X + ESP  
    time, displacement, velocity, acceleration, base_reaction, DI, T, XImax, delta = ANALYSIS_DYN_SDOF(XMAX)
    Fmax = XImax - XMAX
    print('Fmax: ', Fmax)
    # DF -------------------------------------------------------
