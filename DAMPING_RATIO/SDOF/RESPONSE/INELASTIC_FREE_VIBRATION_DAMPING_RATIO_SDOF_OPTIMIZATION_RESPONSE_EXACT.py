...
for I in range(NI):
    U = DU * (I+1) / NI  # Intial displacement of each increment                     
    for J in range(NJ):
        print('\n')
        print('------------------------')
        ...
        X = DR             # Intial Guess Damping Ratio
        # X -------------------------------------------------------
        time, displacement, velocity, acceleration, base_reaction, DI, T, XI, delta = ANALYSIS_DYN_SDOF(X, U, M)
        #print(f'XI: {XI:.8e}')
        #------------------------------------------------------------------------------------------------
        # EXACT SOLUTION:
        ...
        ITERATION += 1     
        if solution[0] >= 0.0: # WE ARE GOING TO OUTPUT JUST POSITIVE DAMPING RATIOS DATA 
            MI.append(M)
            UI.append(U)
            XXI.append(solution[0])
            TII.append(T)
...
S01.CLUSTER_DATA(X, Y, XLABEL, YLABEL, MAX_CLUSTERS=3)
#------------------------------------------------------------------------------------------------
XLABEL = 'Intial Displacement'
YLABEL = 'Damping Ratio'
TITLE = f'{YLABEL} and {XLABEL} scatter chart'
COLOR = 'brown'
X = UI
Y = XXI
S01.PLOT_SCATTER(X, Y , XLABEL, YLABEL, TITLE, COLOR, LOG = 0, ORDER = 4)
