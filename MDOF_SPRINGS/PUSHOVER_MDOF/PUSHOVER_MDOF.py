...
#--------------------------------------------------------------------------------------------------------------------#
#     NONLINEAR STATIC PUSHOVER ASSESSMENT: DISPLACEMENT-BASED EQUIVALENT MDOF FORMULATION FOR ELASTIC AND INELASTIC #
...
######################################################################################################################
"""
Performs pushover analysis of a Multi Degree of Freedom (MDOF)
...
#%%-------------------------------------------------------------------------------
# Define parameters
MASS = [150000.0, 100000.0, 250000.0, 350000.0]   # [kg] Mass
...
FY1_1, DY1_1 = 2772.0 * stiffness_factor_1, 0.01     # First yield point
FY2_1, DY2_1 = 3104.6 * stiffness_factor_1, 0.05     # Peak force
FY3_1, DY3_1 = 1663.2 * stiffness_factor_1, 0.08     # Post-peak softening
FY4_1, DY4_1 = 1663.2 * stiffness_factor_1, 0.17     # Plateau
FY5_1, DY5_1 = 277.2 * stiffness_factor_1, 0.28      # Further softening
FY6_1, DY6_1 = 200.0 * stiffness_factor_1, 0.41      # Near-zero stiffness
FY7_1, DY7_1 = 100.0 * stiffness_factor_1, 0.61      # Zero force (failure)
...
FY7_2, DY7_2 = 80.0 * stiffness_factor_2, 0.52
...
FY7_3, DY7_3 = 140.0 * stiffness_factor_3, 0.43
...
FY7_4, DY7_4 = 25.0 * stiffness_factor_4, 0.51
...
KP_list = np.multiply([KP_1, KP_2, KP_3, KP_4], 10000)
KN_list = np.multiply([KN_1, KN_2, KN_3, KN_4], 10000)
...
for i, KP in enumerate(KP_list):
    ...
    omega = np.sqrt(K_elastic[i] / MASS[i])
    C.append(2 * zi[i] * omega * MASS[i])  # Damping Coefficient
...
#%%-------------------------------------------------------------------------------
def PUSHOVER_MDOF(DINCR, DMAX, SPRING_KIND, M, C, K_elastic, KP, KN, duration, dt): 
    ...
    IDctrlNODE = 5  ## INCREMENTAL DISPLACEMENT NODE IN X DIRECTION
...
DMAX = 0.13            # [m] Max. Pushover Incremental Displacement
DINCR = 0.001          # [m] Pushover Increment

SPRING_KIND = 'ELASTIC'
DATA = PUSHOVER_MDOF(DINCR, DMAX, SPRING_KIND, MASS, C, K_elastic, KP, KN, duration, dt)
...
# Print the Model
#printModel()
ops.printModel("-JSON", "-file", "PUSHOVER_MDOF.json")
#%%-------------------------------------------------------------------------------
# ELASTIC UNDAMPED PERIODS AND MODE SHAPES
import numpy as np
import matplotlib.pyplot as plt
...
K = [KE01, KE02, KE03, KE04]

# Example stiffness values (replace with actual KE01–KE04 if needed)
K_vals = np.array([KE01, KE02, KE03, KE04], dtype=float)

# Given mass values
M_vals = np.array(MASS)
...
modes = modes / np.max(np.abs(modes), axis=0)

# Plot each mode separately
for i in range(4):
    plt.figure()
    plt.plot(modes[:, i], np.arange(1, 5), marker='o')
    plt.gca().invert_yaxis()
    plt.xlabel("Normalized Mode Shape")
    plt.ylabel("Story / DOF")
