...
(MDOF) soil-structure systems. The methodology combines traditional nonlinear time-history
...
"""
#%%------------------------------------------------------------------------------------------------
# REPORT: Report Text for Better Learning Soil-Structure Interaction for Building Structures 
'https://www.nehrp.gov/pdf/nistgcr12-917-21.pdf'
# PAPER: A Review on Soil-Foundation-Interaction Models
'https://civiljournal.semnan.ac.ir/article_6930_3afa77b6a63f5e47a12417d33db7e766.pdf'
# PAPER: Modeling Soil Behavior with Simple Springs, Part 1 & 2
'https://nfba.org/aws/NFBA/asset_manager/get_file/760352?ver=0'
'https://nfba.org/aws/NFBA/asset_manager/get_file/760349?ver=0'    
# PAPER: Spring-Based Soil–Structure Interaction Modeling of Pile–Abutment Joints in Short-Span Integral Abutment Bridges with LR and RSM
'https://www.mdpi.com/2075-5309/15/19/3493'    
#%%------------------------------------------------------------------------------------------------
...
def ANALYSIS_MDOF(i, mSTRUCTURE):
    ...
    #ops.uniaxialMaterial('Elastic', Soil_Tag, 0.1*Soil_Stiffness, 0.0, Soil_Stiffness)
    ...
    while stable == 0 and current_time < duration:
        ...
        base_reaction.append(-ops.eleResponse(1, 'force')[0])             # Soil Spring Force
        SDF.append(ops.eleResponse(2, 'force')[0])                        # Soil Damping Force
        DI.append(100*(displacement_stru[-1] - DY) / (DSU - DY))          # [%] Structural Ductility Damage Index 
        stiffness.append(np.abs(base_reaction[-1]/displacement_soil[-1])) # [N/m] Stiffness
    ...
    OUTPUT = (time, displacement_soil, displacement_stru,
            ...
            acceleration_stru, base_reaction, SDF, DI, PERIOD,
...
labels = {
    ...
    7: "SOIL_BASE_REACTION_FORCE",
    8: "DAMAGE_INDEX",
    9: "DAMPING_RATIO",
    10: "STIFFNESS",
    11: "SOIL_DAMING_FORCE",
}
...
# ----------------------------------------
# Plot 10: Soil Stiffness Spectrum
# ----------------------------------------
X = T
Y = DATA
XLABEL = "Period T [s]"
YLABEL = "Soil Stiffness Spectrum [N/m]"
TITLE =  "Soil Stiffness Spectrum"
SEMILOGY = False

PLOT_2D(10, XLABEL, YLABEL, TITLE, SEMILOGY)

# ----------------------------------------
# Plot 11: Soil Damping Spectrum
# ----------------------------------------
X = T
Y = DATA
XLABEL = "Period T [s]"
YLABEL = "Soil Damping Foce Spectrum [N.s/m]"
TITLE =  "Soil Damping Force Spectrum"
SEMILOGY = False

PLOT_2D(11, XLABEL, YLABEL, TITLE, SEMILOGY)
...
plt.plot(displacement_soil, base_reaction,color='black')
plt.plot(displacement_soil, SDF,color='red')
plt.xlabel("Displacement [m]")
plt.ylabel("Force [N]")
plt.title("Displacement & Soil Forces Relation From Last Dynamic Analysis")
plt.legend(['Soil Base-Reaction Force', 'Soil Damping Force'])
plt.grid(True)
plt.tight_layout()
plt.show()
#%%------------------------------------------------------------------------------------------------
