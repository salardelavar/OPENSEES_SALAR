...
"""
[1] The function EQUIVALENT_VISCOUS_DAMPING_RATIO_FUN computes the dissipated energy (E_d) using either the convex‑hull area (method=1) or the shoelace formula (method=2), then calculates the equivalent viscous damping ratio as ζ = 100·E_d / (4π·E_s), where E_s = ½·F_max·u_max.
...
"""
def EQUIVALENT_VISCOUS_DAMPING_RATIO_FUN(displacement, base_shear, method, XLABEL, YLABEL, TITLE, PLOT):
    import numpy as np
...
method = 1 # 
PLOT = True
zeta = EQUIVALENT_VISCOUS_DAMPING_RATIO_FUN(u, F, method, XLABEL, YLABEL, TITLE, PLOT)
