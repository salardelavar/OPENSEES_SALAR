...
#     FRAGILITY ANALYSIS BASED ON ACCELERATION AND STRUCTURAL DUCTILITY DAMAGE INDEX AND EVALUATION       #
#           OF EQULIVALENT VISCOUS DAMPING RATIO  WITH INCREMENTAL DYNAMIC ANALYSIS (IDA)                 #  
#                OF A SINGLE-DEGREE-OF-FREEDOM (SDOF) SYSTEM UTILIZING 100 GROUND MOTIONS IN OPENSEES     #
#---------------------------------------------------------------------------------------------------------#
...
def ANALYSIS_SDOF(i, m, g):
    ...
    while stable == 0 and current_time < duration:
        ...
        di = S066.DAMAGE_INDEX_FUN(displacement[-1], DY, DSU)        # [%] Structural Ductility Damage Index 
        DI.append(di)  
...
DATA = {
    ...
    8: [],  # EQULIVALENT VISCOUS DAMPING RATIO
    }
...
for j in range(NUM_SEISMIC):
    ...
    for i in range(NUM_G):
        m = mi #* (i+1 / NUM_G)
        gi = 2*GMfact * ((i+1) / NUM_G) # [m/s^2] standard acceleration of gravity ineach step
        ...
        method = 1 # 
        zeta = S055.EQULIVALENT_VISCOUS_DAMPING_RATIO_FUN(displacement, base_reaction, method, XLABEL, YLABEL, TITLE)
...
labels = {
    ...
    6: "DAMPING_RATIO",
    7: "STIFFNESS",
    8: "EQULIVALENT_VISCOUS_DAMPING_RATIO",
}
...
def PLOT_2D(COUNT, XLABEL, YLABEL, TITLE):
    ...
    for j in range(NUM_SEISMIC):
        #plt.plot(G, DATA[COUNT][j], alpha=0.4, label=f'Sim {j+1} | Max: {np.max(np.abs(DATA[COUNT][j])):.4e}')
        plt.plot(G, DATA[COUNT][j], alpha=0.4)
    ...
    plt.plot(G, mean_curve, color='navy', linestyle='-.', linewidth=3, label='Mean')
    
    #plt.plot(G, std_curveDOWN, color='steelblue', linestyle='-', linewidth=2, label='Mean - Std')
    #plt.plot(G, std_curveUP,   color='deepskyblue', linestyle='-', linewidth=2, label='Mean + Std')
    
    plt.plot(G, median_curve, color='red', linestyle='--', linewidth=2, label='Median')
    
    plt.plot(G, q1_curve, color='green', linestyle='-.', linewidth=2, label='Q1 (25%)')
    plt.plot(G, q3_curve, color='orange', linestyle='-.', linewidth=2, label='Q3 (75%)')

    # Standard deviation band around the mean
    plt.fill_between(G, mean_curve - std_curve, mean_curve + std_curve,
...
X = G
Y = DATA
XLABEL = "Standard Acceleration of Gravity [m/s^2]"
...
# ---------------------------------------------------
# Plot 8: Equivalent viscous damping Ratio Spectrum
# ---------------------------------------------------
X = G
Y = DATA
XLABEL = "Standard Acceleration of Gravity [m/s^2]"
...
S01.HISROGRAM_BOXPLOT(max_zeta, HISTO_COLOR='yellow', LABEL='Equivalent viscous damping ratio')
#%%------------------------------------------------------------------------------------------------
####  FRAGILITY ANALYSIS
...
results_df.to_excel('INELASTIC_SDOF_INCREMENTAL_DYNAMIC_ANALYSIS_SEISMIC_DATA_RESULTS.xlsx', index=False)
#%%------------------------------------------------------------------------------------------------  
XLABEL = 'Displacement'
