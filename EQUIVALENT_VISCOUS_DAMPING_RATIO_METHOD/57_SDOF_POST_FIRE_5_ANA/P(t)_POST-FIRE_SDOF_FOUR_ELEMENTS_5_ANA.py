...
#              COMPREHENSIVE EVALUATION OF THE EQUIVALENT VISCOUS DAMPING RATIO OF A POST-FIRE            #
#             NONLINEAR SDOF SYSTEM WITH FOUR ELEMENTS STRUCTURE UNDER MULTIPLE LOADING PROTOCOLS         #                                   #
#---------------------------------------------------------------------------------------------------------#
...
Assume we have a structure where, after a fire, certain regions of
 ...
 member. Now, we assess the strength of this structure by performing eight different analyses.
...
This OpenSees script performs rigorous nonlinear static and dynamic analysis of a single‑degree‑of‑freedom (SDOF) oscillator for performance‑based earthquake engineering.
...
#%%----------------------------------------------------
def POST_FIRE_SDOF_FOUR_ELEMENTS(MAT_TYPE, TOTAL_MASS, ANAL_TYPE):
    # Initialize model
    ops.wipe()
    ...
    # THERMAL EFFECTS OF STEEL MATERIAL FORCE-DISPLACEMENT   
        if II == 0:
            ...
            T = 530.0
        # FIRE EFFECTS
        kE, kFy, kFu, esu = STEEL_FIRE_PROPERTIES(T) # THERMAL EFFECTS 100°C 
        EsI = kE*4500000.0                           # [N/m] Spring Elastic Stiffness
        fyI = kFy*85000.0                            # [N] Yield Force of Structure
        fuI = kFu*(1.18*85000.0)                     # [N] Ultimate Force of Structure
        ...
        if MAT_TYPE == 'INELASTIC':
            S08.OPENSEEES_HYSTERETICSM_FORCE_DISP_FUN(MAT_TAG, DP, FP, DN, FN, PLOT = True, X_LABEL='Displacement (m)', Y_LABEL='Force [N]', TITLE='FORCE-DISPLACEMENT CURVE')
        if MAT_TYPE == 'ELASTIC':
...
DATA = POST_FIRE_SDOF_FOUR_ELEMENTS(MAT_TYPE, TOTAL_MASS, ANAL_TYPE)
...
# Save to Excel
with pd.ExcelWriter("POST_FIRE_SDOF_FOUR_ELEMENTS_5_ANA_OUTPUT.xlsx", engine='openpyxl') as writer:
