...
# Function to plot heatmap for transition probability matrix
def PLOT_HEATMAP(transition_matrix):
    plt.figure(figsize=(8, 6))
    sns.heatmap(transition_matrix, annot=True, fmt=".2f", cmap="Blues", xticklabels=DAMAGE_STATES, yticklabels=DAMAGE_STATES)
    plt.xlabel("Next State")
    plt.ylabel("Current State")
    plt.title("Transition Probability Matrix Heatmap")
    plt.show()

# Plot the heatmap using updated transition probabilities
PLOT_HEATMAP(updated_transition_probs)
