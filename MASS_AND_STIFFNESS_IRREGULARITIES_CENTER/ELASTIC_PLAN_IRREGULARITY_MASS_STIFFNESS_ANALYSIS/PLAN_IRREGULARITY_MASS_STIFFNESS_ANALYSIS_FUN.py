def PLAN_IRREGULARITY_MASS_STIFFNESS_ANALYSIS_FUN(storey_height=3000.0):
    """
    ...
    Internal data (12 columns + 4 shear walls):
        Columns:  [x, y, b, h, E, mass]   (b,h = cross‑section dimensions [mm])
        Shear walls: [x, y, length, thickness, E, mass, orientation]
                      orientation = 0 → wall length along x (resists y‑loading)
                      orientation = 1 → wall length along y (resists x‑loading)

    Returns:
        x_CR, y_CR          : centre of rigidity coordinates [mm]
        x_CM, y_CM          : centre of mass coordinates [mm]
    ...
    columns = np.array([
        ...
        [12000,   0, 300, 300, 25000, 80000],   # C3 (heavy mass)
        [12000,4000, 300, 300, 25000, 900],   # C4
        [12000,8000, 300, 300, 25000, 800],   # C5
        [6000, 8000, 300, 300, 25000, 800],   # C6
        [   0, 8000, 300, 300, 25000, 800],   # C7
        [   0, 4000, 300, 300, 25000, 800],   # C8
        [3000, 2000, 250, 250, 30000,1200],   # C9  interior heavier + stiffer
    ...
    # ==================== CENTRE OF MASS (CM) ====================
    M_tot = np.sum(mass_all)
    x_CM = np.sum(mass_all * x_all) / M_tot
    y_CM = np.sum(mass_all * y_all) / M_tot
    ...
    print("       (12 columns + 4 shear walls)")
    print("="*60)
    ...
    for i in range(len(shear_walls)):
        ...
        if orient == 0:          # length along x
            ...
            rect = Rectangle((xw - tw/2, yw - Lw/2), tw, Lw,
                             edgecolor='red', facecolor='salmon', alpha=0.6, linewidth=2)
        ax.add_patch(rect)
        # Tag wall number; position slightly above the wall's centroid if the wall is long,
        # else keep centred (font size adjusted for visibility)
    ...
    ax.set_title('Structural Plan: 12 Columns + 4 Shear Walls\n(irregular mass & stiffness)')
    ax.set_xlabel('x [mm]')
    ax.set_ylabel('y [mm]')
    ax.legend()
...
PLAN_IRREGULARITY_MASS_STIFFNESS_ANALYSIS_FUN(storey_height=3000)