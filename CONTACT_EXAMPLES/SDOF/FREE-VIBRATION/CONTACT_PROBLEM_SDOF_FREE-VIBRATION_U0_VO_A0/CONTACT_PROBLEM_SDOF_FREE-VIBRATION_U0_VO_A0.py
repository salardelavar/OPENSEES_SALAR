...
#%% ---------------------------------------------
def FREE_VIBRATION_SDOF(IA, IU, IV, SPRING_TYPE, u0, v0, a0, duration, dt, GAP_DIST):
    #%% Create model
    ops.wipe()
    ...
    DATA = (time, reaction, disp, vel, accel, stiffness,
            ...
            force1, force2,
            np.array(PERIOD_MIN), np.array(PERIOD_MAX), np.array(DI))
...
GAP_DIST = 0.025  # [m] Gap Distance (Contact Distance)
...
(time, reactionE, dispE, velE, accelE, stiffnessE,
 ...
 force1E, force2E,
 period_minE, period_maxE, diE) = DATA
...
(time, reactionI, dispI, velI, accelI, stiffnessI,
 ...
 force1I, force2I,
 period_minI, period_maxI, diI) = DATA
...
# Save to Excel
with pd.ExcelWriter("CONTACT_PROBLEM_SDOF_FREE-VIBRATION_U0_VO_A0_OUTPUT.xlsx", engine='openpyxl') as writer:
