...
ops.uniaxialMaterial('Viscous', 3, -C, 2.0) # Material for C (alpha=2.0 for nonlinear)
ops.uniaxialMaterial('Viscous', 4, C, 1.0)  # Material for C (alpha=1.0 for linear)  
...
ops.element('zeroLength', 1, 1, 2, '-mat', 1, 2, 3, 4, '-dir', 1, 1, 1, 1)
#------------------------------------------------------------------------------------------------
# Define damping
...
plt.ylabel("Force (kN)")
plt.title("Hysteretic Force-Displacement Curve for SDOF Stiffness Spring")
plt.legend()
plt.grid(True)
plt.show()
