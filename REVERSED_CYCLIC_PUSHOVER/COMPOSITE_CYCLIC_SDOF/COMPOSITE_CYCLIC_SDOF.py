...
# REVERSED CYCLIC PUSHOVER ANALYSIS OF SINGLE DEGREE OF FREEDOM (SDOF) REINFORCED CONCRETE SECTION WITH   #
#             PLATES EVALUATING STRAIN HARDENING AND ULTIMATE STRAIN CRITERIA USING OPENSEES              #
#---------------------------------------------------------------------------------------------------------#
...
def CYCLIC_LOADING(num_cycles, samples_per_cycle, MAX_VALUE, EXPO=False, exponent=2):
    ...
    if not EXPO:
        ...
        plt.ylabel('Strain or Curvature Amplitude')
        plt.grid(True)
        plt.tight_layout()
        plt.show()
...
MAX_VALUE = 0.005 # [mm] MAXIMUM STRAIN VALUE
TIME, AMP = CYCLIC_LOADING(num_cycles, samples_per_cycle, MAX_VALUE, EXPO=True, exponent=2) # Using this for Cyclic Strain
...
eyP = fyP/EsP      # [mm/mm] Steel Plate Yield Strain
fuP = 1.1818*fyP   # [N/mm²] Steel Plate Ultimate Strength
esuP = eyP*41.8    # [mm/mm] Steel Plate Ultimate Strain
...
#%%------------------------------------------------------------------------------
def CYCLIC_ANALYSIS(b, h, cover, Es, fy, As, ECU, CYCL, NUM, STEEL_KIND):
    # Reset model
    ops.wipe()
    ...
    if STEEL_KIND == 2:# WITH HARDENING AND ULTIMATE STRAIN    
        ...
        # INFO LINK: https://opensees.berkeley.edu/wiki/index.php/ZeroLengthSection_Element
    ...
    # INFO LINK: https://opensees.berkeley.edu/wiki/index.php/ZeroLength_Element
    ...
    for step in range(steps):
        ...
        # Record results
        ops.reactions()
        S = ops.nodeReaction(1, 2) # SHEAR BASE REACTION
        A = ops.nodeReaction(1, 1) # AXIAL BASE REACTION
        ...
        ROT.append(rot)
        KS.append(np.abs(S)/np.abs(disp_Y)) # LATERAL STIFFNESS IN Y
...
FORCE_S, FORCE_A, MOMENT, DISP_X, DISP_Y, ROT, KA, KS, KI, STEP = CYCLIC_ANALYSIS(B, H, cover, Es, fy, As, esu, CYCL=AMP, NUM=len(AMP), STEEL_KIND=1)
# WITH HARDENING AND ULTIMATE STRAIN
FORCE_S02, FORCE_A02, MOMENT02, DISP_X02, DISP_Y02, ROT02, KA02, KS02, KI02, STEP02  = CYCLIC_ANALYSIS(B, H, cover, Es, fy, As, esu, CYCL=AMP, NUM=len(AMP), STEEL_KIND=2)
...
plt.plot(DISP_Y, FORCE_S, color='green', linewidth=2)
plt.plot(DISP_Y02, FORCE_S02, color='lime', linestyle='--', linewidth=2)
#plt.scatter(DISP_Y, FORCE_S, color='purple', linewidth=2)
#plt.scatter(DISP_Y02, FORCE_S02, color='magenta', linestyle='--', linewidth=2)
...
#plt.scatter(DISP_X, FORCE_A, color='purple', linewidth=2)
#plt.scatter(DISP_X02, FORCE_A02, color='magenta', linestyle='--', linewidth=2)
...
results_df.to_excel('COMPOSITE_CYCLIC_SDOF_RESULTS.xlsx', index=False) 
