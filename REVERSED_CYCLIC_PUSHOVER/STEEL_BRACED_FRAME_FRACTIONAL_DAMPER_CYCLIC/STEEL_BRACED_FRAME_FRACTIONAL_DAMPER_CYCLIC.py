...
# FRACTIONAL DAMPER
# Key envelope points
FY = 100e4   # [N] Yield force
EY = 1.0     # [mm] Yield displacement
FU = 150e4   # [N] Ultimate force
EU = 10.0    # [mm] Ultimate displacement
...
def CYCLIC_ANALYSIS(LENGTH_COL, LENGTH_BM, CYCL, NUM, STEEL_KIND):
    ...
    # Pinching parameters
    pinchXFD  = 0.1   # 10% pinching in displacement
    pinchYFD  = 0.2   # 20% strength loss under pinching
    # Damage parameters (no cyclic degradation)
    damage1FD = 0.0   # Strength deterioration
    damage2FD = 0.0   # Stiffness deterioration
    # Fractional exponent (smoothness of backbone)
    betaFD    = 0.2
    ops.uniaxialMaterial('Hysteretic', FDmatTag, FY, EY, FU, EU, 0.2*FU, 1.35*EU,
                                                -FY, -EY, -FU, -EU, -0.32*FU, -1.21*EU,
                                                pinchXFD, pinchYFD, damage1FD, damage2FD, betaFD)
    ...
    ops.element('zeroLength', 7, 5, 5, '-mat', FDmatTag, FDmatTag, '-dir', 1, 2)
