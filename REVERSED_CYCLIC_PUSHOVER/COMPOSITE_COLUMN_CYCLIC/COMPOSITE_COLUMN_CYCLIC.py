...
def CYCLIC_ANALYSIS(LENGTH, b, h, cover, Es, fy, As, ECU, CYCL, NUM, STEEL_KIND, num_cycles=3, amp_curv=0.000001):
    ...
    return FORCE_S, FORCE_A, MOMENT, DISP_X, DISP_Y, ROT, KI, STEP

#%%------------------------------------------------------------------------------
# WITHOUT HARDENING AND ULTIMATE STRAIN
FORCE_S, FORCE_A, MOMENT, DISP_X, DISP_Y, ROT, KI, STEP = CYCLIC_ANALYSIS(LENGTH, B, H, cover, Es, fy, As, esu, CYCL=AMP, NUM=len(AMP), STEEL_KIND=1)
# WITH HARDENING AND ULTIMATE STRAIN
FORCE_S02, FORCE_A02, MOMENT02, DISP_X02, DISP_Y02, ROT02, KI02, STEP02  = CYCLIC_ANALYSIS(LENGTH, B, H, cover, Es, fy, As, esu, CYCL=AMP, NUM=len(AMP), STEEL_KIND=2)
...
#plt.plot(KI, np.abs(FORCE_A), color='black', linewidth=2)
#plt.plot(KI02, np.abs(FORCE_A02), color='grey', linestyle='--', linewidth=2)
plt.scatter(KI, np.abs(FORCE_A), color='black', linewidth=2)
plt.scatter(KI02, np.abs(FORCE_A02), color='grey', linestyle='--', linewidth=2)
