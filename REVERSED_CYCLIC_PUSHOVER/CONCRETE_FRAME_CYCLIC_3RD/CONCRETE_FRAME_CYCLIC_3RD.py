...
#               REVERSED CYCLIC PUSHOVER ANALYSIS OF THREE-STORY CONCRETE FRAME SECTION                   #
...
def CYCLIC_ANALYSIS(LENGTH_COL, LENGTH_BM, CYCL, NUM, STEEL_KIND):
    ...
    ops.node(3, 0.0, LENGTH_COL)           # FIRST STORY
    ops.node(4, LENGTH_BM, LENGTH_COL)     # FIRST  STORY
    ops.node(5, 0.0, 2*LENGTH_COL)         # SECOND STORY
    ops.node(6, LENGTH_BM, 2*LENGTH_COL)   # SECOND STORY
    ops.node(7, 0.0, 3*LENGTH_COL)         # THIRD STORY
    ops.node(8, LENGTH_BM, 3*LENGTH_COL)   # THIRD STORY
    ops.fix(1, 1, 1, 1)
    ops.fix(2, 1, 1, 1)
    ...
    # STORY  TWO
    ops.element('nonlinearBeamColumn', 4, 3, 5, numIntgrPts, secTagC, transfTag) # COLUMN 03
    ops.element('nonlinearBeamColumn', 5, 4, 6, numIntgrPts, secTagC, transfTag) # COLUMN 04
    ops.element('nonlinearBeamColumn', 6, 5, 6, numIntgrPts, secTagB, transfTag) # BEAM 02
    # STORY THREE
    ops.element('nonlinearBeamColumn', 7, 5, 7, numIntgrPts, secTagC, transfTag) # COLUMN 05
    ops.element('nonlinearBeamColumn', 8, 6, 8, numIntgrPts, secTagC, transfTag) # COLUMN 06
    ops.element('nonlinearBeamColumn', 9, 7, 8, numIntgrPts, secTagB, transfTag) # BEAM 03
    ...
    ops.load(6, 1.0, -1.0, 0.0)
    ops.load(7, 1.0, -1.0, 0.0)
    ops.load(8, 1.0, -1.0, 0.0)
    ...
    for step in range(steps):
        ...
        ops.integrator('DisplacementControl', 3, 1, (1/3)*CYCL[step]) 
        ops.integrator('DisplacementControl', 4, 1, (1/3)*CYCL[step]) 
        ops.integrator('DisplacementControl', 5, 1, (2/3)*CYCL[step]) 
        ops.integrator('DisplacementControl', 6, 1, (2/3)*CYCL[step])
        ops.integrator('DisplacementControl', 7, 1, (3/3)*CYCL[step]) 
        ops.integrator('DisplacementControl', 8, 1, (3/3)*CYCL[step])
        OK = ops.analyze(1)
        ...
        #print(rot, M)
        disp_X = ops.nodeDisp(7, 1) # LATERAL DISPLACEMENT IN X FOR NODE 7
        disp_Y = ops.nodeDisp(7, 2) # LATERAL DISPLACEMENT IN Y FOR NODE 7
        rot = ops.nodeDisp(7, 3)    # ROTATION IN Z FOR NODE 7
        FORCE_S.append(S)
        FORCE_A.append(A)
        MOMENT.append(M)
...
plt.xlabel('Displacement in X - STORY 3 [mm]')
plt.legend(['Steel01: WITHOUT HARDENING AND ULTIMATE STRAIN', 'Hysteretic: WITH HARDENING AND ULTIMATE STRAIN'])
plt.grid()
plt.show()
...
plt.ylabel('Axial Force [N]')
plt.xlabel('Displacement in Y - STORY 3 [mm]')
plt.legend(['Steel01: WITHOUT HARDENING AND ULTIMATE STRAIN', 'Hysteretic: WITH HARDENING AND ULTIMATE STRAIN'])
plt.grid()
plt.show()
...
plt.ylabel('Displacement - X - STORY 3 [mm]')
plt.xlabel('Steps')
plt.legend(['Steel01: WITHOUT HARDENING AND ULTIMATE STRAIN', 'Hysteretic: WITH HARDENING AND ULTIMATE STRAIN'])
plt.grid()
plt.show()
...
#plt.scatter(STEP02, DISP_X02, color='gold', linestyle='--', linewidth=2)
plt.title('Displacement During the Analysis')
plt.ylabel('Displacement - Y - STORY 3 [mm]')
plt.xlabel('Steps')
plt.legend(['Steel01: WITHOUT HARDENING AND ULTIMATE STRAIN', 'Hysteretic: WITH HARDENING AND ULTIMATE STRAIN'])
plt.grid()
plt.show()
...
results_df.to_excel('CONCRETE_FRAME_CYCLIC_3RD_RESULTS.xlsx', index=False) 
